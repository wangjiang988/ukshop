...
class publishControl extends MircroShopControl{
    ...
    public function goods_saveOp() {
        ...
        if(empty($_POST['commend_message'])) { 
            $commend_goods_info['commend_message'] = Uk86Language::uk86_get('microshop_goods_default_commend_message');
        } else {
        ...
        if(empty($goods_relation)) {
            ...
            if(!empty($default_class)) {
                ...
                if(empty($goods_class)) {
                    showDialog(Uk86Language::uk86_get('microshop_goods_class_none'),'reload','error','');
                } else {
        ...
        $message = Uk86Language::uk86_get('nc_common_save_fail');
            //分享内容
            if($result) {
                $message = Uk86Language::uk86_get('nc_common_save_succ');
                //计数
    ...
    public function personal_image_uploadOp() {
        ...
        if(isset($_SESSION['member_id'])) {
            if(!empty($_FILES['personal_image_ajax']['name'])) {
                ...
                $upload->uk86_set('default_dir',ATTACH_MICROSHOP.DS.$_SESSION['member_id']);
                $upload->uk86_set('thumb_width','60,240');
                $upload->uk86_set('thumb_height', '5000,50000');
                $upload->uk86_set('thumb_ext',	'_tiny,_list');	

                $result = $upload->uk86_upfile('personal_image_ajax');
                if(!$result) {
                    $data['status'] = 'fail';
    ...
    public function personal_limitOp() {
        ...
        if($result) {
            ...
            self::return_json(Uk86Language::uk86_get('micro_personal_limit_error'),'false');
        }
    }
    ...
    public function personal_link_addOp() {
        ...
        if(empty($link)) {
            self::return_json(Uk86Language::uk86_get('wrong_argument'),'false');
        }
        ...
        if($result) {
            ...
            self::return_json(Uk86Language::uk86_get('microshop_wrong_url'),'false');
        }
    }
    ...
    public function personal_saveOp() {
        ...
        if(!$personal_limit) {
            ...
            showDialog(Uk86Language::uk86_get('micro_personal_limit_error'),'','error','');
        }
        ...
        if(empty($_POST['commend_message'])) { 
            $personal_info['commend_message'] = Uk86Language::uk86_get('microshop_personal_default_commend_message');
        } else {
