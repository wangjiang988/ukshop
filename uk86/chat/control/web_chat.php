...
class web_chatControl extends BaseControl {
	public function __construct(){
		parent::__construct();
		Uk86Language::uk86_read('member_chat');
	...
	public function send_msgOp(){
		...
		if (($member_id < 1) || ($member_id != $f_id)) $this->error(Uk86Language::uk86_get('nc_member_chat_login'));
		$member = $model_chat->getMember($t_id);
		if ($t_name != $member['member_name']) $this->error(Uk86Language::uk86_get('nc_member_chat_name_error'));
		...
		if($chat_msg['m_id']) {
			...
			$this->error(Uk86Language::uk86_get('nc_member_chat_add_error'));
		}
	}
	/**
	 * friends info
	...
	public function get_infoOp(){
		if ($_SESSION['member_id'] < 1) $this->error(Uk86Language::uk86_get('nc_member_chat_login'));
		$val = '';
		$member = array();
		$model_chat	= Model('web_chat');
	...
	public function json($json){
		if (strtoupper(CHARSET) == 'GBK'){
			$json = Uk86Language::uk86_getUTF8($json);//GBKtoUTF-8
		}
