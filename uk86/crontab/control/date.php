...
//         $this->_add_points();
    ...
    private function _add_points() {
        ...
        //24小时之内登录的会员送U币和经验值,每次最多处理5W个会员
       ...
       if (C('points_isuse')) {
        ...
        //24小时之内完成了实物订单送U币和经验值,每次最多处理5W个订单
        $model_order = Model('order');
        $condition = array();
        ...
        if (!empty($list)) {
            foreach ($list as $val) {
                ...
                $param['param'] = array(
                    ...
                    'voucher_url' => uk86_urlShop('member_voucher', 'index')
                );
        ...
        if (!empty($list)) {
            foreach ($list as $val) {
                ...
                $param['param'] = array(
                    ...
                    'vr_order_url' => uk86_urlShop('member_vr_order', 'index')
                );
                Uk86QueueClient::push('sendMemberMsg', $param);
            }
        }
    ...
    private function _goods_click_update() {
        $data = uk86_rcache('updateRedisDate', 'goodsClick');
        foreach ($data as $key => $val) {
            Model('goods')->editGoodsById(array('goods_click' => array('exp', 'goods_click +'.$val)), $key);
        }
        uk86_dcache('updateRedisDate', 'goodsClick');
    }
            ...
            for ($i=0; $i<$countnum; $i+=$eachnum){//每次查询100条
                ...
                foreach ((array)$member_list as $k=>$v){
                    ...
                    if ($browse_goodsid) {
                        ...
                        //处理存入数据库的浏览历史缓存信息
                        $_cache = uk86_rcache($hash_key, 'goodsbrowse');
                        ...
                        //重新赋值浏览历史缓存
                        uk86_dcache($hash_key, 'goodsbrowse');
