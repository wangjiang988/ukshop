...
class publishControl extends CMSMemberControl{
	...
	public function publish_article_saveOp(){
        if(empty($_POST['article_title'])) {
            uk86_showMessage(Uk86Language::uk86_get('article_title_null'),'','','error');
        }
        //插入文章
        $param = array();
        ...
        if(!empty($_POST['article_image_all'])) {
            ...
            foreach ($_POST['article_image_all'] as $value) {
                ...
                $article_image_url = uk86_getCMSArticleImageUrl($file_path, $file_name, 'max');
        ...
        if($article_id) {
            uk86_showMessage(Uk86Language::uk86_get('nc_common_save_succ'),self::CMS_MEMBER_ARTICLE_URL);
        } else {
            uk86_showMessage(Uk86Language::uk86_get('nc_common_save_fail'),self::CMS_MEMBER_ARTICLE_URL,'','error');
        }
    }
    ...
    public function publish_picture_saveOp() {
        ...
        if(!empty($_POST['picture_image_all'])) {
            ...
            for ($i = 0; $i < $image_count; $i++) {
                ...
                $picture_image_url = uk86_getCMSArticleImageUrl($file_path, $file_name, 'max');
        ...
        if($picture_id) {
            uk86_showMessage(Uk86Language::uk86_get('nc_common_save_succ'),self::CMS_MEMBER_PICTURE_URL);
        } else {
            uk86_showMessage(Uk86Language::uk86_get('nc_common_save_fail'),self::CMS_MEMBER_PICTURE_URL,'','error');
        }
