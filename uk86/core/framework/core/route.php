...
class Uk86Route {
     ...
     *
     */
    public function Uk86_parseRule() {
        if ($this->_config['url_model']) {
            $this->_uk86_parseRuleRewrite();
        } else {
            $this->_uk86_parseRuleNormal();
        }
    }
     ...
     *
     */
    private function _uk86_parseRuleNormal() {
        //不进行任何处理
    }
     ...
     *
     */
    private function _uk86_parseRuleRewrite() {
     ...
     * @return mixed
     */
    private function _Uk86_shopPathInfo($path_info) {
     ...
     * @return mixed
     */
    private function _uk86_cmsPathInfo($path_info) {
        $reg_match_from = array(
            '/^article-(\d+)$/',
            '/^picture-(\d+)$/'
        );
