 ...
 * @return string
 */
function uk86_thumb($goods = array(), $type = ''){
    $type_array = explode(',_', ltrim(GOODS_IMAGES_EXT, '_'));
    if (!in_array($type, $type_array)) {
        $type = '240';
    }
    if (empty($goods)){
        return UPLOAD_SITE_URL.'/'.uk86_defaultGoodsImage($type);
    }
 ...
 * @return string
 */
function uk86_cthumb($file, $type = '', $store_id = false) {
    $type_array = explode(',_', ltrim(GOODS_IMAGES_EXT, '_'));
    if (!in_array($type, $type_array)) {
        $type = '240';
    }
 ...
 * @return string
 */
function uk86_goodsQRCode($goods_info) {
 ...
 * @return string
 */
function uk86_gthumb($image_name = '', $type = ''){
 ...
 * @return string
 */
function uk86_snsThumb($image_name = '', $type = ''){
 ...
 * @return string
 */
function uk86_brandImage($image_name = '') {
    if ($image_name != '') {
...
*/
function uk86_orderState($order_info) {
 ...
 * @return string
 */
function uk86_orderPaymentName($payment_code) {
    return str_replace(
 ...
 * @return string 描述输出
 */
function uk86_orderGoodsType($goods_type) {
    return str_replace(
            array('1','2','3','4','5'),
 ...
 * @return string 描述输出
 */
function uk86_billState($bill_state) {
    return str_replace(
            array('1','2','3','4'),
