...
function uk86_checkSeccode($nchash,$value){
	...
	if (!$return) uk86_setNcCookie('seccode'.$nchash,'',-3600);
	return $return;
}

/**
 * 设置cookie QQ wap 登录  byukshop.com 
 *
 ...
 */
function uk86_setNc2Cookie($name, $value, $expire='3600', $path='', $domain='', $secure=false){
    if (empty($path)) $path = '/';
 ...
 * @param string $secure 是否通过安全的 HTTPS 连接来传输 cookie,默认为false
 */
function uk86_setNcCookie($name, $value, $expire='3600', $path='', $domain='', $secure=false){
	if (empty($path)) $path = '/';
	if (empty($domain)) $domain = SUBDOMAIN_SUFFIX ? SUBDOMAIN_SUFFIX : '';
 ...
 * @return void
 */
function uk86_requestNotFound($act = null, $op = null) {
    uk86_showMessage('您访问的页面不存在！', SHOP_SITE_URL, 'exception', 'error', 1, 3000);
    exit;
}
 ...
 * @param number $time
 */
function uk86_wap_showDialog($msg, $succ='succ', $url='', $is_url = true, $time = 2, $exit = true){
	if($is_url){
		if($url == '' || empty($url)){
			$url = uk86_getReferer();
		}
 ...
 */
function uk86_showMessage($msg,$url='',$show_type='html',$msg_type='succ',$is_show=1,$time=2000){
	if(is_mobile()){
		global $config;
		if(!empty($config['wap_site_url'])){
			uk86_wap_showDialog($msg, $msg_type, $url);
		}
	}
	if (!class_exists('Uk86Language')) uk86_import('libraries.uk86language');
	Uk86Language::uk86_read('core_lang_index');
	$lang	= Uk86Language::uk86_getLangContent();
	/**
	 * 如果默认为空，则跳转至上一步链接
	 */
	$url = ($url!='' ? $url : uk86_getReferer());
	...
	switch ($show_type){
		...
		default:
			...
			if (is_array($url)){
				foreach ($url as $k => $v){
					$url[$k]['url'] = $v['url']?$v['url']:uk86_getReferer();
				}
			}
			/**
			 * 读取信息布局的语言包
			 */
			Uk86Language::uk86_read("msg");
			/**
			 * html输出形式
			 * 指定为指定项目目录下的error模板文件
			 */
            Tpl::setDir('');
			Tpl::output('html_title',Uk86Language::uk86_get('nc_html_title'));
			Tpl::output('msg',$msg);
			Tpl::output('url',$url);
...
function showDialog($message = '', $url = '', $alert_type = 'error', $extrajs = '', $time = 2){
	if (empty($_GET['inajax'])){
		if ($url == 'reload') $url = '';
		uk86_showMessage($message.$extrajs,$url,'html',$alert_type,1,$time*1000);
	}
 ...
 * @return string 字符串类型的返回结果
 */
function uk86_getNchash($act = '', $op = ''){
 ...
 * @return string 字符串类型的返回结果
 */
function uk86_decrypt($txt, $key = '', $ttl = 0){
	if (empty($txt)) return $txt;
	if (empty($key)) $key = md5(MD5_KEY);
 ...
 * @return obj 对象形式的返回结果
 */
function uk86_Logic($model = null, $base_path = null){
    static $_cache = array();
    $cache_key = $model.'.'.$base_path;
    if (!is_null($model) && isset($_cache[$cache_key])) return $_cache[$cache_key];
    $base_path = $base_path == null ? BASE_DATA_PATH : $base_path;
 ...
 */
function uk86_readDirList($path){
	if (is_dir($path)) {
		$handle = @opendir($path);
		$dir_list = array();
		if ($handle){
 ...
 * @return string 字符串类型的返回结果
 */
function uk86_replaceSpecialChar($string){
	$str = str_replace("\r\n", "", $string);
 ...
 */
function uk86_showEditor($id, $value='', $width='700px', $height='300px', $style='visibility:hidden;',$upload_state="true", $media_open=false, $type='all'){
	//是否开启多媒体
	$media = '';
	if ($media_open){
		$media = ", 'flash', 'media'";
...
*/
function uk86_ncPriceFormat($price) {
...
* @return string	$price_format
*/
function uk86_ncPriceFormatForList($price) {
    if ($price >= 10000) {
 ...
 * @return string 通知内容
 */
function uk86_ncReplaceText($message,$param){
 ...
 * @param string $dot 尾缀
 */
function uk86_str_cut($string, $length, $dot = '')
{
 ...
 * @return string
 */
function uk86_unicodeToUtf8($str, $order = "little")
{
	$utf8string ="";
	$n=strlen($str);
	for ($i=0;$i<$n ;$i++ )
 ...
 */
if(!function_exists('uk86_memory_get_usage')){
	function uk86_memory_get_usage(){//目前程序不兼容5以下的版本
		return 0;
	}
}

// 记录和统计时间（微秒）
function uk86_addUpTime($start,$end='',$dec=3) {
    static $_info = array();
    if(!empty($end)) { // 统计时间
        if(!isset($_info[$end])) {
 ...
 * @return string
 */
function uk86_defaultGoodsImage($key){
 ...
 * @return string
 */
function uk86_getMemberAvatar($member_avatar){
 ...
 * @return string
 */
function uk86_getMemberAvatarForID($id){
 ...
 * @return string
 */
function uk86_getMemberNameForId($id){
	$info = Model()->table('member')->where(array('member_id' => $id))->field('member_name')->find();
 ...
 * @return string
 */
function uk86_getStoreLogo($img, $type = 'store_avatar'){
 ...
 * 获取文章URL
 */
function uk86_getCMSArticleUrl($article_id) {
    if(URL_MODEL) {
        // 开启伪静态
 ...
 * 获取画报URL
 */
function uk86_getCMSPictureUrl($picture_id) {
    if(URL_MODEL) {
        // 开启伪静态
 ...
 * 获取文章图片URL
 */
function uk86_getCMSArticleImageUrl($image_path, $image_name, $type='list') {
    if(empty($image_name)) {
 ...
 * 获取文章图片URL
 */
function uk86_getCMSImageName($image_name_string) {
 ...
 * 获取CMS专题图片
 */
function uk86_getCMSSpecialImageUrl($image_name='') {
 ...
 * 获取CMS专题路径
 */
function uk86_getCMSSpecialImagePath($image_name='') {
 ...
 * 获取CMS首页图片
 */
function uk86_getCMSIndexImageUrl($image_name='') {
 ...
 * 获取CMS首页图片路径
 */
function uk86_getCMSIndexImagePath($image_name='') {
 ...
 * 获取CMS专题Url
 */
function uk86_getCMSSpecialUrl($special_id) {
 ...
 * 获取商城专题Url
 */
function uk86_getShopSpecialUrl($special_id) {
 ...
 * 获取CMS专题静态文件
 */
function uk86_getCMSSpecialHtml($special_id) {
 ...
 * 获取微商城个人秀图片地址
 */
function uk86_getMicroshopPersonalImageUrl($personal_info,$type=''){
    ...
    if(!empty($personal_image_list)){
        foreach ($personal_image_list as $value) {
            ...
            if(is_file(BASE_UPLOAD_PATH.DS.ATTACH_MICROSHOP.DS.$file_path)) {
                ...
                $personal_image_array[] = uk86_getMicroshopDefaultImage();
            }
        }
    } else {
        $personal_image_array[] = uk86_getMicroshopDefaultImage();
...
function uk86_getMicroshopDefaultImage() {
 ...
 * 获取开店申请图片
 */
function uk86_getStoreJoininImageUrl($image_name='') {
 ...
 * 获取开店装修图片地址
 */
function uk86_getStoreDecorationImageUrl($image_name = '', $store_id = null) {
    if(empty($store_id)) {
 ...
 * 获取运单图片地址
 */
function uk86_getWaybillImageUrl($image_name = '') {
 ...
 * 如果文件名中包含"_"使用"#"代替
 *
 * @example uk86_import('cache'); //require_once(BASE_PATH.'/framework/libraries/cache.php');
 * @example uk86_import('libraries.cache');	//require_once(BASE_PATH.'/framework/libraries/cache.php');
 * @example uk86_import('function.core');	//require_once(BASE_PATH.'/framework/function/core.php');
 * @example uk86_import('.control.adv')	//require_once(BASE_PATH.'/control/adv.php');
 *
 * @param 要加载的文件 $libname
 * @param 文件扩展名 $file_ext
 */
function uk86_import($libname,$file_ext='.php'){
	//替换为目录符号/
	if (strstr($libname,'.')){
 ...
 * @return string
 */
function uk86_random($length, $numeric = 0) {
 ...
 * @return string
 */
function uk86_template($tplpath){
 ...
 */
function uk86_showValidateError($error){
	if (!empty($_GET['inajax'])){
 ...
 */
function uk86_lazypage($delay_eachnum,$delay_page,$count,$ispage=false,$page_nowpage=1,$page_eachnum=1,$page_limitstart=1){
	//是否有多余
	$hasmore = true;
	$limitstart = 0;
	if ($ispage == true){
...
function F($name, $value = null, $path = 'cache', $ext = '.php') {
    ...
    if (!is_null($value)) {
        ...
        return uk86_write_file($filename,$value);
    }
 ...
 * @return bool
 */
function uk86_write_file($filepath, $data, $mode = null)
{
 ...
 * @return boolean
 */
function uk86_mk_dir($dir, $mode = '0777') {
    if (is_dir($dir) || @mkdir($dir, $mode))
        return true;
    if (!uk86_mk_dir(dirname($dir), $mode))
        return false;
...
function uk86_pagecmd($cmd ='', $arg = ''){
	if (!class_exists('uk86page'))	uk86_import('uk86page');
	static $page;
	if ($page == null){
		$page = new Uk86Page();
	}
	...
	switch (strtolower($cmd)) {
		case 'seteachnum':		$page->uk86_setEachNum($arg);break;
		case 'settotalnum': 	$page->uk86_setTotalNum($arg);break;
		case 'setstyle': 		$page->uk86_setStyle($arg);break;
		case 'show': 			return $page->uk86_show($arg);break;
		case 'obj':				return $page;break;
		case 'gettotalnum':		return $page->uk86_getTotalNum();break;
		case 'gettotalpage':	return $page->uk86_getTotalPage();break;
		default:				break;
	}
}
 ...
 */
function uk86_throw_exception($error){
	if (!defined('IGNORE_EXCEPTION')){
		uk86_showMessage($error,'','exception');
	}else{
		exit();
	}
}
 ...
 * @return string
 */
	function uk86_compress_code($content) {
    $stripStr = '';
    //分析php源码
 ...
 * @return object
 */
function uk86_get_obj_instance($class, $method='', $args = array()){
	static $_cache = array();
	...
	if (isset($_cache[$key])){
		...
		if (class_exists($class)){
			...
			uk86_throw_exception('Class '.$class.' isn\'t exists!');
		}
	}
}
 ...
 * @return array
 */
function uk86_array_under_reset($array, $key, $type=1){
	if (is_array($array)){
		$tmp = array();
		foreach ($array as $v) {
			if ($type === 1){
 ...
 * @return mixed
 */
function uk86_rkcache($key, $callback = false)
{
    if (C('cache_open')) {
    ...
    if (!$cacher) {
        throw new uk86_rkcache('Cannot fetch cache object!');
    }
   $value = $cacher->get($key);
//     $cachefile = new Uk86CacheFile();
//     $value = $cachefile->uk86_get($key);
    ...
    if ($value === false && $callback !== false) {
        ...
        if (!is_callable($callback)) {
            throw new Exception('Invalid uk86_rkcache callback!');
        }

        $value = call_user_func($callback, $key);
        uk86_wkcache($key, $value);
    }
 ...
 * @return boolean
 */
function uk86_wkcache($key, $value, $expire = null)
{
    if (C('cache_open')) {
        $cacher = Cache::getInstance('cacheredis');
    } else {
...
//     $cachefile = new Uk86CacheFile();
//     return $cachefile->uk86_set($key, $value, null, $expire);
}
/**
 * KV缓存 删
 * 
 ...
 * @return array/bool
 */
function uk86_rcache($key = null, $prefix = '', $fields = '*'){
 ...
 * @return bool 返回值
 */
function uk86_wcache($key = null, $data = array(), $prefix, $period = 0){
 ...
 * @return boolean
 */
function uk86_dcache($key = null, $prefix = ''){
 ...
 * @return string 推荐位内容
 */
function uk86_rec($rec_id = null){
	uk86_import('function.uk86_rec_position');
	return uk86_rec_position($rec_id);
}
 ...
 */
function L($key = ''){
	if (class_exists('Uk86Language')){
		if (strpos($key,',') !== false){
			$tmp = explode(',',$key);
			$str = Uk86Language::uk86_get($tmp[0]).Uk86Language::uk86_get($tmp[1]);
			return isset($tmp[2])? $str.Uk86Language::uk86_get($tmp[2]) : $str;
		}else{
			return Uk86Language::uk86_get($key);
		}
	}else{
		return null;
	}
 ...
 */
function uk86_loadfunc($filename, $file_ext = '.php'){
 ...
 */
function uk86_loadadv($ap_id = null, $type = 'html'){
	if (!is_numeric($ap_id)) return false;
	if (!function_exists('uk86_advshow')) uk86_import('function.adv');
	return uk86_advshow($ap_id,$type);
}
 ...
 * @return string
 */
function uk86_removeUBBTag($ubb){
	$ubb = str_replace(array(
 ...
 * @return string
 */
function uk86_themeImagePath($param){
 ...
 * @return string
 */
function uk86_themeImageUrl($param){
 ...
 * @return string
 */
function uk86_snsShareFrom($sign) {
    switch ($sign) {
        case '1' :
        case '2' :
...
function uk86_getChat($layout){
	if (!C('node_chat') || !file_exists(BASE_CORE_PATH.'/framework/libraries/uk86chat.php')) return 'error';
	if (!class_exists('Uk86Chat')) uk86_import('libraries.chat');
 ...
 *
 * 若指向网站首页，可以传空:
 * uk86_url() => 表示act和op均为index，返回当前站点网址
 *
 * uk86_url('search,'index','array('cate_id'=>2)); 实际指向 index.php?act=search&op=index&cate_id=2
 * 传递数组参数时，若act（或op）值为index,则可以省略
 * 上面示例等同于
 * uk86_url('search','',array('act'=>'search','cate_id'=>2));
 *
 ...
 * @return string
 */
function uk86_url($act = '', $op = '', $args = array(), $model = false, $site_url = ''){
    //伪静态文件扩展名
    $ext = '.html';
    //入口文件名
    $file = 'index.php';
 ...
 * @return string
 */
function uk86_urlShop($act = '', $op = '', $args = array(), $store_domain = ''){
...
//    }
    ...
    return uk86_url($act, $op, $args, $rewrite_flag, SHOP_SITE_URL);
}
 ...
 * @return string
 */
function uk86_urlAdmin($act = '', $op = '', $args = array()){
    return uk86_url($act, $op, $args, false, ADMIN_SITE_URL);
}
/**
 ...
 * @return string
 */
function uk86_urlCMS($act = '', $op = '', $args = array()){
    return uk86_url($act, $op, $args, false, CMS_SITE_URL);
}
/**
 ...
 * @return string
 */
function uk86_urlCircle($act = '', $op = '', $args = array()){
    return uk86_url($act, $op, $args, false, CIRCLE_SITE_URL);
}
/**
 ...
 * @return string
 */
function uk86_urlMicroshop($act = '', $op = '', $args = array()){
    return uk86_url($act, $op, $args, false, MICROSHOP_SITE_URL);
}
/**
 * 验证是否为平台店铺
 *
 * @return boolean
 */
function uk86_checkPlatformStore(){
 ...
 * @return string
 */
function uk86_getStoreStateClassName($store_info) {
    $result = 'open';
 ...
 * @return string
 */
function uk86_encryptShow($str,$start,$length) {
    $end = $start - 1 + $length;
    $array = str_split($str);
 ...
 */
function uk86_callback($state = true, $msg = '', $data = array()) {
    return array('state' => $state, 'msg' => $msg, 'data' => $data);
}

function uk86_getOrderTimeById($order_id){
	$condetion = array('order_id' => $order_id);
	$result = Model()->table('order')->where($condetion)->field('add_time')->find();
 ...
 */
function uk86_getNumText($text, $num){
	if(strlen($text) <= $num){
		return $text;
	}
	$array = array();
 ...
 * @return string
 */
function uk86_replaceUBBTag($ubb, $video_sign = 1){
	if($video_sign){
