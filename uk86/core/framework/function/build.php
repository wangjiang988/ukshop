...
function uk86_build(){
	...
	$list = array(
		...
		BASE_PATH.'/framework/libraries/uk86language.php',
...
//	}
	$compile .= uk86_compile($lang_file);
	$compile .= "\nUk86Language::uk86_appendLanguage(\$lang);";

	$compile .= "\nBase::run();";
	file_put_contents(RUNCOREPATH,uk86_compress_code("<?php defined('InUk86') or exit('Access Invalid!');".$compile));
}
 ...
 * @return string
 */
function uk86_compile($filename) {
    $content = file_get_contents($filename);
    //过滤不需要编译的内容
    $content = preg_replace('/\/\/\[NC_SKIPBUILD\](.*?)\/\/\[\/NC_SKIPBUILD\]/s', '', $content);
    $content = str_ireplace("defined('InUk86') or exit('Access Invalid!')", '', $content);
