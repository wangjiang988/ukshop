...
$C['anti_link_spam'] = (isset($C['anti_link_spam']) && is_array($C['anti_link_spam']) && count($C['anti_link_spam']) == 2 && (empty($C['anti_link_spam'][0]) or uk86_hl_regex($C['anti_link_spam'][0])) && (empty($C['anti_link_spam'][1]) or uk86_hl_regex($C['anti_link_spam'][1]))) ? $C['anti_link_spam'] : 0;
...
if($C['cdata'] or $C['comment']){$t = preg_replace_callback('`<!(?:(?:--.*?--)|(?:\[CDATA\[.*?\]\]))>`sm', 'uk86_hl_cmtcd', $t);}
$t = preg_replace_callback('`&amp;([a-zA-Z][a-zA-Z0-9]{1,30}|#(?:[0-9]{1,8}|[Xx][0-9A-Fa-f]{1,7}));`', 'uk86_hl_ent', str_replace('&', '&amp;', $t));
...
// main
$t = preg_replace_callback('`<(?:(?:\s|$)|(?:[^>]*(?:>|$)))|>`m', 'uk86_hl_tag', $t);
$t = $C['balance'] ? uk86_hl_bal($t, $C['keep_bad'], $C['parent']) : $t;
$t = (($C['cdata'] or $C['comment']) && strpos($t, "\x01") !== false) ? str_replace(array("\x01", "\x02", "\x03", "\x04", "\x05"), array('', '', '&', '<', '>'), $t) : $t;
$t = $C['tidy'] ? uk86_hl_tidy($t, $C['tidy'], $C['parent']) : $t;
unset($C, $e);
...
function uk86_hl_bal($t, $do=1, $in='div'){
// balance tags
// by content
...
function uk86_hl_prot($p, $c=null){
// check URL scheme
global $C;
$b = $a = '';
...
for($i = count(($t = explode(';', $t))); --$i>=0;){
 ...
 foreach(explode(',', $a) as $v){
  ...
  if(isset($y[$x]['match']) && !uk86_hl_regex($y[$x]['match'])){unset($y[$x]['match']);}
  if(isset($y[$x]['nomatch']) && !uk86_hl_regex($y[$x]['nomatch'])){unset($y[$x]['nomatch']);}
 }
...
if($C['make_tag_strict'] && isset($eD[$e])){
 $trt = uk86_hl_tag2($e, $a, $C['make_tag_strict']);
...
foreach($aA as $k=>$v){
 if(((isset($C['deny_attribute']['*']) ? isset($C['deny_attribute'][$k]) : !isset($C['deny_attribute'][$k])) && (isset($aN[$k][$e]) or isset($aNU[$k]) or preg_match('`data-((?!xml)[^:]+$)`', $k)) && !isset($rl['n'][$k]) && !isset($rl['n']['*'])) or isset($rl[$k])){
  ...
  if($k == 'style' && !$C['style_pass']){
   ...
   $v = preg_replace_callback('`(url(?:\()(?: )*(?:\'|"|&(?:quot|apos);)?)(.+?)((?:\'|"|&(?:quot|apos);)?(?: )*(?:\)))`iS', 'uk86_hl_prot', $v);
  ...
  if(isset($rl[$k]) && is_array($rl[$k]) && ($v = uk86_hl_attrval($v, $rl[$k])) === 0){continue;}
  $a[$k] = str_replace('"', '&quot;', $v);
 }
}
