...
class Uk86UploadFile{
	 ...
	 *	$upload->uk86_set('default_dir','upload');
	 *	$upload->uk86_set('max_size',1024);
	 *	//生成4张缩略图，宽高依次如下
	 ...
	 *	//4张缩略图名称扩展依次如下
	 *	$upload->uk86_set('thumb_ext',	'_small,_mid,_max,_tiny');
	 *	//生成新图的扩展名为.jpg
	 *	$upload->uk86_set('new_ext','jpg');
	 *	//开始上传
	 ...
	 * @return bool
	 */
	public function uk86_upfile($field){

		//上传文件
		$this->upload_file = $_FILES[$field];
		if ($this->upload_file['tmp_name'] == ""){
			$this->setError(Uk86Language::uk86_get('cant_find_temporary_files'));
			return false;
		}

		//对上传文件错误码进行验证
		$error = $this->uk86_fileInputError();
		if (!$error){
			return false;
		}
		//验证是否是合法的上传文件
		if(!is_uploaded_file($this->upload_file['tmp_name'])){
			$this->setError(Uk86Language::uk86_get('upload_file_attack'));
			return false;
		}

		//验证文件大小
		if ($this->upload_file['size']==0){
			$error = Uk86Language::uk86_get('upload_file_size_none');
			$this->setError($error);
			return false;
		}
		if($this->upload_file['size'] > $this->max_size*1024){
			$error = Uk86Language::uk86_get('upload_file_size_cant_over').$this->max_size.'KB';
			$this->setError($error);
			return false;
		}
		...
		if(!in_array($this->ext,$this->allow_type)){
			$error = Uk86Language::uk86_get('image_allow_ext_is').implode(',',$this->allow_type);
			$this->setError($error);
			return false;
		}
		...
		if(!$image_info = @getimagesize($this->upload_file['tmp_name'])) {
			$error = Uk86Language::uk86_get('upload_image_is_not_image');
			$this->setError($error);
			return false;
		}
...
//				}
		...
		if(@move_uploaded_file($this->upload_file['tmp_name'],BASE_UPLOAD_PATH.DS.$this->save_path.DS.$this->file_name)){
			//产生缩略图
			if ($ifresize){
				$resizeImage	= new Uk86ResizeImage();
				$save_path = rtrim(BASE_UPLOAD_PATH.DS.$this->save_path,'/');
				for ($i=0;$i<count($thumb_width);$i++){
					$resizeImage->uk86_newImg($save_path.DS.$this->file_name,$thumb_width[$i],$thumb_height[$i],$scale[$i],$thumb_ext[$i].'.',$save_path,$this->filling);
					if ($i==0) {
			...
			$this->setError(Uk86Language::uk86_get('upload_file_fail'));
			return false;
		}
//		$this->setErrorFileName($this->upload_file['tmp_name']);
	 ...
	 * @return bool
	 */
	public function uk86_create_thumb($pic_path){
...
//				$resizeImage->newImg($save_path.DS.$this->file_name,$thumb_width[$i],$thumb_height[$i],$scale[$i],$thumb_ext[$i].'.',$save_path,$this->filling);
				$resizeImage->uk86_newImg($pic_path,$thumb_width[$i],$thumb_height[$i],$scale[$i],$thumb_ext[$i].'.',dirname($pic_path),$this->filling);
			}
		}
	}
	/**
	 ...
	 */
	private function uk86_fileInputError(){
		switch($this->upload_file['error']) {
			...
			case 1:
				...
				$this->setError(Uk86Language::uk86_get('upload_file_size_over'));
				return false;
				break;
			...
			case 3:
				//文件只有部分被上传
				$this->setError(Uk86Language::uk86_get('upload_file_is_not_complete'));
				return false;
				break;

			case 4:
				//没有文件被上传
				$this->setError(Uk86Language::uk86_get('upload_file_is_not_uploaded'));
				return false;
				break;

			case 6:
				//找不到临时文件夹
				$this->setError(Uk86Language::uk86_get('upload_dir_chmod'));
				return false;
				break;

			case 7:
				//文件写入失败
				$this->setError(Uk86Language::uk86_get('upload_file_write_fail'));
				return false;
				break;
	...
	public function setPath(){
        ...
        if(!is_writable(BASE_UPLOAD_PATH.DS.$this->default_dir)) {
			$this->setError(Uk86Language::uk86_get('upload_file_dir').$this->default_dir.Uk86Language::uk86_get('upload_file_dir_cant_touch_file'));
			return false;
		}
