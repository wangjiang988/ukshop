...
class Uk86GdImage{
	 ...
	 *
	 */
	public function uk86_setWatermark($watermark){
		$this->uk86_set('jpeg_quality',$watermark['jpeg_quality']);
		if($watermark['wm_image_name'] != ''){
			$this->uk86_set('wm_image_name',BASE_UPLOAD_PATH.DS.ATTACH_WATERMARK.DS.$watermark['wm_image_name']);
		}
		$this->uk86_set('wm_image_pos',$watermark['wm_image_pos']);
		$this->uk86_set('wm_image_transition',$watermark['wm_image_transition']);
		$this->uk86_set('wm_text',$watermark['wm_text']);
		$this->uk86_set('wm_text_size',$watermark['wm_text_size']);
		$this->uk86_set('wm_text_angle',$watermark['wm_text_angle']);
		$this->uk86_set('wm_text_pos',$watermark['wm_text_pos']);
		$this->uk86_set('wm_text_font',BASE_DATA_PATH.DS.'resource'.DS.'font'.DS.$watermark['wm_text_font'].'.ttf');
		$this->uk86_set('jpeg_quality',$watermark['jpeg_quality']);
		$this->uk86_set('wm_text_color', $watermark['wm_text_color']);
		return true;
	}
	 ...
	 * 		$img = new GDImage();
	 * 		//文字水印
	 * 		$img->wm_text = "www.ukshop.com";
	 * 		$img->wm_text_font = "./STXINWEI.TTF";
	 * 		//图片水印
	 * 		$img->wm_image_name="水印图片名";
	 * 		$img->uk86_create("生成图片文件名(包括路径)");
	 * ?>
	 * </code>
	 *
	 ...
	 */
	public function uk86_create($filename="")
	{
		...
		$src_image_type = $this->uk86_get_type($this->src_image_name);
		$src_image = $this->uk86_createImage($src_image_type,$this->src_image_name);
		if (!$src_image) {
			return false;
		}
		...
		if ($this->wm_image_name){
			...
			$wm_image_type = $this->uk86_get_type($this->wm_image_name);
			$wm_image = $this->uk86_createImage($wm_image_type,$this->wm_image_name);
			$wm_image_w=ImageSX($wm_image);
			$wm_image_h=ImageSY($wm_image);
			$temp_wm_image = $this->uk86_getPos($src_image_w,$src_image_h,$this->wm_image_pos,$wm_image);
		...
		if ($this->wm_text){
			$temp_wm_text = $this->uk86_getPos($src_image_w,$src_image_h,$this->wm_text_pos);
		...
		if ($this->save_file)
		{
			switch ($this->uk86_get_type($this->save_file)){
	 ...
	 */
	private function uk86_createImage($type, $img_name){
		if (!$type){
			$type = $this->uk86_get_type($img_name);
		}
	 ...
	 */
	private function uk86_getPos($sourcefile_width, $sourcefile_height, $pos, $wm_image=''){
		if  ($wm_image){
	 ...
	 * @return string 字符串形式的返回结果
	 */
	private function uk86_gb2utf8($gb)
	{
		if(!trim($gb))
		return $gb;
		$filename="./gb2312.txt";
		...
		while($gb)
		{
			if (ord(substr($gb,0,1))>127)
			{
				...
				$utf8.=$this->uk86_u2utf8(hexdec($codetable[hexdec(bin2hex($tthis))-0x8080]));
			}
			else
			{
				$tthis=substr($gb,0,1);
				$gb=substr($gb,1,strlen($gb)-1);
				$utf8.=$this->uk86_u2utf8($tthis);
			}
		}
	 ...
	 * @return string 字符串形式的返回结果
	 */
	private function uk86_get_type($img_name)
	{
