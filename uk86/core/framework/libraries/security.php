...
class Uk86Security{
	 ...
	 */
	public static function uk86_getToken(){
		$token = uk86_encrypt(TIMESTAMP,md5(MD5_KEY));
	 ...
	 * @return unknown
	 */
	public static function uk86_fliterHtmlSpecialChars($string) {
	 ...
	 * 
	 */
	public static function uk86_getAddslashesForInput($array,$ignore=array()){
				...
				if (is_string($v)) {
					...
					if($k != 'statistics_code') {
						if (!in_array($k,$ignore)){
							//如果不是编辑器，则转义< > & "
                            $v = self::uk86_fliterHtmlSpecialChars($v);
                        } else {
					...
					if($k == 'statistics_code') {
						...
						$array[$k] = self::uk86_getAddslashesForInput($v);
					}
				}
			}
			return $array;
	...
	public static function uk86_getAddSlashes($array){
		if (!empty($array)){
			while (list($k,$v) = each($array)) {
				if (is_string($v)) {
					if (!get_magic_quotes_gpc()) {
					...
					$array[$k] = self::uk86_getAddSlashes($v);
				}
			}
			return $array;
		}else {
