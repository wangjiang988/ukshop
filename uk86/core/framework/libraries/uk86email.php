...
final class Uk86Email{
	 ...
	 */
	public function uk86_send($email_to, $subject, $message, $from = ''){
		if(empty($email_to)) return false;
		$message = base64_encode($this->uk86_html($subject, $message));
		$email_to = $this->uk86_to($email_to);
		$header = $this->uk86_header($from);

		/**
		 * 发送
		 */
		if(!$fp = @fsockopen($this->email_server, $this->email_port, $errno, $errstr, 30)) {
			$this->uk86_resultLog($this->email_server.':'.$this->email_port." CONNECT - Unable to connect to the SMTP server");
			return false;
		}
		...
		if(substr($lastmessage, 0, 3) != '220') {
			$this->uk86_resultLog($this->email_server.':'.$this->email_port.$lastmessage);
			return false;
		}
		...
		if(substr($lastmessage, 0, 3) != 220 && substr($lastmessage, 0, 3) != 250) {
			$this->uk86_resultLog($this->email_server.':'.$this->email_port." HELO/EHLO - $lastmessage");
			return false;
		...
		if(substr($lastmessage, 0, 3) != 334) {
			$this->uk86_resultLog($this->email_server.':'.$this->email_port." AUTH LOGIN - $lastmessage");
			return false;
		}
		...
		$lastmessage = fgets($fp, 512);
		if(substr($lastmessage, 0, 3) != 334) {
			$this->uk86_resultLog($this->email_server.':'.$this->email_port." USERNAME - $lastmessage");
			return false;
		}
		...
		if(substr($lastmessage, 0, 3) != 235) {
			$this->uk86_resultLog($this->email_server.':'.$this->email_port." PASSWORD - $lastmessage");
			return false;
		}
		...
		if(substr($lastmessage, 0, 3) != 250) {
			...
			$lastmessage = fgets($fp, 512);
			if(substr($lastmessage, 0, 3) != 250) {
				$this->uk86_resultLog($this->email_server.':'.$this->email_port." MAIL FROM - $lastmessage");
				return false;
			}
		}
		...
		if(substr($lastmessage, 0, 3) != 250) {
			fputs($fp, "RCPT TO: <".preg_replace("/.*\<(.+?)\>.*/", "\\1", $email_to).">\r\n");
			$lastmessage = fgets($fp, 512);
			$this->uk86_resultLog($this->email_server.':'.$this->email_port." RCPT TO - $lastmessage");
			return false;
		}
		...
		if(substr($lastmessage, 0, 3) != 354) {
			$this->uk86_resultLog($this->email_server.':'.$this->email_port." DATA - $lastmessage");
			return false;
		}
		...
		$lastmessage = fgets($fp, 512);
		if(substr($lastmessage, 0, 3) != 250) {
			$this->uk86_resultLog($this->email_server.':'.$this->email_port." END - $lastmessage");
		}
		fputs($fp, "QUIT\r\n");
		return true;
	}

	public function uk86_send_sys_email($email_to, $subject, $message){
		$this->uk86_set('email_server',C('email_host'));
		$this->uk86_set('email_port',C('email_port'));
		$this->uk86_set('email_user',C('email_id'));
		$this->uk86_set('email_password',C('email_pass'));
		$this->uk86_set('email_from',C('email_addr'));
		$this->uk86_set('site_name',C('site_name'));
		$result = $this->uk86_send($email_to, $subject, $message);
		return $result;
	}
	 ...
	 */
	private function uk86_html($subject, $message){
	 ...
	 * @return string 字符串形式的返回结果
	 */
	private function uk86_to($email_to){
	 ...
	 * @return string 字符串形式的返回结果
	 */
	private function uk86_subject($subject){
	 ...
	 * @return string 字符串形式的返回结果
	 */
	private function uk86_message($message){
	 ...
	 */
	private function uk86_header($from=''){
		if ($from == ''){
		...
		$header .= "X-Mailer: ukshop {$this->email_delimiter}";
	 ...
	 * @return bool 布尔形式的返回结果
	 */
	private function uk86_resultLog($msg){
		if (DeBug === true){
			$fp = fopen('txt.txt','a+');
			fwrite($fp,$msg);
			fclose($fp);
