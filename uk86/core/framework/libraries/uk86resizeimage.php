...
class Uk86ResizeImage{
	 ...
	 *
	 * 使用示例
	 * uk86_newImg('图片', '宽度', '高度','缩略图宽高比','缩略图后缀名')
	 * $resizeImage	= new Uk86ResizeImage();
	 * $resizeImage->uk86_newImg($save_path.DS.$this->file_name,$thumb_width,$thumb_height,$scale,$thumb_ext[$i].'.',$save_path);
	 * 
	 ...
	 */
	public function uk86_newImg($img, $wid, $hei,$scale,$small_ext="_small.",$dst_img='',$filling = true){
...
// 		$this->config['thumb_quality'] = (C('thumb_quality') ? C('thumb_quality') : 100);
	...
	private function uk86_thumb_gd(){
		$this->uk86_initi_img();
...
//			imageCopyMerge($newimg, $snewimg,$dst_x,$dst_y,0,0,$this->width*$wh/$this->height,$wh,100);
	...
	private function uk86_thumb_im(){
		$this->uk86_dst_img();
		if($this->scale > 0 && $this->filling){
...
//		}
	 ...
	 * @return 
	 */
	private function uk86_initi_img()
	{
		if($this->type=="jpg"){
