    ...
    public function __call($method,$args) {
        if(in_array(strtolower($method),array('table','order','where','on','limit','having','group','lock','master','distinct','index','attr','key'),true)) {
			...
			if (is_numeric($args[1]) && $args[1] > 0){
			    ...
			    if ($args[0] > 0){
			        $this->options[strtolower($method)] =   $args[0];
			        uk86_pagecmd('setEachNum',	$args[0]);
			        $this->unoptions = false;
			        uk86_pagecmd('setTotalNum',	$args[1]);
			        return $this;
			    }else{
			        $args[0] = 10;
			    }
			...
			uk86_pagecmd('setEachNum',	$args[0]);
			$this->unoptions = false;
			uk86_pagecmd('setTotalNum',	$this->get_field('COUNT(*) AS nc_count'));
        	return $this;
...
//            if(isset($pkValue)) $data[$pk]   =  $pkValue;
    ...
    */
    ...
    public function showpage($style = null){
    	return uk86_pagecmd('show',$style);
    }
    ...
    public function gettotalnum(){
    	return uk86_pagecmd('gettotalnum');
    }
    ...
    public function gettotalpage(){
    	return uk86_pagecmd('gettotalpage');
    }
    /**
...
//         }
    ...
    public function buildSelectSql($options=array()) {
    	if (is_numeric($options['page'])){
			$page = uk86_pagecmd('obj');
			if ($options['limit'] !== 1){
				$options['limit'] = $page->uk86_getLimitStart().",".$page->uk86_getEachNum();
			}
    	}
