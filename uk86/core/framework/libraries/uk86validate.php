...
Class Uk86Validate{
	 ...
	 *	$a->uk86_setValidate("344d",true,"","不可以为空");
	 *	$a->uk86_setValidate("fdsfsfd",true,"Email","请填写正确的EMAIL");
	 *	echo $a->uk86_validate();
	 *
	 *  //显示结果：
	 *  请填写正确的EMAIL
	 * ? >
	...
	public function uk86_validate(){
		...
		foreach($this->validateparam as $k=>$v){
            ...
            if ($this->validateparam[$k]['result'] && $v['input'] != ""){
				switch($v['validator']){
					case "custom":
						$this->validateparam[$k]['result'] = $this->uk86_check($v['input'],$v['regexp']);
						break;
					case "compare":
						if ($v['operator'] != ""){
                    ...
                    case "range":
                        ...
                        break;
					default:
						$this->validateparam[$k]['result'] = $this->uk86_check($v['input'],$this->validator[$v['validator']]);
				}
			}
		}
		$error = $this->getError();
	 ...
	 */
	private function uk86_check($str='',$validator=''){
		if ($str != "" && $validator != ""){
	 ...
	 * @return void
	 */
	public function uk86_setValidate($validateparam){
