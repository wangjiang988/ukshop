...
class Uk86seccode {
	...
	function image() {
		...
		$this->uk86_adulterate && $this->uk86_adulterate();
		$this->uk86_giffont();
		$this->uk86_scatter && $this->uk86_scatter($this->im);
	...
	function uk86_scatter(&$obj, $level = 0) {
		$rgb = array();
		$this->uk86_scatter = $level ? $level : $this->uk86_scatter;
		$width = imagesx($obj);
		$height = imagesy($obj);
		for($j = 0;$j < $height;$j++) {
			for($i = 0;$i < $width;$i++) {
			...
			for($i = 0;$i < $width;$i++) {
				$r = rand(-$this->uk86_scatter, $this->uk86_scatter);
				imagesetpixel($obj, $i + $r , $j , $rgb[$i]);
			}
		}
	}

	function uk86_giffont() {
		$seccode = $this->code;
		$seccodedir = array();
