...
class Db{
	...
	private function __construct(){
		if (!function_exists ("mysql_connect")){
			uk86_throw_exception('Db Error: mysql is not install');
		}
	}
		...
		if (@self::$link[$host] = mysql_connect($conf['dbhost'].':'.$conf['dbport'], $conf['dbuser'], $conf['dbpwd'])){
			...
			switch (strtoupper($conf['dbcharset'])){
				...
				case 'GBK':
					...
					break;
				default:
					uk86_throw_exception("Db Error: charset is Invalid");
			}
...
// 	    if (is_object(self::$link[$host]) && !mysql_ping(self::$link[$host])) {
	...
	public static function select($param, $obj_page = '', $host = 'slave'){
		...
		if (empty($param)){
			uk86_throw_exception('Db Error: select param is empty!');
		}
    ...
    public static function commit($host = 'master'){
    	if (!self::$iftransacte){
    		...
    		if (!$result) uk86_throw_exception("Db Error: ".mysql_error(self::$link[$host]));
    	}
    }
