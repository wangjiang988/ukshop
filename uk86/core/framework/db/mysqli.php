...
class Db{
	...
	private function __construct(){
		if (!extension_loaded('mysqli')){
			uk86_throw_exception("Db Error: mysqli is not install");
		}
	}
	...
	private static function connect($host = 'slave'){
		...
		if (mysqli_connect_errno()) uk86_throw_exception("Db Error: database connect failed");
...
//         if (is_object(self::$link[$host]) && !self::$link[$host]->ping()) {
	...
	public static function query($sql, $host = 'master'){
		self::connect($host);
		if (C('debug')) uk86_addUpTime('queryStartTime');
		$query = self::$link[$host]->query($sql);
		if (C('debug')) uk86_addUpTime('queryEndTime');
		if ($query === false){
	...
	public static function select($param, $obj_page='', $host = 'slave'){
		...
		if (empty($param)) uk86_throw_exception('Db Error: select param is empty!');
		...
		if (!empty($tmp_table) && count($tmp_table) > 1){
			...
			if ((count($tmp_table)-1) != count($param['join_on'])){
				uk86_throw_exception('Db Error: join number is wrong!');
			}
		...
		if ($obj_page instanceof Uk86Page ){
			$count_query = self::query($count_sql,$host);
			$count_fetch = mysqli_fetch_array($count_query,MYSQLI_ASSOC);
			$obj_page->uk86_setTotalNum($count_fetch['count']);
			$param['limit'] = $obj_page->uk86_getLimitStart().",".$obj_page->uk86_getEachNum();
		}
		if ($param['limit'] != ''){
	...
	public static function delete($table_name, $where = '', $host = 'master'){
		...
		if (trim($where) != ''){
			...
			uk86_throw_exception('Db Error: the condition of delete is empty!');
		}
	}
    ...
    public static function commit($host = 'master'){
    	if (!self::$iftransacte){
    		...
    		if (!$result) uk86_throw_exception("Db Error: ".mysqli_error(self::$link[$host]));
    	}
    }
