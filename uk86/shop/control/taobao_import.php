...
class taobao_importControl extends BaseSellerControl {
	private function checkStore(){
        if(!uk86_checkPlatformStore()){
            ...
            if (intval($this->store_grade['sg_goods_limit']) != 0) {
                if ($goods_num >= $this->store_grade['sg_goods_limit']) {uk86_showMessage(L('store_goods_index_goods_limit') . $this->store_grade['sg_goods_limit'] . L('store_goods_index_goods_limit1'), 'index.php?act=store_goods&op=goods_list', 'html', 'error');
                }
            }
        }
    }
	...
	public function indexOp(){
		...
		if(!$_POST){
			...
			if(empty($file['name'])){uk86_showMessage($lang['store_goods_import_choose_file'],'','html','error');}
			...
			if(!is_array($gc_row) or count($gc_row) == 0){uk86_showMessage($lang['store_goods_import_wrong_class1'],'','html','error');
			}
