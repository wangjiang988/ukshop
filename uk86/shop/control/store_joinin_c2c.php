...
class store_joinin_c2cControl extends BaseHomeControl {
    ...
    public function step2Op() {
        ...
        if(!empty($_POST)) {
            ...
            //  申请类型
            $param['join_type'] = STORE_JOIN_TYPE_PERSON;
    ...
    private function step3_save_valid($param) {
        ...
        $obj_validate->validateparam = array(
            ...
            array("input"=>$param['settlement_bank_account_number'], "require"=>"true","validator"=>"Length","min"=>"1","max"=>"50","message"=>"支付宝账号不能为空且必须小于50个字"),
        );
        $error = $obj_validate->uk86_validate();
        if ($error != ''){
            uk86_showMessage($error);
    ...
    public function pay_saveOp() {
        ...
        if(empty($param['paying_money_certificate'])) {
            uk86_showMessage('请上传付款凭证','','','error');
        }
