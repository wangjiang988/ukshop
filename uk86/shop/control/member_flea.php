...
class member_fleaControl extends BaseFleaMemberControl{
	...
	public function add_goodsOp() {
		...
		if($goods_num >= 10){
			uk86_showMessage(Uk86Language::uk86_get('store_goods_index_flea_notice1'),'','','error');
		}
		/**
		 * 实例化商品分类模型
		 */
	...
	public function save_goodsOp() {
		...
		if (!empty($_POST['form_submit']) && $_POST['form_submit'] == 'ok'){
			...
			if ($error != ''){
				uk86_showMessage($lang['error'].$error,'','html','error');
			}
			/**
		 	 * 实例化店铺商品模型
			 */
			...
			if($state) {
				...
				uk86_showMessage($lang['store_goods_index_flea_add_success'],'index.php?act=member_flea');
			} else {
				uk86_showMessage($lang['store_goods_index_flea_add_fail'],'index.php?act=member_flea','html','error');
			}
		}
	}
	               	/**
	...
	public function drop_goodsOp() {
		...
		if($input_goods_count !== $verify_count) {
			showDialog(Uk86Language::uk86_get('para_error'),'','html','error');
		}
	...
	public function edit_save_goodsOp() {
		...
		if ($_POST['form_submit'] == 'ok' &&  $goods_id!= 0){
			...
			if($state) {
				...
				uk86_showMessage($lang['store_goods_index_flea_goods_edit_success'],'index.php?act=member_flea');
			} else {
				uk86_showMessage($lang['store_goods_index_flea_goods_edit_fail'],'index.php?act=member_flea','html','error');
			}
		}
	}
	/**
