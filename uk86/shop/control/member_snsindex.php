...
class member_snsindexControl extends BaseMemberControl {
	...
	public function __construct(){
		...
		Uk86Language::uk86_read('member_sns');
		//允许插入新记录的最大条数
	...
	public function addtraceOp(){
		...
		$validate_arr[] = array("input"=>$_POST["content"], "require"=>"true","message"=>Uk86Language::uk86_get('sns_sharemood_content_null'));
		$validate_arr[] = array("input"=>$_POST["content"], "validator"=>'Length',"min"=>0,"max"=>140,"message"=>Uk86Language::uk86_get('sns_content_beyond'));
		//发帖数超过最大次数出现验证码
		if(intval(uk86_cookie('weibonum'))>=self::MAX_RECORDNUM){
		...
		if ($result){
			//建立cookie
			if (uk86_cookie('weibonum') != null && intval(uk86_cookie('weibonum')) >0){
				uk86_setNcCookie('weibonum',intval(uk86_cookie('weibonum'))+1,2*3600);//保存2小时
			}else{
				uk86_setNcCookie('weibonum',1,2*3600);//保存2小时
			}
			...
			showDialog(Uk86Language::uk86_get('sns_share_succ'),'','succ',$js);
		}else {
			showDialog(Uk86Language::uk86_get('sns_share_fail'),'','error');
		}
	}
	/**
	 * 添加分享已买到的宝贝
	...
	public function sharegoodsOp(){
		...
		if ($_POST['form_submit'] == 'ok'){
			$obj_validate = new Uk86Validate();
			$validate_arr[] = array("input"=>$_POST["choosegoodsid"], "require"=>"true","message"=>Uk86Language::uk86_get('sns_sharegoods_choose'));
			$validate_arr[] = array("input"=>$_POST["comment"], "validator"=>'Length',"min"=>0,"max"=>140,"message"=>Uk86Language::uk86_get('sns_content_beyond'));
			//发帖数超过最大次数出现验证码
			if(intval(uk86_cookie('weibonum'))>=self::MAX_RECORDNUM){
				$validate_arr[] = array("input"=>$_POST["captcha"], "require"=>"true","message"=>Uk86Language::uk86_get('wrong_null'));
			}
			...
			if (intval($_POST["choosegoodsid"]) <= 0){
				$error .= Uk86Language::uk86_get('sns_sharegoods_goodserror');
			}
			if ($error != ''){
				showDialog($error,'','error');
			}
			...
			if (empty($goods_info)){
				showDialog(Uk86Language::uk86_get('sns_sharegoods_goodserror'),'','error');
			}
			...
			if (empty($sharegoods_info)){
				...
				$insert_arr['share_content'] = $_POST['comment']?$_POST['comment']:Uk86Language::uk86_get('sns_sharegoods_title');
				...
				//更新分享商品信息
				$update_arr = array();
				$update_arr['share_content'] = $_POST['comment']?$_POST['comment']:Uk86Language::uk86_get('sns_sharegoods_title');
			...
			if ($result){
				...
				$insert_arr['trace_membername'] = $_SESSION['member_name'];
				$insert_arr['trace_memberavatar'] = $member_info['member_avatar'];
				$insert_arr['trace_title'] = $_POST['comment']?$_POST['comment']:Uk86Language::uk86_get('sns_sharegoods_title');
				$content_str = '';
					...
					<div class=\"goodsinfo\">
						<dl>
							...
							<dd>".Uk86Language::uk86_get('sns_sharegoods_freight').Uk86Language::uk86_get('nc_colon').Uk86Language::uk86_get('currency').$goods_info['goods_freight']."</dd>
	                  		<dd nctype=\"collectbtn_{$goods_info['goods_id']}\"><a href=\"javascript:void(0);\" onclick=\"javascript:collect_goods(\'{$goods_info['goods_id']}\',\'succ\',\'collectbtn_{$goods_info['goods_id']}\');\">".Uk86Language::uk86_get('sns_sharegoods_collect')."</a></dd>
	                  	</dl>
	                  </div>
	             </div>";
				...
				if (C('share_isuse') == 1){
					...
					//分享内容数组
					$params = array();
					$params['title'] = Uk86Language::uk86_get('sns_sharegoods_title');
					$params['url'] = uk86_urlShop('goods' , 'index', array('goods_id'=>$goods_info['goods_id']));
					$params['comment'] = $goods_info['goods_name'].$_POST['comment'];
					$params['images'] = uk86_thumb($goods_info, 240);
					//分享之qqweibo
				...
				showDialog(Uk86Language::uk86_get('sns_share_succ'),$url,'succ',$js);
			}else {
				showDialog(Uk86Language::uk86_get('sns_share_fail'),$url,'error');
			}
		} else {
			//查询已购买商品信息
	...
	public function sharestoreOp(){
		...
		if ($_POST['form_submit'] == 'ok'){
			$obj_validate = new Uk86Validate();
			$validate_arr[] = array("input"=>$_POST["choosestoreid"], "require"=>"true","message"=>Uk86Language::uk86_get('sns_sharestore_choose'));
			$validate_arr[] = array("input"=>$_POST["comment"], "validator"=>'Length',"min"=>0,"max"=>140,"message"=>Uk86Language::uk86_get('sns_content_beyond'));
			//发帖数超过最大次数出现验证码
			if(intval(uk86_cookie('weibonum'))>=self::MAX_RECORDNUM){
				$validate_arr[] = array("input"=>$_POST["captcha"], "require"=>"true","message"=>Uk86Language::uk86_get('wrong_null'));
			...
			if (empty($store_info)){
				showDialog(Uk86Language::uk86_get('sns_store_error'),'','error');
			}
			...
			if ($result){
				...
				$insert_arr['trace_membername'] = $_SESSION['member_name'];
				$insert_arr['trace_memberavatar'] = $member_info['member_avatar'];
				$insert_arr['trace_title'] = $_POST['comment']?$_POST['comment']:Uk86Language::uk86_get('sns_sharestore_title');
				$content_str = '';
				...
				$store_info['store_url'] = uk86_urlShop('show_store', 'index', array('store_id'=>$store_info['store_id']));
				$content_str .= "<div class=\"fd-media\">
					...
					<div class=\"goodsinfo\">
						<dl>
							<dt><a target=\"_blank\" href=\"{$store_info['store_url']}\">".$store_info['store_name']."</a></dt>
	                  		<dd nctype=\"storecollectbtn_{$store_info['store_id']}\"><a href=\"javascript:void(0);\" onclick=\"javascript:collect_store(\'{$store_info['store_id']}\',\'succ\',\'storecollectbtn_{$store_info['store_id']}\');\">".Uk86Language::uk86_get('sns_sharestore_collect')."</a></dd>
	                  	</dl>
	                  </div>
	             </div>";
				$insert_arr['trace_content'] = $content_str;
				...
				if (C('share_isuse') == 1){
				    ...
				    //分享内容数组
				    $params = array();
				    $params['title'] = Uk86Language::uk86_get('sns_sharestore_title');
				    $params['url'] = uk86_urlShop('show_store' , 'index', array('store_id'=>$store_info['store_id']));
	...
	public function deltraceOp(){
		...
		if ($result){
			...
			if ($_GET['type'] == 'href'){
				showDialog(Uk86Language::uk86_get('nc_common_del_succ'),'index.php?act=member_snshome&op=trace&mid='.$_SESSION['member_id'],'succ');
			}else {
				$js = "location.reload();";
				showDialog(Uk86Language::uk86_get('nc_common_del_succ'),'','succ',$js);
			}
		} else {
			showDialog(Uk86Language::uk86_get('nc_common_del_fail'),'','error');
		}
	}
	/**
	 * SNS动态列表
	...
	public function editprivacyOp(){
		...
		if ($result){
			...
			showDialog(Uk86Language::uk86_get('sns_setting_succ'),'','succ',$js);
		}else {
			showDialog(Uk86Language::uk86_get('sns_setting_fail'),'','error');
		}
	}
	/**
	 * 删除分享和喜欢商品
	...
	public function addcommentOp(){
		...
		if ($result){
			...
			if (uk86_cookie('commentnum') != null && intval(uk86_cookie('commentnum')) >0){
				uk86_setNcCookie('commentnum',intval(uk86_cookie('commentnum'))+1,2*3600);//保存2小时
			}else{
				uk86_setNcCookie('commentnum',1,2*3600);//保存2小时
			}
	...
	public function delcommentOp(){
		...
		if (empty($comment_info)){
			showDialog(Uk86Language::uk86_get('sns_comment_recorderror'),'','error');
		}
		//删除评论
		$condition = array();
		...
		if ($result){
			...
			if ($_GET['irefresh']){
				...
				if (uk86_cookie('forwardnum') != null && intval(uk86_cookie('forwardnum')) >0){
					uk86_setNcCookie('forwardnum',intval(uk86_cookie('forwardnum'))+1,2*3600);//保存2小时
				}else{
					uk86_setNcCookie('forwardnum',1,2*3600);//保存2小时
				}
				if ($_GET['type']=='home'){
	...
	public function sharegoods_oneOp(){
		...
		if ($_GET['dialog']){
			$js = "CUR_DIALOG = ajax_form('sharegoods', '".Uk86Language::uk86_get('sns_sharegoods_tofriend')."', 'index.php?act=member_snsindex&op=sharegoods_one&gid={$gid}', 480);";
			showDialog('','','js',$js);
		}
		//查询商品信息
	...
	public function sharestore_oneOp(){
		...
		if ($_GET['dialog']){
			$js = "ajax_form('sharestore', '".Uk86Language::uk86_get('sns_sharestore')."', 'index.php?act=member_snsindex&op=sharestore_one&sid={$sid}', 480);";
			showDialog('','','js',$js);
		}
		//查询店铺信息
		$store_model = Model('store');
		...
		if (empty($store_info) || $store_info['store_state'] == 0){
			showDialog(Uk86Language::uk86_get('sns_sharestore_storeerror'),'','error');
		}
		$store_info['store_url'] = uk86_urlShop('show_store', 'index', array('store_id'=>$store_info['store_id']));
		//判断系统是否开启站外分享功能
		if (C('share_isuse') == 1){
