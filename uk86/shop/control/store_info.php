...
class store_infoControl extends BaseSellerControl {
	...
	public function reopenOp(){
	    ...
	    if (!empty($reopen_list)) {
	        ...
	        if (!uk86_checkPlatformStore() && $re_end_time - TIMESTAMP < 2592000 && $re_end_time - TIMESTAMP >= 0) {
	            //(<30天)
	            $store_info['reopen'] = true;
	        } else {
...
// 	            $model_store->editStore(array('store_end_time'=>$data['re_end_time']),array('store_id'=>$_SESSION['store_id']));
	           ...
	           showDialog(Uk86Language::uk86_get('nc_common_save_succ').'，需付款金额'.uk86_ncPriceFormat($data['re_pay_amount']).'元，请尽快完成付款，付款完成后请上传付款凭证','reload','succ','',5);
	        }
	    } else {
	       showDialog(Uk86Language::uk86_get('nc_common_del_fail'));
	    }
