...
class sns_albumControl extends BaseSNSControl {
	public function __construct() {
		 ...
		 * 读取语言包
		 */
		Uk86Language::uk86_read('sns_album');
	...
	public function album_cateOp(){
		...
		if(empty($class_info)){
			$insert = array();
			$insert['ac_name']		= Uk86Language::uk86_get('sns_buyershow');
			$insert['member_id']	= $this->master_id;
			$insert['ac_des']		= Uk86Language::uk86_get('sns_buyershow_album_des');
			$insert['ac_sort']		= 1;
			$insert['is_default']	= 1;
	...
	public function album_add_saveOp(){
		if (uk86_chksubmit()){
			...
			if($class_count >= 10){
				showDialog(Uk86Language::uk86_get('album_class_save_max_10'),'index.php?act=sns_album','error');
			}
			$insert = array();
			...
			if($return){
				showDialog(Uk86Language::uk86_get('album_class_save_succeed'),'index.php?act=sns_album','succ',empty($_GET['inajax'])?'':'CUR_DIALOG.close();');
			}
		}
	...
	public function album_edit_saveOp(){
		...
		if($re){
			showDialog(Uk86Language::uk86_get('album_class_edit_succeed'),'index.php?act=sns_album','succ',empty($_GET['inajax'])?'':'CUR_DIALOG.close();');
		}else{
	...
	public function album_delOp(){
		...
		if($return){
			showDialog(Uk86Language::uk86_get('album_class_file_del_succeed'),'index.php?act=sns_album','succ');
		}else{
			showDialog(Uk86Language::uk86_get('album_class_file_del_lose'));
		}
	}
	...
	private function profile_menu($menu_type,$menu_key=''){
		$menu_array	= array();
		$album_menuname = ($this->relation==3?Uk86Language::uk86_get('sns_me'):'TA').Uk86Language::uk86_get('nc_sns_path_my_album');
		switch ($menu_type) {
			case 'album':
				$menu_array	= array(
			...
			case 'album_pic':
				$menu_array	= array(
					1=>array('menu_key'=>'album','menu_name'=>$album_menuname,'menu_url'=>'index.php?act=sns_album&mid='.$this->master_id),
					2=>array('menu_key'=>'pic_list','menu_name'=>Uk86Language::uk86_get('nc_sns_path_album_pic_list'),'menu_url'=>'index.php?act=sns_album&op=album_pic_list&id='.intval($_GET['id']).'&mid='.$this->master_id),
				);
				break;
			case 'album_pic_info':
				$menu_array	= array(
					1=>array('menu_key'=>'album','menu_name'=>$album_menuname,'menu_url'=>'index.php?act=sns_album&mid='.$this->master_id),
					2=>array('menu_key'=>'pic_info','menu_name'=>Uk86Language::uk86_get('nc_sns_path_album_pic_info'),'menu_url'=>'index.php?act=sns_album&op=album_pic_info&id='.intval($_GET['id']).'&class_id='.intval($_GET['class_id']).'$mid='.$this->master_id),
				);
				break;
		}
	...
	public function ajax_change_imgmessageOp(){
		...
		if(strtoupper(CHARSET) == 'GBK'){
			$pic_info['ap_name'] = Uk86Language::uk86_getUTF8($pic_info['ap_name']);
		}
