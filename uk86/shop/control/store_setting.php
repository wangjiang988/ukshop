...
class store_settingControl extends BaseSellerControl {
		...
		if (uk86_chksubmit()){
			...
			if (!empty($store_domain) && $store_domain != $store_info['store_domain']){
				...
				if ($store_domain_count < $subdomain_length[0] || $store_domain_count > $subdomain_length[1]){
					showDialog(Uk86Language::uk86_get('store_setting_wrong_uri').': '.$config_subdomain_length,'reload','error');
				}
				if (!preg_match('/^[\w-]+$/i',$store_domain)){//判断域名是否正确
					showDialog(Uk86Language::uk86_get('store_setting_lack_uri'));
				}
				...
				if (!empty($store) && ($store_id != $store['store_id'])){
					showDialog(Uk86Language::uk86_get('store_setting_exists_uri'),'reload','error');
				}
				//判断二级域名是否为系统禁止
				...
				if(!empty($subdomain_reserved) && is_array($subdomain_reserved)){
						if (in_array($store_domain,$subdomain_reserved)){
							showDialog(Uk86Language::uk86_get('store_setting_invalid_uri'));
						}
				}
			...
			if (!empty($_FILES['store_banner']['name'])){
				...
				$result = $upload->uk86_upfile('store_banner');
				if ($result){
			...
			if (!empty($_FILES['store_label']['name'])){
				...
				$upload->uk86_set('file_name','');
				$upload->uk86_set('ifremove',false);
				$result = $upload->uk86_upfile('store_label');
				if ($result){
            ...
            $model_class->editStore($param, array('store_id' => $store_id));
           showDialog(Uk86Language::uk86_get('nc_common_save_succ'),'index.php?act=store_setting&op=store_setting','succ');
        }
        /**
         * 实例化店铺等级模型
         */
	...
	public function store_slideOp() {
		...
		if ($_POST['form_submit'] == 'ok'){
			...
			showDialog(Uk86Language::uk86_get('nc_common_save_succ'),'index.php?act=store_setting&op=store_slide','succ');
		}
	...
	public function silde_image_uploadOp(){
		$upload = new Uk86UploadFile();
		$upload->uk86_set('default_dir',ATTACH_SLIDE);
		...
		if(intval($_POST['file_id']) > 0){
			...
			if(!$result){
				...
				$output['error']	= Uk86Language::uk86_get('store_slide_upload_fail','UTF-8');
				echo json_encode($output);die;
			}
	...
	public function dorp_imgOp(){
		...
		echo json_encode(array('succeed'=>Uk86Language::uk86_get('nc_common_save_succ','UTF-8')));die;
	}
	...
	private function profile_menu($menu_key='') {
        ...
        $menu_array = array(
            1=>array('menu_key'=>'store_setting','menu_name'=>Uk86Language::uk86_get('nc_member_path_store_config'),'menu_url'=>'index.php?act=store_setting&op=store_setting'),
            4=>array('menu_key'=>'store_slide','menu_name'=>Uk86Language::uk86_get('nc_member_path_store_slide'),'menu_url'=>'index.php?act=store_setting&op=store_slide'),
