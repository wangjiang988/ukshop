            ...
            if (!empty($boothgoods_list)) {
                ...
                if (!empty($goods_list)) {
                    ...
                    foreach ($goods_list as $key => $val) {
                        ...
                        $goods_list[$key]['goods_image'] = uk86_thumb($val);
                    ...
                    $goodsclass_list = uk86_array_under_reset($goodsclass_list, 'gc_id');
    ...
    public function booth_quota_addOp() {
        if (uk86_chksubmit()) {
            ...
            if ($quantity <= 0 || $quantity > 12) {
               showDialog('参数错误，购买失败。', uk86_urlShop('store_promotion_booth', 'booth_quota_add'), '', 'error' );
            }
            // 实例化模型
            $model_booth = Model('p_booth');
            ...
            if ($return) {
               ...
               showDialog('购买成功', uk86_urlShop('store_promotion_booth', 'booth_goods_list'), 'succ');
            } else {
               showDialog('购买失败', uk86_urlShop('store_promotion_booth', 'booth_quota_add'));
            }
        }
        // 输出导航
...
//      }
        ...
        //获取当前价格
        $current_price = intval(C('promotion_booth_price'));
		$price_quantity = $quota_quantity * intval(C('promotion_booth_price')); // 扣款数
        //获取该用户已有套餐
        $model_booth = Model('p_booth');
		$where = array();
        $where['store_id'] = $_SESSION ['store_id'];
        ...
        if(empty($booth_quota)) {
            ...
            $data['store_name']         = $_SESSION['store_name'];
            $data['booth_quota_starttime'] = TIMESTAMP;
            $data['booth_quota_endtime']   = TIMESTAMP + $add_time;
            $data['booth_state']     = 1;
            $return = $model_booth->addBoothQuota($data);
        } else {
            $param = array();
            $param['booth_quota_endtime'] = array('exp', 'booth_quota_endtime + ' . $add_time);
            $return = $model_booth->editBoothQuota($param, array('booth_quota_id' => $booth_quota['booth_quota_id']));
        }
        ...
        if ($return) {
                ...
                $this->addcron(array('exetime' => $end_time, 'exeid' => $_SESSION['store_id'], 'type' => 4), true);
                $this->recordSellerLog('续费'.$quota_quantity.'套推荐展位，单位元');
                showDialog('购买成功', uk86_urlShop('store_promotion_booth', 'booth_list'), 'succ');
            } else {
               showDialog('购买失败', uk86_urlShop('store_promotion_booth', 'booth_quota_add'));
            }
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'booth_goods_list':
                $menu_array	= array(
                    1=>array('menu_key'=>'booth_goods_list', 'menu_name'=>'商品列表', 'menu_url'=>uk86_urlShop('store_promotion_booth', 'booth_goods_list'))
                );
                break;
            case 'booth_quota_add':
                $menu_array = array(
                    1=>array('menu_key'=>'booth_goods_list', 'menu_name'=>'商品列表', 'menu_url'=>uk86_urlShop('store_promotion_booth', 'booth_goods_list')),
                    2=>array('menu_key'=>'booth_quota_add', 'menu_name'=>'购买套餐', 'menu_url'=>uk86_urlShop('store_promotion_booth', 'booth_quota_add'))
                );
                break;
            case 'booth_renew':
                $menu_array = array(
                    1=>array('menu_key'=>'booth_goods_list', 'menu_name'=>'商品列表', 'menu_url'=>uk86_urlShop('store_promotion_booth', 'booth_goods_list')),
                    2=>array('menu_key'=>'booth_renew', 'menu_name'=>'套餐续费', 'menu_url'=>uk86_urlShop('store_promotion_booth', 'booth_renew'))
                );
                break;
        }
