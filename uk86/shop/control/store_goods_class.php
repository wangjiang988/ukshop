...
class store_goods_classControl extends BaseSellerControl {
	...
	public function goods_class_saveOp() {
		...
		if(isset($_POST['stc_id'])) {
			...
			if($state) {
				showDialog(Uk86Language::uk86_get('nc_common_save_succ'),'index.php?act=store_goods_class&op=index','succ',empty($_GET['inajax']) ?'':'CUR_DIALOG.close();');
			} else {
				showDialog(Uk86Language::uk86_get('nc_common_save_fail'));
			}
		} else {
		    $class_array		= array();
		    $class_array['stc_name']      = $_POST['stc_name'];
	...
	public function drop_goods_classOp() {
		...
		if ($drop_state){
			showDialog(Uk86Language::uk86_get('nc_common_del_succ'),'index.php?act=store_goods_class&op=store_goods_class','succ');
		}else{
