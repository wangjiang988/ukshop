...
class flea_goodsControl extends BaseFleaControl {
	...
	public function indexOp() {
		 ...
		 * 加载语言包 
		 */
		Uk86Language::uk86_read('home_layout,home_flea_goods_index');
		 ...
		 * 验证goods_id
		 */
		if(empty($_GET['goods_id']))uk86_showMessage($lang['miss_argument'],'','html','error');//"缺少参数:商品编号"
		...
		if(empty($goods_array))uk86_showMessage($lang['goods_index_no_goods'],'','html','error');//'商品不存在'
		/**
		 * 图片路径
		 */
	...
	public function save_consultOp(){
		...
		if(empty($_SESSION['member_id'])){
			uk86_showMessage(Uk86Language::uk86_get('flea_consult_notice'),'','','error');
		}
		Uk86Language::uk86_read('home_flea_goods_index');
		$lang	= Uk86Language::uk86_getLangContent();
		/**
		 * 判断商品编号的存在性和合法性
		 */
		...
		if(empty($goods_info)){
			if($_POST['type_name']==''){
				uk86_showMessage($lang['goods_index_goods_not_exists'],'','html','error');
			}
		}
		/**
		 * 咨询内容的非空验证
		...
		if(trim($_POST['content'])===""){
			uk86_showMessage($lang['goods_index_input_consult'],'','html','error');//'请输入咨询内容'
		}
		$model_member = Model('member');
		...
		if($consult->addConsult($input)){
			...
			uk86_showMessage($lang['goods_index_consult_success'],$succ_link);//'咨询发布成功'
		}else{
			uk86_showMessage($lang['goods_index_consult_fail'],'','html','error');//'咨询发布超时'
		}
	}
}
...
function checkQuality($flea_quality){
	...
	Uk86Language::uk86_read('common_flea');
	$lang	= Uk86Language::uk86_getLangContent();
	switch ($flea_quality){
		case '10':
			echo $lang['new_10'];
