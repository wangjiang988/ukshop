...
class member_evaluateControl extends BaseMemberControl{
    ...
    public function addOp(){
        ...
        if (!$order_id){
            uk86_showMessage(Uk86Language::uk86_get('wrong_argument'),'index.php?act=member_order','html','error');
        }
        ...
        if (empty($order_info) || !$order_info['evaluate_able']){
            uk86_showMessage(Uk86Language::uk86_get('member_evaluation_order_notexists'),'index.php?act=member_order','html','error');
        }

        //查询店铺信息
        $store_info = $model_store->getStoreInfoByID($order_info['store_id']);
        if(empty($store_info)){
            uk86_showMessage(Uk86Language::uk86_get('member_evaluation_store_notexists'),'index.php?act=member_order','html','error');
        }
        ...
        if (!$_POST){
            for ($i = 0, $j = count($order_goods); $i < $j; $i++) {
                $order_goods[$i]['goods_image_url'] = uk86_cthumb($order_goods[$i]['goods_image'], 60, $store_info['store_id']);
            }
...
//             //添加店铺评价
           ...
           showDialog(Uk86Language::uk86_get('member_evaluation_evaluat_success'),'index.php?act=member_order', 'succ');
        }
    }
...
//             }
    ...
    public function add_imageOp() {
        ...
        if(!empty($geval_info['geval_image'])) {
            uk86_showMessage('该商品已经发表过晒单', '', '', 'error');
        }
