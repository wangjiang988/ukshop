...
class member_complainControl extends BaseMemberControl{
    ...
    public function __construct() {
        parent::__construct();
        Uk86Language::uk86_read('member_layout,member_member_index,member_complain');
    }
    ...
    public function complain_newOp() {
		...
		if ($order_id < 1 || $goods_id < 1) {//参数验证
			uk86_showMessage(Uk86Language::uk86_get('wrong_argument'),'index.php?act=member_order&op=index','html','error');
		}
		$condition = array();
        ...
        if($if_complain < 1) {
            uk86_showMessage(Uk86Language::uk86_get('para_error'),'','html','error');
        }
        //检查是不是正在进行投诉
        if($this->check_complain_exist($goods_id)) {
            uk86_showMessage(Uk86Language::uk86_get('complain_repeat'),'','html','error');//'您已经投诉了该订单请等待处理'
        }
        ...
        if(empty($complain_subject_list)) {
            uk86_showMessage(Uk86Language::uk86_get('complain_subject_error'),'','html','error');
        }
        $model_refund = Model('refund_return');
    ...
    public function complain_saveOp() {
        ...
        if($if_complain < 1) {
           showDialog(Uk86Language::uk86_get('para_error'),'index.php?act=member_order&op=index','error');
        }
        //检查是不是正在进行投诉
        if($this->check_complain_exist($input['order_goods_id'])) {
           showDialog(Uk86Language::uk86_get('complain_repeat'),'','error');
        }
		...
		if ($state) {
			showDialog(Uk86Language::uk86_get('complain_submit_success'),'index.php?act=member_complain&op=index','succ');
		} else {
    ...
    public function complain_cancelOp() {
        ...
        if(intval($complain_info['complain_state']) === 10) {
           ...
           showDialog(Uk86Language::uk86_get('complain_cancel_success'),'reload','succ');
        } else {
        	showDialog(Uk86Language::uk86_get('complain_cancel_fail'),'','error');
        }
    }
    /*
     * 处理用户申请仲裁
    ...
    public function apply_handleOp() {
        ...
        uk86_showMessage(Uk86Language::uk86_get('handle_submit_success'),'index.php?act=member_complain');
    }
    ...
    public function get_complain_talkOp() {
        ...
        foreach($complain_talk_list as $talk) {
            ...
            $talk_list[$i]['talk'].= '('.$talk['talk_member_name'].')'.Uk86Language::uk86_get('complain_text_say').':'.$talk['talk_content'];
            $i++;
        }
    ...
    private function profile_menu($menu_key='') {
        $menu_array = array(
            array('menu_key'=>'complain_accuser_list','menu_name'=>Uk86Language::uk86_get('complain_manage_title'),'menu_url'=>'index.php?act=member_complain')
        );
