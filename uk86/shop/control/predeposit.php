...
class predepositControl extends BaseMemberControl {
	public function __construct(){
		parent::__construct();
		Uk86Language::uk86_read('member_member_predeposit');
	}
    ...
    public function recharge_showOp(){
        ...
        if ($pdr_id <= 0){
           showDialog(Uk86Language::uk86_get('predeposit_parameter_error'),'','error');
        }
        ...
        if (!$info){
           showDialog(Uk86Language::uk86_get('predeposit_record_error'),'','error');
        }
	...
	public function recharge_delOp(){
		...
		if ($result){
			showDialog(Uk86Language::uk86_get('nc_common_del_succ'),'reload','succ','CUR_DIALOG.close()');
		}else {
	...
	public function pd_cash_addOp(){
		if (uk86_chksubmit()){
			...
			$validate_arr[] = array("input"=>$pdc_amount, "require"=>"true",'validator'=>'Compare','operator'=>'>=',"to"=>'0.01',"message"=>Uk86Language::uk86_get('predeposit_cash_add_pricemin_error'));
			$validate_arr[] = array("input"=>$_POST["pdc_bank_name"], "require"=>"true","message"=>Uk86Language::uk86_get('predeposit_cash_add_shoukuanbanknull_error'));
			$validate_arr[] = array("input"=>$_POST["pdc_bank_no"], "require"=>"true","message"=>Uk86Language::uk86_get('predeposit_cash_add_shoukuanaccountnull_error'));
			$validate_arr[] = array("input"=>$_POST["pdc_bank_user"], "require"=>"true","message"=>Uk86Language::uk86_get('predeposit_cash_add_shoukuannamenull_error'));
...
//			}
			...
			if (floatval($member_info['available_predeposit']) < $pdc_amount){
				showDialog(Uk86Language::uk86_get('predeposit_cash_shortprice_error'),'index.php?act=predeposit&op=pd_cash_list','error');
			}
			try {
    			...
    			if (!$insert) {
    			    throw new Exception(Uk86Language::uk86_get('predeposit_cash_add_fail'));
    			}
    			//冻结可用预存款
    			$data = array();
    			...
    			$model_pd->commit();
    			showDialog(Uk86Language::uk86_get('predeposit_cash_add_success'),'index.php?act=predeposit&op=pd_cash_list','succ','CUR_DIALOG.close()');
			} catch (Exception $e) {
			    $model_pd->rollback();
	...
	public function pd_cash_infoOp(){
		...
		if ($pdc_id <= 0){
			uk86_showMessage(Uk86Language::uk86_get('predeposit_parameter_error'),'index.php?act=predeposit&op=pd_cash_list','html','error');
		}
		$model_pd = Model('predeposit');
		$condition = array();
		$condition['pdc_member_id'] = $_SESSION['member_id'];
		...
		if (empty($info)){
			uk86_showMessage(Uk86Language::uk86_get('predeposit_record_error'),'index.php?act=predeposit&op=pd_cash_list','html','error');
		}
	...
	private function profile_menu($menu_type,$menu_key=''){
		$menu_array	= array(
			array('menu_key'=>'loglist','menu_name'=>Uk86Language::uk86_get('nc_member_path_predeposit_loglist'),	'menu_url'=>'index.php?act=predeposit&op=pd_log_list'),
			array('menu_key'=>'recharge_list','menu_name'=>Uk86Language::uk86_get('nc_member_path_predeposit_rechargelist'),	'menu_url'=>'index.php?act=predeposit&op=index'),
			array('menu_key'=>'cashlist','menu_name'=>Uk86Language::uk86_get('nc_member_path_predeposit_cashlist'),	'menu_url'=>'index.php?act=predeposit&op=pd_cash_list'),
            array(
		...
		switch ($menu_type) {
			case 'rechargeinfo':
				$menu_array[] = array('menu_key'=>'rechargeinfo','menu_name'=>Uk86Language::uk86_get('nc_member_path_predeposit_rechargeinfo'),	'menu_url'=>'');
				break;
			case 'recharge_add':
			...
			case 'log':
				break;
			case 'cashadd':
			    $menu_array[] = array('menu_key'=>'cashadd','menu_name'=>Uk86Language::uk86_get('nc_member_path_predeposit_cashadd'),	'menu_url'=>'index.php?act=predeposit&op=pd_cash_add');
				break;
			case 'cashinfo':
				$menu_array[] = array('menu_key'=>'cashinfo','menu_name'=>Uk86Language::uk86_get('nc_member_path_predeposit_cashinfo'),	'menu_url'=>'');
				break;
		}
