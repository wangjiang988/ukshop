...
class member_favoritesControl extends BaseMemberControl{
	public function __construct(){
        parent::__construct();
        Uk86Language::uk86_read('member_layout,member_member_favorites');
    }
	/**
	 * 增加商品收藏
	 */
	public function favoritesgoodsOp(){
		...
		if ($fav_id <= 0){
			echo json_encode(array('done'=>false,'msg'=>Uk86Language::uk86_get('favorite_collect_fail','UTF-8')));
			die;
		}
		...
		if(!empty($favorites_info)){
			echo json_encode(array('done'=>false,'msg'=>Uk86Language::uk86_get('favorite_already_favorite_goods','UTF-8')));
			die;
		}
		//判断商品是否为当前会员所有
		...
		if ($goods_info['store_id'] == $_SESSION['store_id']){
			echo json_encode(array('done'=>false,'msg'=>Uk86Language::uk86_get('favorite_no_my_product','UTF-8')));
			die;
		}
		//添加收藏
		$insert_arr = array();
		...
		if ($result){
			...
			echo json_encode(array('done'=>true,'msg'=>Uk86Language::uk86_get('favorite_collect_success','UTF-8')));
			die;
		}else{
			echo json_encode(array('done'=>false,'msg'=>Uk86Language::uk86_get('favorite_collect_fail','UTF-8')));
			die;
	...
	public function favoritesstoreOp(){
		...
		if(!empty($favorites_info)){
			echo json_encode(array('done'=>false,'msg'=>Uk86Language::uk86_get('favorite_already_favorite_store','UTF-8')));
			die;
		}
		//判断店铺是否为当前会员所有
		if ($fav_id == $_SESSION['store_id']){
			echo json_encode(array('done'=>false,'msg'=>Uk86Language::uk86_get('favorite_no_my_store','UTF-8')));
			die;
		}
		//添加收藏
		$insert_arr = array();
	...
	public function delfavoritesOp(){
		if (!$_GET['fav_id'] || !$_GET['type']){
			showDialog(Uk86Language::uk86_get('member_favorite_del_fail'),'','error');
		}
			...
			if ($result){
				...
				if (!empty($fav_arr)){
					if ($type=='goods'){
						...
						showDialog(Uk86Language::uk86_get('favorite_del_success'),'index.php?act=member_favorites&op=fglist&show='.$_GET['show'],'succ');
					}else {
						$fav_str = "'".implode("','",$fav_arr)."'";
						//更新收藏数量
						$store_model = Model('store');
						$store_model->editStore(array('store_collect'=>array('exp', 'store_collect - 1')),array('store_id'=>array('in', $fav_str)));
						showDialog(Uk86Language::uk86_get('favorite_del_success'),'index.php?act=member_favorites&op=fslist','succ');
					}
				}
			}else {
				showDialog(Uk86Language::uk86_get('favorite_del_fail'),'','error');
			}
	...
	private function profile_menu($menu_type,$menu_key='') {
		$menu_array = array(
			1=>array('menu_key'=>'fav_goods','menu_name'=>Uk86Language::uk86_get('nc_member_path_collect_list'),	'menu_url'=>'index.php?act=member_favorites&op=fglist'),
			2=>array('menu_key'=>'fav_store','menu_name'=>Uk86Language::uk86_get('nc_member_path_collect_store'), 'menu_url'=>'index.php?act=member_favorites&op=fslist')
		);
