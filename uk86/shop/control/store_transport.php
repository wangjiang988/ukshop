...
class store_transportControl extends BaseSellerControl {
	...
	public function deleteOp(){
		...
		if ($transport['store_id'] != $_SESSION['store_id']){
			uk86_showMessage(Uk86Language::uk86_get('transport_op_fail'),$_SERVER['HTTP_REFERER'],'html','error');
		}
		//查看是否正在被使用
		if ($model_transport->isUsing($id)){
			uk86_showMessage(Uk86Language::uk86_get('transport_op_using'),$_SERVER['HTTP_REFERER'],'html','error');
		}
	...
	public function cloneOp(){
		...
		$transport = $model_transport->getTransportInfo(array('id'=>$id));
		unset($transport['id']);
		$transport['title'] .= Uk86Language::uk86_get('transport_clone_name');
	    ...
	    try {
            ...
            if (!$insert) throw new Exception(Uk86Language::uk86_get('transport_op_fail'));
            $model_transport->commit();
            header('location: '.$_SERVER['HTTP_REFERER']);exit;
        }catch (Exception $e){
            $model_transport->rollback();
            uk86_showMessage($e->getMessage(),$_SERVER['HTTP_REFERER'],'html','error');
        }
	}
	...
	public function saveOp(){
		if (!uk86_chksubmit()) return false;
		//读取语言包
		Uk86Language::uk86_read('transport');
		...
		if (is_array($_POST['default']['kd'])){
			...
			$trans_list[0]['area_name'] = Uk86Language::uk86_get('transport_country');
    ...
    public function offpay_areaOp()
    {
        /*if (!checkPlatformStore()) {
            uk86_showMessage('本功能仅限平台自营店使用', '', 'html', 'error');
        }*/
	...
	private function profile_menu($menu_type,$menu_key='') {
		...
		switch ($menu_type) {
            ...
            case 'offpay_area':
				$menu_array = array(
				1=>array('menu_key'=>'transport',	'menu_name'=>Uk86Language::uk86_get('nc_member_path_postage'),		'menu_url'=>'index.php?act=store_transport'),
				);
                //if (checkPlatformStore()) {
                    $menu_array[] = array(
