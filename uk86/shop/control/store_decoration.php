...
class store_decorationControl extends BaseSellerControl {
    ...
    private function _validate_background_setting_input($decoration_id, $setting) {
        ...
        if(!empty($setting['background_image'])) {
            $setting['background_image_url'] = uk86_getStoreDecorationImageUrl($setting['background_image'], $_SESSION['store_id']);
    ...
    public function decoration_banner_saveOp() {
        ...
        if($result) {
            $data['message'] = '保存成功';
            $data['image_url'] = uk86_getStoreDecorationImageUrl($banner['image'], $_SESSION['store_id']);
        } else {
            $data['error'] = '保存失败';
        }
        echo json_encode($data);die;
    ...
    private function profile_menu($menu_key='') {
        $menu_array = array(
            1=>array('menu_key'=>'decoration_setting','menu_name'=>'店铺装修','menu_url'=>uk86_urlShop('store_decoration', 'decoration_setting')),
            2=>array('menu_key'=>'decoration_album','menu_name'=>'装修图库','menu_url'=>uk86_urlShop('store_decoration', 'decoration_album')),
        );
