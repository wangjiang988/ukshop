...
class member_sharemanageControl extends BaseMemberControl {
	...
	public function unbindOp(){
		$type = $_GET['type'];
		if (empty($type)){
			showDialog(Uk86Language::uk86_get('wrong_argument'),'index.php?act=member_sharemanage','error');
		}
		$model = Model('sns_binding');
		...
		if ($result){
			$js = "location.reload();";
			showDialog(Uk86Language::uk86_get('sharebind_unbind_succ'),'','succ',$js);
		}else {
			showDialog(Uk86Language::uk86_get('sharebind_unbind_fail'),'','succ');
		}
	}
	/**
	 * 验证某接口是否已经绑定
	...
	public function checkbindOp(){
        ...
        $result = '';
        if (empty($app_key)){
            $result = json_encode(array('done'=>false,'msg'=>Uk86Language::uk86_get('sharebind_bind_fail')));
        } else {
            $model = Model('sns_binding');
	...
	public function share_qqzoneOp(){
		...
		if (C('share_qqzone_isuse') != 1){
			uk86_showMessage(Uk86Language::uk86_get('sharebind_unused'),'index.php?act=member_sharemanage','html','error');
		}
		include_once BASE_DATA_PATH.'/api/snsapi/qqzone/oauth/qq_callback.php';
		if (!$_SESSION['qqzone']){
			echo "<script>alert('".Uk86Language::uk86_get('sharebind_bind_fail')."');</script>";
	...
	private function profile_menu($menu_key='',$array=array()) {
		$menu_array		= array();
		$menu_array = array(
			1=>array('menu_key'=>'sns_binding',	'menu_name'=>'分享绑定',	'menu_url'=> uk86_urlShop('member_sharemanage', 'index')),
		);
		if(!empty($array)) {
			$menu_array[] = $array;
		}
