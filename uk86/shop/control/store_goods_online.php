...
class store_goods_onlineControl extends BaseSellerControl {
    public function __construct() {
        parent::__construct ();
        Uk86Language::uk86_read ('member_store_goods_index');
    }
    ...
    public function edit_goodsOp() {
        ...
        if ($common_id <= 0) {
            uk86_showMessage(L('wrong_argument'), '', 'html', 'error');
        }
        $model_goods = Model('goods');
    ...
    public function edit_save_goodsOp() {
        ...
        $update_common['store_floor_id']     = intval($_POST['store_floor_id']);    //楼层id
        $update_common['store_floor_name']   = $_POST['store_floor_name'];         //楼层名称
        // 序列化保存手机端商品描述数据
        if ($_POST['m_body'] != '') {
        ...
        if (is_array ( $_POST ['spec'] )) {
            foreach ($_POST['spec'] as $value) {
                ...
                if (!empty($goods_info)) {
                    ...
                    $update['store_floor_id']    = $update_common['store_floor_id'];   //楼层id
                    $update['store_floor_name']  = $update_common['store_floor_name']; //楼层名称
                    // 虚拟商品不能有赠品
    ...
    public function edit_imageOp() {
        ...
        if ($common_id <= 0) {
            uk86_showMessage(L('wrong_argument'), uk86_urlShop('seller_center'), 'html', 'error');
        }
        $model_goods = Model('goods');
        ...
        $image_list = uk86_array_under_reset($image_list, 'color_id', 2);
    ...
    public function edit_save_imageOp() {
        if (uk86_chksubmit()) {
            ...
            if ($common_id <= 0 || empty($_POST['img'])) {
               showDialog(L('wrong_argument'), uk86_urlShop('store_goods_online', 'index'));
            }
            $model_goods = Model('goods');
            // 删除原有图片信息
            ...
            $this->recordSellerLog('编辑商品，平台货号：'.$common_id);
               ...
               showDialog(L('nc_common_save_fail'), uk86_urlShop('store_goods_online', 'index'));
            }
        }
    }
    ...
    public function goods_unshowOp() {
        ...
        if ($return) {
           ...
           showDialog(L('store_goods_index_goods_unshow_success'), uk86_getReferer() ? uk86_getReferer() : 'index.php?act=store_goods_online&op=goods_list', 'succ', '', 2);
        } else {
    ...
    private function profile_menu($menu_type,$menu_key, $allow_promotion = array()) {
        ...
        switch ($menu_type) {
            case 'goods_list':
                $menu_array = array(
                   array('menu_key' => 'goods_list',    'menu_name' => '出售中的商品', 'menu_url' => uk86_urlShop('store_goods_online', 'index'))
                );
                break;
            case 'edit_detail':
                if ($allow_promotion['lock'] === false) {
                    $menu_array = array(
                        array('menu_key' => 'edit_detail',  'menu_name' => '编辑商品', 'menu_url' => uk86_urlShop('store_goods_online', 'edit_goods', array('commonid' => $_GET['commonid'], 'ref_url' => $_GET['ref_url']))),
                        array('menu_key' => 'edit_image',   'menu_name' => '编辑图片', 'menu_url' => uk86_urlShop('store_goods_online', 'edit_image', array('commonid' => $_GET['commonid'], 'ref_url' => ($_GET['ref_url'] ? $_GET['ref_url'] : uk86_getReferer())))),
                    );
                }
            ...
            case 'edit_class':
                $menu_array = array(
                    array('menu_key' => 'edit_class',   'menu_name' => '选择分类', 'menu_url' => uk86_urlShop('store_goods_online', 'edit_class', array('commonid' => $_GET['commonid'], 'ref_url' => $_GET['ref_url']))),
                    array('menu_key' => 'edit_detail',  'menu_name' => '编辑商品', 'menu_url' => uk86_urlShop('store_goods_online', 'edit_goods', array('commonid' => $_GET['commonid'], 'ref_url' => $_GET['ref_url']))),
                    array('menu_key' => 'edit_image',   'menu_name' => '编辑图片', 'menu_url' => uk86_urlShop('store_goods_online', 'edit_image', array('commonid' => $_GET['commonid'], 'ref_url' => ($_GET['ref_url'] ? $_GET['ref_url'] : uk86_getReferer())))),
                );
                break;
