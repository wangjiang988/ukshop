...
class store_activityControl extends BaseSellerControl {
	...
	public function activity_applyOp(){
		...
		if($activity_id <= 0){
			uk86_showMessage(Uk86Language::uk86_get('para_error'),'index.php?act=store_activity&op=store_activity','html','error');
		}
		...
		if(empty($activity_info) || $activity_info['activity_type'] != '1' || $activity_info['activity_state'] != 1 || $activity_info['activity_start_date']>time() || $activity_info['activity_end_date']<time()){
			uk86_showMessage(Uk86Language::uk86_get('store_activity_not_exists'),'index.php?act=store_activity&op=store_activity','html','error');
		}
	...
	public function activity_apply_saveOp(){
		...
		if(empty($_POST['item_id'])){
			showDialog(Uk86Language::uk86_get('store_activity_choose_goods'),'index.php?act=store_activity&op=store_activity');
		}
		$activity_id = intval($_POST['activity_id']);
		if($activity_id <= 0){
			showDialog(Uk86Language::uk86_get('para_error'),'index.php?act=store_activity&op=store_activity');
		}
		...
		if(empty($activity) || $activity['activity_type'] != '1' || $activity['activity_state'] != '1' || $activity['activity_start_date']>time() || $activity['activity_end_date']<time()){
			showDialog(Uk86Language::uk86_get('store_activity_not_exists'),'index.php?act=store_activity&op=store_activity');
		}
		...
		showDialog(Uk86Language::uk86_get('store_activity_submitted'),'reload','succ');
	}
	...
	private function profile_menu($menu_key='') {
        ...
        $menu_array = array(
				1=>array('menu_key'=>'store_activity','menu_name'=>Uk86Language::uk86_get('nc_member_path_activity_list'),'menu_url'=>'index.php?act=store_activity&op=store_activity'),
				2=>array('menu_key'=>'activity_apply','menu_name'=>Uk86Language::uk86_get('nc_member_path_join_activity'),'menu_url'=>'')
        );
