...
class store_consultControl extends BaseSellerControl {
	public function __construct() {
		parent::__construct();
		Uk86Language::uk86_read('member_store_consult_index');
	}
    ...
    public function drop_consultOp(){
        ...
        if($state) {
           showDialog(Uk86Language::uk86_get('store_consult_drop_success'),'reload','succ');
        } else {
           showDialog(Uk86Language::uk86_get('store_consult_drop_fail'));
        }
    }
    ...
    public function reply_saveOp(){
        ...
        if($state){
            ...
            $param['param'] = array(
                ...
                'consult_url' => uk86_urlShop('member_consult', 'my_consult')
            );
	...
	private function profile_menu($menu_type,$menu_key='',$array=array()) {
		...
		switch ($menu_type) {
			case 'consult':
				$menu_array	= array(
				1=>array('menu_key'=>'consult_list',	'menu_name'=>Uk86Language::uk86_get('nc_member_path_all_consult'),			'menu_url'=>'index.php?act=store_consult&op=consult_list'),
				2=>array('menu_key'=>'to_reply',	'menu_name'=>Uk86Language::uk86_get('nc_member_path_unreplied_consult'),			'menu_url'=>'index.php?act=store_consult&op=consult_list&type=to_reply'),
				3=>array('menu_key'=>'replied',	'menu_name'=>Uk86Language::uk86_get('nc_member_path_replied_consult'),			'menu_url'=>'index.php?act=store_consult&op=consult_list&type=replied'));
				break;
		}
		if(!empty($array)) {
			$menu_array[] = $array;
