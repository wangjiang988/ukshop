...
class connectControl extends BaseHomeControl
{
    ...
    {
        parent::__construct();
        Uk86Language::uk86_read("home_login_register,home_login_index,home_qqconnect");
        /**
         * 判断qq互联功能是否开启
         */
        if (C('qq_isuse') != 1) {
            uk86_showMessage(Uk86Language::uk86_get('home_qqconnect_unavailable'), 'index.php', 'html', 'error');//'系统未开启QQ互联功能'
        }
        /**
         * 初始化测试数据
         */
        if (!$_SESSION['openid']) {
            uk86_showMessage(Uk86Language::uk86_get('home_qqconnect_error'), 'index.php', 'html', 'error');//'系统错误'
        }
    ...
    public function registerOp()
    {
        ...
        if (uk86_chksubmit()) {
            ...
            uk86_showMessage(Uk86Language::uk86_get('nc_common_save_succ'), SHOP_SITE_URL);
        } else {
            //检查登录状态
            ...
            $user_array['member_email_bind'] = 0;
            ...
            if ($result) {
                ...
                uk86_showMessage(Uk86Language::uk86_get('login_usersave_regist_fail'), SHOP_SITE_URL . '/index.php?act=login&op=register', 'html', 'error');//"会员注册失败"
            }
        }
    }
    ...
    public function bindqqOp()
    {
        if (is_mobile()) {
            ...
            $url = 'index.php?act=member_connect&op=qqbind';
        }
        $model_member = Model('member');
        //验证QQ账号用户是否已经存在
        $array = array();
        ...
        if (is_array($member_info) && count($member_info) > 0) {
            ...
            uk86_showMessage(Uk86Language::uk86_get('home_qqconnect_binding_exist'), $url, 'html', 'error');//'该QQ账号已经绑定其他商城账号,请使用其他QQ账号与本账号绑定'
        }
        //获取qq账号信息
        require_once(BASE_PATH . '/api/qq/user/get_user_info.php');
        $qquser_info = get_user_info($_SESSION["appid"], $_SESSION["appkey"], $_SESSION["token"], $_SESSION["secret"], $_SESSION["openid"]);
        ...
        if ($edit_state) {
            uk86_showMessage(Uk86Language::uk86_get('home_qqconnect_binding_success'), $url);
        } else {
            uk86_showMessage(Uk86Language::uk86_get('home_qqconnect_binding_fail'), $url, 'html', 'error');//'绑定QQ失败'
        }
    }
