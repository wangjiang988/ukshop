...
class store_order_printControl extends BaseSellerControl {
	public function __construct() {
		parent::__construct();
		Uk86Language::uk86_read('member_printorder');
	}
	...
	public function indexOp() {
		...
		if (empty($order_info)){
			uk86_showMessage(Uk86Language::uk86_get('member_printorder_ordererror'),'','html','error');
		}
		...
		if (!empty($order_info['extend_order_goods'])){
			...
			foreach ($order_info['extend_order_goods'] as $k => $v){
				$v['goods_name'] = uk86_str_cut($v['goods_name'],100);
				$goods_all_num += $v['goods_num'];
				$v['goods_all_price'] = uk86_ncPriceFormat($v['goods_num'] * $v['goods_price']);
		...
		Tpl::output('goods_total_price',uk86_ncPriceFormat($goods_total_price));
