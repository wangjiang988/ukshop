...
class member_returnControl extends BaseMemberControl {
	...
	public function shipOp(){
		...
		if (uk86_chksubmit()) {
			...
			if ($state) {
				showDialog(Uk86Language::uk86_get('nc_common_save_succ'),'index.php?act=member_return&op=index','succ');
			} else {
	...
	public function delayOp(){
		...
		if (uk86_chksubmit()) {
			if ($return['seller_state'] != '2' || $return['goods_state'] != '3') {//检查状态,防止页面刷新不及时造成数据错误
				showDialog(Uk86Language::uk86_get('wrong_argument'),'reload','error','CUR_DIALOG.close();');
			}
			$refund_array = array();
			$refund_array['delay_time'] = time();
			$refund_array['goods_state'] = '2';
			$state = $model_refund->editRefundReturn($condition, $refund_array);
			if ($state) {
				showDialog(Uk86Language::uk86_get('nc_common_save_succ'),'reload','succ','CUR_DIALOG.close();');
			} else {
				showDialog(Uk86Language::uk86_get('nc_common_save_fail'),'reload','error','CUR_DIALOG.close();');
			}
		}
		$model_trade = Model('trade');
		$return_delay = $model_trade->getMaxDay('return_delay');//发货默认5天后才能选择没收到
	...
	private function profile_menu($menu_type,$menu_key='') {
		...
		switch ($menu_type) {
			case 'member_order':
				$menu_array = array(
				array('menu_key'=>'buyer_refund','menu_name'=>Uk86Language::uk86_get('nc_member_path_buyer_refund'),	'menu_url'=>'index.php?act=member_refund'),
				array('menu_key'=>'buyer_return','menu_name'=>Uk86Language::uk86_get('nc_member_path_buyer_return'),	'menu_url'=>'index.php?act=member_return'),
