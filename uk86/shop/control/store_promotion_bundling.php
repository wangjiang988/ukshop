...
class store_promotion_bundlingControl extends BaseSellerControl {
    ...
    public function __construct() {
         ...
         * 读取语言包
         */
        Uk86Language::uk86_read('member_layout,member_store_promotion_bundling');
        //检查是否开启
    ...
    public function bundling_listOp() {
        ...
        if ($hasList) {
            ...
            if (!empty($bundling_list)) {
                ...
                $bgoods_array = uk86_array_under_reset($bgoods_array, 'goods_id');
                if (!empty($bgoods_array)) {
                    ...
                    $goods_array = uk86_array_under_reset($goods_array, 'goods_id');
                }
                ...
                foreach ($bundling_list as $key => $val) {
                    ...
                    $bundling_list[$key]['img'] = uk86_thumb($goods_array[$bgoods_array[$val['bl_id']]['goods_id']], 60);
                }
            }
            Tpl::output('list', $bundling_list);

            // 状态数组
            $state_array = array(0=>Uk86Language::uk86_get('bundling_status_0') , 1=>Uk86Language::uk86_get('bundling_status_1'));
...
//         showDialog(Uk86Language::uk86_get('mansong_quota_quantity_error'));
//      }
        ...
        //获取当前价格
        $current_price = intval(C('promotion_bundling_price'));
		$price_quantity = $quota_quantity * intval(C('promotion_bundling_price')); // 扣款数
        //获取该用户已有套餐
        $model_bundling = Model('p_bundling');
        $current_quota= $model_bundling->getBundlingQuotaInfoCurrent($_SESSION['store_id']);
        ...
        if(empty($current_quota)) {
            ...
            $data['bl_quota_month']     = $quota_quantity;
            $data['bl_quota_starttime'] = TIMESTAMP;
            $data['bl_quota_endtime']   = TIMESTAMP + $add_time;
            $data['bl_state']     = 1;
            $return = $model_bundling->addBundlingQuota($data);
        } else {
            $param = array();
            $param['bl_quota_endtime'] = array('exp', 'bl_quota_endtime + ' . $add_time);
            $return = $model_bundling->editBundlingQuota($param, array('bl_quota_id' => $current_quota['bl_quota_id']));
        }
        ...
        if ($return) {
                ...
                // 添加任务队列
                $end_time = TIMESTAMP + 60 * 60 * 24 * 30 * $quota_quantity;
                $this->addcron(array('exetime' => $end_time, 'exeid' => $_SESSION['store_id'], 'type' => 3), true);

                $this->recordSellerLog('购买'.$quota_quantity.'套优惠套装，单位元');
               showDialog(L('bundling_quota_price_succ'), uk86_urlShop('store_promotion_bundling', 'bundling_list'), 'succ');
            } else {
               showDialog(L('bundling_quota_price_fail'), uk86_urlShop('store_promotion_bundling', 'bundling_quota_add'));
            }
	}
    ...
    public function bundling_quota_addOp() {
        if (uk86_chksubmit()) {
            ...
            if ($quantity <= 0 || $quantity > 12) {
               showDialog(Uk86Language::uk86_get('bundling_quota_price_fail'), uk86_urlShop('store_promotion_bundling', 'bundling_quota_add'), '', 'error' );
            }
            // 实例化模型
            $model_bundling = Model('p_bundling');
    ...
    public function bundling_addOp() {
        ...
        // 是否能使用编辑器
        if(uk86_checkPlatformStore()){ // 平台店铺可以使用编辑器
    ...
    public function bundling_add_goodsOp() {
        ...
        $goods_list = $model_goods->getGoodsListForPromotion($where, '*, count(distinct goods_commonid)', 8, 'bundling', 'goods_commonid');
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            ...
            case 'bundling_quota_list':
                $menu_array	= array(
                    1=>array('menu_key'=>'bundling_list', 'menu_name'=>Uk86Language::uk86_get('bundling_list'), 'menu_url'=>uk86_urlShop('store_promotion_bundling', 'bundling_list'))
                );
                break;
            case 'bundling_quota_add':
                $menu_array	= array(
                    1=>array('menu_key'=>'bundling_list', 'menu_name'=>Uk86Language::uk86_get('bundling_list'), 'menu_url'=>uk86_urlShop('store_promotion_bundling', 'bundling_list')),
                    2=>array('menu_key'=>'bundling_quota_add', 'menu_name'=>Uk86Language::uk86_get('bundling_quota_add'), 'menu_url'=>uk86_urlShop('store_promotion_bundling', 'bundling_quota_add'))
                );
                break;
            case 'bundling_renew':
                $menu_array	= array(
                    1=>array('menu_key'=>'bundling_list', 'menu_name'=>Uk86Language::uk86_get('bundling_list'), 'menu_url'=>uk86_urlShop('store_promotion_bundling', 'bundling_list')),
                    2=>array('menu_key'=>'bundling_renew', 'menu_name'=>'套餐续费', 'menu_url'=>uk86_urlShop('store_promotion_bundling', 'bundling_renew'))
                );
                break;
            case 'bundling_add':
                $menu_array	= array(
                    1=>array('menu_key'=>'bundling_list', 'menu_name'=>Uk86Language::uk86_get('bundling_list'), 'menu_url'=>uk86_urlShop('store_promotion_bundling', 'bundling_list')),
                    2=>array('menu_key'=>'bundling_add', 'menu_name'=>Uk86Language::uk86_get('bundling_add'), 'menu_url'=>uk86_urlShop('store_promotion_bundling', 'bundling_add'))
                );
                break;
            case 'bundling_edit':
                $menu_array	= array(
                    1=>array('menu_key'=>'bundling_list', 'menu_name'=>Uk86Language::uk86_get('bundling_list'), 'menu_url'=>uk86_urlShop('store_promotion_bundling', 'bundling_list')),
                    2=>array('menu_key'=>'bundling_edit', 'menu_name'=>Uk86Language::uk86_get('bundling_edit'), 'menu_url'=>uk86_urlShop('store_promotion_bundling', 'bundling_edit'))
                );
            break;
        }
