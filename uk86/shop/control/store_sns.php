...
class store_snsControl extends BaseSellerControl{
    public function __construct() {
        parent::__construct ();
        Uk86Language::uk86_read('store_sns,member_sns');
    }
	...
	public function store_sns_saveOp(){
		...
		$obj_validate->validateparam = array(
				array("input"=>$_POST["content"],"require"=>"true","validator"=>"Length","max"=>140,"min"=>1,"message"=>Uk86Language::uk86_get('store_sns_center_error')),
				array("input"=>$_POST["goods_url"],"require"=>"false","validator"=>"url","message"=>Uk86Language::uk86_get('store_goods_index_goods_price_null')),
		);
		...
		switch ($_POST['type']){
			...
			case '10':
				if(is_array($_POST['goods_id'])){
					...
					showDialog(Uk86Language::uk86_get('store_sns_choose_goods'));
				}
					...
					foreach ($goods_array as $val){
						if( CHARSET == 'GBK') {
							foreach ((array)$val as $k=>$v){
								$val[$k] = Uk86Language::uk86_getUTF8($v);
							}
						}
		...
		if($rs){
			showDialog(Uk86Language::uk86_get('nc_common_op_succ'), 'index.php?act=store_sns', 'succ');
		}else{
	...
	private function profile_menu($menu_key) {
		$menu_array	= array(
				1=>array('menu_key'=>'store_sns_add', 'menu_name'=>Uk86Language::uk86_get('store_sns_add'), 'menu_url'=>'index.php?act=store_sns&op=add'),
				2=>array('menu_key'=>'store_sns_setting', 'menu_name'=>Uk86Language::uk86_get('store_sns_setting'), 'menu_url'=>'index.php?act=store_sns&op=setting'),
				3=>array('menu_key'=>'store_sns_brower', 'menu_name'=>Uk86Language::uk86_get('store_sns_browse'), 'menu_url'=>uk86_urlShop('store_snshome', 'index', array('sid' => $_SESSION['store_id'])), 'target'=>'_blank')
		);
	...
	private function getGoodsByUrl($url){
		...
		if(intval($id) > 0){
			...
			if(!empty($result) && is_array($result)){
				...
				showDialog(Uk86Language::uk86_get('store_sns_goods_url_error'));
			}
		}else{
			showDialog(Uk86Language::uk86_get('store_sns_goods_url_error'));
		}
