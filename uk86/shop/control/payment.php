...
class paymentControl extends BaseHomeControl{
	...
	public function real_orderOp(){
		...
		if ($result['data']['api_pay_state'] || empty($result['data']['api_pay_amount'])) {
			uk86_showMessage('该订单不需要支付', $url, 'html', 'error');
		}
	 ...
	 * @author bzhang
	 */
	public function promotion_orderOp(){
		...
		//计算所需支付金额等支付单信息
		$result = $logic_payment->getPromotionVrOrderInfo($order_sn, $_SESSION['member_id']);
		if(!$result['state']) {
			uk86_showMessage($result['msg'], $url, 'html', 'error');
		}
	 ...
	 * 促销商品购买
	 */
	public function pm_orderOp(){
		$order_sn = $_POST['order_sn'];
		$payment_code = $_POST['payment_code'];
		$url = 'index.php?act=member_pm_order';
...
//		$result = $logic_payment->getVrOrderInfo($order_sn, $_SESSION['member_id']);
	...
	public function pd_orderOp(){
        ...
        if ($result['data']['pdr_payment_state'] || empty($result['data']['api_pay_amount'])) {
            uk86_showMessage('该充值单不需要支付', $url, 'html', 'error');
        }
	...
	public function notifyOp(){
        ...
        $out_trade_no = $_POST['out_trade_no'];    //唯一订单号
        $trade_no = $_POST['trade_no'];			  //支付宝交易号
		//参数判断
		if(!preg_match('/^\d{18}$/',$out_trade_no)) exit($fail);  //判断订单号是否合法
