...
class searchControl extends BaseHomeControl {
    ...
    public function indexOp() {
        Uk86Language::uk86_read('home_goods_class_index');
        ...
        if (intval($_GET['cate_id']) > 0) {
            ...
            $channel=$model_channel->getChannelByCateId($default_classid);

            if(!empty($channel)&&is_array($channel)){    //如果该分类已经绑定了频道，则跳转到该频道
                header("location:".uk86_urlShop('channel','index',array('id'=>$channel['channel_id'])));
            }
        } elseif ($_GET['keyword'] != '') {
            //从TAG中查找分类
        ...
        if (is_array($indexer_ids)) {
            ...
            uk86_pagecmd('setEachNum',self::PAGESIZE);
            uk86_pagecmd('setTotalNum',$indexer_count);
        ...
        if (!empty($goods_list)) {
            ...
            foreach ($goods_list as $key => $value) {
                ...
                foreach ($goods_list as $k => $v){
                	if($v['goods_commonid'] == $value['goods_commonid'] && $v['goods_id'] != $value['goods_id']){
...
//		d(Tpl::getOutPut());
     ...
     * 本地化页面
     */
    public function local_searchOp(){
        Uk86Language::uk86_read('home_goods_class_index');
        $this->_model_search = Model('search');
        //显示左侧分类
        //默认分类，从而显示相应的属性和品牌
...
//        Tpl::output('checked_attr', $checked_attr);
        ...
        if (is_array($indexer_ids)) {
            ...
            if (intval($_GET['area_id']) > 0) {
                $condition['areaid_2'] = intval($_GET['area_id']);    //根据城市筛选
            }
            if ($_GET['type'] == 1) {
                $condition['is_own_shop'] = 1;
            }
           ...
           // $condition['is_virtual'] = 1;  //虚拟商品
            $goods_list = $model_goods->getGoodsListCommon($condition, $fields, $order, self::PAGESIZE);
        }

        Tpl::output('show_page1', $model_goods->showpage(4));  //显示分页并显示风格
        Tpl::output('show_page', $model_goods->showpage(5));  //显示分页并显示风格
...
/*        if (!empty($goods_list)) {
        ...
        Tpl::output('goods_list', $goods_list);      //向前台传递商品列表
        if ($_GET['keyword'] != ''){
            Tpl::output('show_keyword',  $_GET['keyword']);
        } else {
            Tpl::output('show_keyword',  $goods_param['class']['gc_name']);
        ...
        //$nav_id = intval($_GET['nav_id']) ? intval($_GET['nav_id']) : 0;
        //Tpl::output('index_sign', $nav_id);
          Tpl::output('index_sign',"search_local");
        // 地区
        $city_array = Model('area')->getServiceGoodsCity();
        Tpl::output('city_array', $city_array);
        //var_dump($city_array);
        uk86_loadfunc('search');
