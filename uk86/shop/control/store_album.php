...
class store_albumControl extends BaseSellerControl {
	...
	public function album_cateOp(){
		...
		if(!$return){
			$album_arr = array();
			$album_arr['aclass_name'] = Uk86Language::uk86_get('album_default_album');
...
//		Tpl::output('show_page',$page->show());
	...
	public function album_add_saveOp(){
		if (uk86_chksubmit()){
			...
			if($class_count['count'] >= 20){
				showDialog(Uk86Language::uk86_get('album_class_save_max_20'),'index.php?act=store_album','error',empty($_GET['inajax'])?'':'CUR_DIALOG.close();');
			}
			/**
			 * 实例化相册模型
			 */
			...
			if($return){
				showDialog(Uk86Language::uk86_get('album_class_save_succeed'),'index.php?act=store_album','succ',empty($_GET['inajax'])?'':'CUR_DIALOG.close();');
			}
		}
		showDialog(Uk86Language::uk86_get('album_class_save_lose'));
	}
	/**
	 * 相册分类编辑
	 */
	public function album_editOp(){
		if(empty($_GET['id'])){
			echo Uk86Language::uk86_get('album_parameter_error');exit;
		}
		/**
		 * 实例化相册模型
		 */
	...
	public function album_edit_saveOp(){
		...
		if($return){
			...
			if($re){
				showDialog(Uk86Language::uk86_get('album_class_edit_succeed'),'index.php?act=store_album','succ',empty($_GET['inajax'])?'':'CUR_DIALOG.close();');
			}
		}else{
			showDialog(Uk86Language::uk86_get('album_class_edit_lose'));
		}
	}
	/**
	 * 相册删除
	...
	public function album_delOp(){
		...
		if(!$return){
			showDialog(Uk86Language::uk86_get('album_class_file_del_lose'));
		}
		/**
		 * 删除分类
		 */
		...
		if($return){
			showDialog(Uk86Language::uk86_get('album_class_file_del_succeed'),'index.php?act=store_album','succ');
		}else{
			showDialog(Uk86Language::uk86_get('album_class_file_del_lose'));
		}
	}
	...
	public function change_album_coverOp(){
		...
		if($return){
			...
			if($re){
				showDialog(Uk86Language::uk86_get('nc_common_op_succ'),'reload','succ');
			}
		}else{
			showDialog(Uk86Language::uk86_get('nc_common_op_fail'));
		}
	...
	public function change_pic_nameOp(){
		...
		if(strtoupper(CHARSET) == 'GBK'){
			$_POST['name'] = Uk86Language::uk86_getGBK($_POST['name']);
		}
	...
	public function album_pic_infoOp(){
		...
		$total_page = (ceil($page->uk86_get('total_num')/$each_num));
		...
		$curpage = intval($_GET['curpage']);
		if (empty($curpage)) $curpage = 1;
		$tatal_page = (ceil($page->uk86_get('total_num')/$each_num));
	...
	public function album_pic_delOp(){
		...
		if(!$return){
			showDialog(Uk86Language::uk86_get('album_class_pic_del_lose'));
		}

		//删除图片
		$return = $model_album->delPic($id, $_SESSION['store_id']);
		if($return){
			showDialog(Uk86Language::uk86_get('album_class_pic_del_succeed'),'reload','succ');
		}else{
			showDialog(Uk86Language::uk86_get('album_class_pic_del_lose'));
		}
	}
	...
	public function album_pic_moveOp(){
		...
		if(uk86_chksubmit()){
			...
			if($return){
				showDialog(Uk86Language::uk86_get('album_class_pic_move_succeed'),'reload','succ');
			}else{
				showDialog(Uk86Language::uk86_get('album_class_pic_move_lose'));
			}
		}
		$param = array();
    ...
    public function replace_image_uploadOp() {
        ...
        $upload->uk86_set('default_dir', ATTACH_GOODS . DS . $_SESSION['store_id'] . DS . $pic_cover);
        ...
        $upload->uk86_set('file_name', $pic_name);
        $return = $upload->uk86_upfile($file);
        if (!$return) {
            // 后缀名必须相同、
            if (strtoupper(CHARSET) == 'GBK') {
                $error = Uk86Language::uk86_getUTF8($upload->error);
            }
	...
	public function album_pic_watermarkOp(){
		...
		if ($store_wm_info['wm_image_name'] == '' && $store_wm_info['wm_text'] == ''){
			showDialog(Uk86Language::uk86_get('album_class_setting_wm'),"index.php?act=store_album&op=store_watermark",'error','CUR_DIALOG.close();');//"请先设置水印"
		}
		...
		foreach ($wm_list as $v) {
			$gd_image->uk86_create(BASE_UPLOAD_PATH.DS.ATTACH_GOODS.DS.$_SESSION['store_id'].DS.str_ireplace('.', '_1280.', $v['apic_cover']));//生成有水印的大图
		}
	...
	private function profile_menu($menu_type,$menu_key=''){
		...
		switch ($menu_type) {
			case 'album':
				$menu_array	= array(
				1=>array('menu_key'=>'album','menu_name'=>Uk86Language::uk86_get('nc_member_path_my_album'),'menu_url'=>'index.php?act=store_album'),
				2=>array('menu_key'=>'watermark','menu_name'=>Uk86Language::uk86_get('nc_member_path_watermark'),'menu_url'=>'index.php?act=store_album&op=store_watermark')
				);
				break;
			case 'album_pic':
				$menu_array	= array(
				1=>array('menu_key'=>'album','menu_name'=>Uk86Language::uk86_get('nc_member_path_my_album'),'menu_url'=>'index.php?act=store_album'),
				2=>array('menu_key'=>'pic_list','menu_name'=>Uk86Language::uk86_get('nc_member_path_album_pic_list'),'menu_url'=>'index.php?act=store_album&op=album_pic_list&id='.intval($_GET['id'])),
				3=>array('menu_key'=>'watermark','menu_name'=>Uk86Language::uk86_get('nc_member_path_watermark'),'menu_url'=>'index.php?act=store_album&op=store_watermark')
				);
				break;
			case 'album_pic_info':
				$menu_array	= array(
				1=>array('menu_key'=>'album','menu_name'=>Uk86Language::uk86_get('nc_member_path_my_album'),'menu_url'=>'index.php?act=store_album'),
				2=>array('menu_key'=>'pic_info','menu_name'=>Uk86Language::uk86_get('nc_member_path_album_pic_info'),'menu_url'=>'index.php?act=store_album&op=album_pic_info&id='.intval($_GET['id']).'&class_id='.intval($_GET['class_id'])),
				3=>array('menu_key'=>'watermark','menu_name'=>Uk86Language::uk86_get('nc_member_path_watermark'),'menu_url'=>'index.php?act=store_album&op=store_watermark')
				);
				break;
		}
