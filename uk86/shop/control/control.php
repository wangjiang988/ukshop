...
class Control{
    ...
    protected function showLayout() {
        ...
        $this->showCartCount();      //获取购物车数量
		
        Tpl::output('hot_search',@explode(',',C('hot_search')));//热门搜索

        $model_class = Model('goods_class');
        $goods_class = $model_class->get_all_category();    //获取商城左侧的商品分类
        $brand_info = $this->list_l_brand(); 
        //var_dump($goods_class);
        Tpl::output('brand_info', $brand_info);
        Tpl::output('show_goods_class',$goods_class);//商品分类

        //获取导航
        Tpl::output('nav_list', self::filter_nav(uk86_rkcache('nav',true)));        
    }
    
    /**
     * 获取推荐品牌以及一级分类id
     */
    public function list_l_brand(){
    	$list = Model()->table('brand')->where(array('brand_apply'=>'1'))->order('brand_sort asc')->select();
    	$brands_l = $this->_tidyBrand($list);
    	$goods_class = Model('goods_class')->getGoodsClassForCacheModel();
    	$brands_l_list = $brands_l['brand_r_list'];
    	foreach ($brands_l_list as $k => $v){
    		$gc_array = $this->_getTopClass($goods_class, $v['class_id']);
    		$brands_l_list[$k]['parent_id'] = $gc_array['gc_id'];
    	}
    	return $brands_l_list;
    }
    ...
    protected function showCartCount() {
        if (uk86_cookie('cart_goods_num') != null){
            $cart_num = intval(uk86_cookie('cart_goods_num'));
        }else {
    ...
    protected function article() {
        ...
        if (C('cache_open')) {
            if ($article = uk86_rkcache("index/article")) {
        ...
        if (C('cache_open')) {
            uk86_wkcache('index/article', array(
...
class BaseHomeControl extends Control {
    ...
    public function __construct(){
        ...
        if(!C('site_status')) uk86_halt(C('closed_reason'));
    }

}

class BaseWapControl extends Control{
	 ...
	 protected function getStoreInfo($store_id) {
	 	...
	 	if(empty($store_info)) {
	 		Tpl::output('store_close', '1');
	 		$this->wap_showDialog('店铺已关闭');
	 	}
...
class BaseFleaControl extends BaseHomeControl{
	public function __construct(){
		...
		Uk86Language::uk86_read('home_flea_index');
		
		//检测闲置市场是否开启，未开启显示提示
		/* if($GLOBALS['setting_config']['flea_isuse']!='1'){
		 showMessage(Uk86Language::uk86_get('flea_index_unable'),'index.php','','error');
		} */
	}
}
...
class BaseMemberControl extends Control {
    ...
    public function __construct(){
        ...
        Uk86Language::uk86_read('common,member_layout');
	...
	/**
     * 获得U币中心会员信息包括会员名、ID、会员头像、会员等级、经验值、等级进度、U币、已领代金券、已兑换礼品、礼品购物车
     */
    ...
    protected function _getCommonOperationsAndNavLink ($act = '') {
        ...
        if ($nav == '') {
            ...
            $nav_link[] = array('title' => '我的商城',  'link' => uk86_urlShop('member', 'home'));
    ...
    private function _getMenuList() {
        $menu_list = array(
                'trade' => array('name' => '交易管理', 'child' => array(
                        'member_order'      => array('name' => '实物交易订单', 'url'=>uk86_urlShop('member_order', 'index')),
                        'member_vr_order'   => array('name' => '虚拟兑码订单', 'url'=>uk86_urlShop('member_vr_order','index')),
                        'member_favorites'  => array('name' => '我的收藏', 'url'=>uk86_urlShop('member_favorites', 'fglist')),
                        'member_evaluate'   => array('name' => '交易评价/晒单', 'url'=>uk86_urlShop('member_evaluate', 'list')),
                        //'predeposit'        => array('name' => '账户余额', 'url'=>uk86_urlShop('predeposit', 'pd_log_list')),
						//'member_flea'     	=> array('name' => '我的闲置', 'url'=>uk86_urlShop('member_flea', 'index')),
                		'member_free'       => array('name' => '我的F码', 'url'=>uk86_urlShop('member_free', 'index')),
                        'member_points'     => array('name' => '我的U币', 'url'=>uk86_urlShop('member_points', 'index')),
                        'member_voucher'    => array('name' => '我的卡券包', 'url'=>uk86_urlShop('member_voucher', 'index'))
                )),
                'serv' => array('name' => '客户服务', 'child' => array(
                        'member_refund'     => array('name' => '退款及退货', 'url'=>uk86_urlShop('member_refund', 'index')),
                        'member_complain'   => array('name' => '交易投诉', 'url'=>uk86_urlShop('member_complain', 'index')),
                        'member_consult'    => array('name' => '商品咨询', 'url'=>uk86_urlShop('member_consult', 'my_consult')),
                        'member_inform'     => array('name' => '违规举报', 'url'=>uk86_urlShop('member_inform', 'index')),
                        'member_mallconsult'=> array('name' => '平台客服', 'url'=>uk86_urlShop('member_mallconsult', 'index'))
                )),
                'info' => array('name' => '资料管理', 'child' => array(
                        'member_information'=> array('name' => '账户信息', 'url'=>uk86_urlShop('member_information', 'member')),
                        'member_security'   => array('name' => '账户安全', 'url'=>uk86_urlShop('member_security', 'index')),
                        'member_address'    => array('name' => '收货地址', 'url'=>uk86_urlShop('member_address', 'address')),
                        'member_message'    => array('name' => '我的消息', 'url'=>uk86_urlShop('member_message', 'message')),
                        'member_snsfriend'  => array('name' => '我的好友', 'url'=>uk86_urlShop('member_snsfriend', 'find')),
                        'member_goodsbrowse'=> array('name' => '我的足迹', 'url'=>uk86_urlShop('member_goodsbrowse', 'list')),
                        'member_connect'    => array('name' => '第三方账号登录', 'url'=>uk86_urlShop('member_connect', 'qqbind')),
                        'member_sharemanage'=> array('name' => '分享绑定', 'url'=>uk86_urlShop('member_sharemanage', 'index'))
                )),
                'app' => array('name' => '应用管理', 'child' => array(
                        'sns'               => array('name' => '个人主页', 'url'=>uk86_urlShop('member_snshome', 'index')),
                        'cms'               => array('name' => '我的CMS', 'url'=>uk86_urlCMS('member_article', 'article_list')),
                        'circle'            => array('name' => '我的圈子', 'url'=>uk86_urlCircle('p_center', 'index')),
               //         'microshop'         => array('name' => '我的微商城', 'url'=>uk86_urlMicroshop('home', 'index', array('member_id' => $_SESSION['member_id'])))
                ))
        );
...
class BaseSNSControl extends Control {
    ...
    public function __construct(){
        ...
        Uk86Language::uk86_read('common,sns_layout');
    ...
    protected function formatDate($time){
        ...
        if ($reference_date_time == $handle_date_time){
            ...
            $timetext = Uk86Language::uk86_get('sns_yesterday');
        }elseif ($reference_date_time-$handle_date_time==60*60*48){
            $timetext = Uk86Language::uk86_get('sns_beforeyesterday');
        }else {
            $month_text = Uk86Language::uk86_get('nc_month');
            $day_text = Uk86Language::uk86_get('nc_day');
    ...
    public function get_member_info() {
        ...
        if(empty($member_info)){
            uk86_showMessage(L('wrong_argument'), 'index.php?act=member_snshome', '', 'error');
        }
        //粉丝数
    ...
    private function add_visit(){
        ...
        //增加主人访问次数
        $cookie_str = uk86_cookie('visitor');
        $cookie_arr = array();
        $is_increase = false;
        if (empty($cookie_str)){
            //cookie不存在则直接增加访问次数
        ...
        if ($is_increase == true){
            ...
            uk86_setNcCookie('visitor',implode('_',$cookie_arr),24*3600);//保存24小时
        }
    }
    //我的关注
...
class BaseStoreControl extends Control {
    ...
    public function __construct(){
        ...
        Uk86Language::uk86_read('common,store_layout,store_show_store_index');
        ...
        if($store_id <= 0) {
            uk86_showMessage(L('nc_store_close'), '', '', 'error');
        }
...
class BaseGoodsControl extends BaseStoreControl {
    ...
    public function __construct(){
        ...
        Uk86Language::uk86_read('common,store_layout');
...
class BaseSellerControl extends Control {
    ...
    public function __construct(){
        Uk86Language::uk86_read('common,store_layout,member_layout');
        if(!C('site_status')) uk86_halt(C('closed_reason'));
        Tpl::setDir('seller');
        ...
        if ($_GET['act'] !== 'seller_login') {
            ...
            if (uk86_checkPlatformStore()) {
                ...
                $store_grade = uk86_rkcache('store_grade', true);
    ...
    private function _getMenuList() {
        $menu_list = array(
            'goods' => array('name' => '商品', 'child' => array(
                ...
                array('name'=>'店铺首页楼层','act'=>'store_goods_floor','op'=>'index'),
//				array('name' => '淘宝导入', 'act'=>'taobao_import', 'op'=>'index'),
            ...
            'promotion' => array('name' => '促销', 'child' => array(
                ...
                array('name' => '卡券包管理', 'act'=>'store_voucher', 'op'=>'templatelist'),
        ...
        //自营店铺发放F码功能
        $is_fcode = Model()->table('store')->where(array('store_id' => $_SESSION['store_id']))->field('is_own_shop')->find();
        if($is_fcode['is_own_shop'] == 1){
        	array_push($menu_list['promotion']['child'],array('name' => '发放F码', 'act'=>'store_fcode_grand', 'op'=>'index'));
        }
        return $menu_list;
    }
    ...
    public function storeAutoShare($data, $type) {
        ...
        if ($auto_setting['sauto_' . $type] == 1) {
            ...
            if ($auto_setting['sauto_' . $type . 'title'] != '') {
                ...
                $title = Uk86Language::uk86_get($auto_title);
            }
        }
        if ($auto_sign) {
            // 插入数据
    ...
    private function checkStoreMsg() {//判断cookie是否存在
        ...
        if (uk86_cookie($cookie_name) != null && intval(uk86_cookie($cookie_name)) >=0){
            ...
            uk86_setNcCookie($cookie_name,intval($countnum),2*3600);//保存2小时
        }
	 ...
	 * @author wangjiang
	 */
	public function _change_pm_order_afterpay($payment_code,$order_sn,$order_amount){
		//修改订单状态
		$condition['order_state'] = ORDER_STATE_PAY;//修改订单状态为已支付
		$condition['payment_code'] = $payment_code;//修改订单支付时间
		$condition['payment_time'] = time();//修改订单支付时间
		$condition['order_amount'] = $order_amount;//替换成实际支付价格。
		Model('pm_order')->editOrder($condition,array('order_sn'=>$order_sn));
	 ...
	 */
	 public function quota_create_orderOp(){
			...
			$result = $logic_buy->createPromotionVrOrder($_POST,$_GET['type'],$_SESSION['member_id'], $_SESSION['member_name'],
	  ...
	  * @author wangjiang
	  */
	public  function quota_add_payOp(){
...
class BaseStoreSnsControl extends Control {
    ...
    protected function getStoreInfo($store_id) {
        ...
        if (empty($store_info)) {
            uk86_showMessage(Uk86Language::uk86_get('store_sns_store_not_exists'),'','html','error');
        }
        //处理地区信息
        $area_array	= array();
        $area_array = explode("\t",$store_info["area_info"]);
        $map_city	= Uk86Language::uk86_get('store_sns_city');
        $city	= '';
