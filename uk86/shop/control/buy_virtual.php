...
class buy_virtualControl extends BaseBuyControl {
    ...
    public function payOp() {
        ...
        if ($order_id <= 0){
            uk86_showMessage('该订单不存在','index.php?act=member_vr_order','html','error');
        }
        ...
        if ($diff_pay_amount == 0) {
            uk86_redirect('index.php?act=buy_virtual&op=pay_ok&order_sn='.$order_info['order_sn'].'&order_id='.$order_info['order_id'].'&order_amount='.uk86_ncPriceFormat($order_info['order_amount']));
        }
        ...
        if (empty($payment_list)) {
            uk86_showMessage('暂未找到合适的支付方式','index.php?act=member_vr_order','html','error');
        }
