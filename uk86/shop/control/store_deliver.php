...
class store_deliverControl extends BaseSellerControl {
	public function __construct() {
		parent::__construct();
		Uk86Language::uk86_read('member_store_index,deliver');
	}
	...
	public function send_address_selectOp() {
	    Uk86Language::uk86_read('deliver');
	...
	public function search_deliverOp(){
		...
		if (!is_numeric($order_sn)) uk86_showMessage(Uk86Language::uk86_get('wrong_argument'),'','html','error');
		$model_order	= Model('order');
	...
	public function delay_receiveOp(){
	    ...
	    if (uk86_chksubmit()) {
	        ...
	        if (!in_array($delay_date,array(5,10,15))) {
	           showDialog(Uk86Language::uk86_get('wrong_argument'),'','error',empty($_GET['inajax']) ?'':'CUR_DIALOG.close();');
	        }
	...
	public function get_expressOp(){
        ...
        if (strtoupper(CHARSET) == 'GBK'){
            $output = Uk86Language::uk86_getUTF8($output);//网站GBK使用编码时,转换为UTF-8,防止json输出汉字问题
        }
    ...
    public function waybill_printOp() {
        ...
        if(empty($store_waybill_info)) {
            uk86_showMessage('请首先绑定打印模板', uk86_urlShop('store_waybill', 'waybill_manage'), '', 'error');
        }
	...
	private function profile_menu($menu_type,$menu_key='') {
		...
		switch ($menu_type) {
			case 'deliver':
				$menu_array = array(
				array('menu_key'=>'deliverno',			'menu_name'=>Uk86Language::uk86_get('nc_member_path_deliverno'),	'menu_url'=>'index.php?act=store_deliver&op=index&state=deliverno'),
				array('menu_key'=>'delivering',			'menu_name'=>Uk86Language::uk86_get('nc_member_path_delivering'),	'menu_url'=>'index.php?act=store_deliver&op=index&state=delivering'),
				array('menu_key'=>'delivered',		'menu_name'=>Uk86Language::uk86_get('nc_member_path_delivered'),	'menu_url'=>'index.php?act=store_deliver&op=index&state=delivered'),
				);
				break;
			case 'search':
				$menu_array = array(
				1=>array('menu_key'=>'nodeliver',			'menu_name'=>Uk86Language::uk86_get('nc_member_path_deliverno'),	'menu_url'=>'index.php?act=store_deliver&op=index&state=nodeliver'),
				2=>array('menu_key'=>'delivering',			'menu_name'=>Uk86Language::uk86_get('nc_member_path_delivering'),	'menu_url'=>'index.php?act=store_deliver&op=index&state=delivering'),
				3=>array('menu_key'=>'delivered',		'menu_name'=>Uk86Language::uk86_get('nc_member_path_delivered'),	'menu_url'=>'index.php?act=store_deliver&op=index&state=delivered'),
				4=>array('menu_key'=>'search',		'menu_name'=>Uk86Language::uk86_get('nc_member_path_deliver_info'),	'menu_url'=>'###'),
				);
				break;
		}
