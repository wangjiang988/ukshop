...
class store_livegroupControl extends BaseSellerControl {
    ...
    public function __construct() {
        parent::__construct();
        //读取语言包
        Uk86Language::uk86_read('member_live');
    }
	...
	public function groupbuy_addOp(){
		if(uk86_chksubmit()){
			...
			if($res){
				uk86_showMessage('添加成功','index.php?act=store_livegroup&op=store_livegroup','','succ');
			}else{
				uk86_showMessage('添加失败','','','error');
			}
		}
	...
	public function edit_livegroupbuyOp(){
		if(uk86_chksubmit()){//数据验证
			...
			if($res){
				uk86_showMessage('编辑成功','index.php?act=store_livegroup&op=store_livegroup','','succ');
			}else{
				uk86_showMessage('编辑失败','index.php?act=store_livegroup&op=store_livegroup','','error');
			}
		}
	...
	public function delgroupOp(){
		...
		if($res){
			uk86_showMessage('删除成功','','','succ');
		}else{
			uk86_showMessage('删除失败','','','error');
		}
	}
