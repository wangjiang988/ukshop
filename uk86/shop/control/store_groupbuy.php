...
class store_groupbuyControl extends BaseSellerControl {
    ...
    public function __construct() {
        ...
        if (intval(C('groupbuy_allow')) !== 1){
            uk86_showMessage(Uk86Language::uk86_get('groupbuy_unavailable'),'index.php?act=seller_center','','error');
        }
    }
    /**
     * 默认显示抢购列表
    ...
    public function groupbuy_quota_create_orderOp(){
        ...
        $result = $logic_buy->createPromotionVrOrder($_POST, '1',$_SESSION['member_id'], $_SESSION['member_name'],
	...
	public function quota_add_saveOp(){
		$this->groupbuy_quota_add_saveOp();
	}
	
    /**
     * 抢购套餐购买保存
	 * @modify wangjiang  修改为购买行为成功后的订单后续处理行为。
	 *  1.修改订单状态，
	 *  2.修改订单的信息.
     **/
    public function groupbuy_quota_add_saveOp() {
        ...
        $groupbuy_quota_quantity = isset($order['goods_num'])?$order['goods_num']:0; //餐套数量
       ...
       showDialog(Uk86Language::uk86_get('groupbuy_quota_add_success'), uk86_urlShop('store_groupbuy', 'groupbuy_list'), 'succ');
    }
    ...
    public function groupbuy_editOp() {
        ...
        if(!empty($_GET['groupbuy_id'])) {
            $condition['groupbuy_id'] = $_GET['groupbuy_id'];
        } else {
            uk86_showMessage('参数错误');
        }
        
        $groupbuy_info = $model_groupbuy->getGroupbuyInfo($condition);

        if ($groupbuy_info['store_id'] != $_SESSION['store_id']) {
            uk86_showMessage('您无权操作');
        }
        ...
        Tpl::output('groupbuy_classes', $model_groupbuy->getGroupbuyClasses());

        self::profile_menu('groupbuy_edit');
        Tpl::showpage('store_groupbuy.edit');
    }

    /**
     * 编辑抢购的保存
     */
    public function groupbuy_edit_saveOp() {
        //获取提交的数据
        $group_id = intval($_POST['groupbuy_id']);
        if(empty($group_id)) {
           showDialog(Uk86Language::uk86_get('param_error'));
        }
        //如何判断是否该店铺
        ...
        if (!uk86_checkPlatformStore()) {
            ...
            $current_groupbuy_quota = $model_groupbuy_quota->getGroupbuyQuotaCurrent($_SESSION['store_id']);
            if(empty($current_groupbuy_quota)) {
               showDialog('当前没有可用套餐，请先购买套餐',uk86_urlShop('store_groupbuy', 'groupbuy_quota_add'),'error');
            }
        }

        $condition = array();
        $condition['groupbuy_id'] = $group_id;
        ...
        $param['groupbuy_rebate'] = uk86_ncPriceFormat(floatval($_POST['groupbuy_price'])/floatval($goods_info['goods_price'])*10);
        ...
        // 保存
        $result = $model_groupbuy->editGroupbuy($param, $condition);
        if($result) {
            // 自动发布动态
            ...
            $this->recordSellerLog('修改抢购活动，抢购名称：'.$param['groupbuy_name'].'，商品名称：'.$param['goods_name']);
           showDialog("修改成功",'index.php?act=store_groupbuy','succ');
        }else {
           showDialog(Uk86Language::uk86_get('groupbuy_add_fail'),'index.php?act=store_groupbuy');
        }
    }
    ...
    public function groupbuy_saveOp() {
        ...
        if($result) {
           ...
           showDialog(Uk86Language::uk86_get('groupbuy_add_success'),'index.php?act=store_groupbuy','succ');
        }else {
           showDialog(Uk86Language::uk86_get('groupbuy_add_fail'),'index.php?act=store_groupbuy');
        }
    }
    ...
    public function groupbuy_goods_infoOp() {
        ...
        $data['goods_image'] = uk86_thumb($goods_info, 240);
        $data['goods_href'] = uk86_urlShop('goods', 'index', array('goods_id' => $goods_info['goods_id']));
    ...
    private function _image_upload($file) {
        ...
        if(!empty($_FILES[$file]['name'])) {
            ...
            if($result) {
                ...
                $data['file_url'] = uk86_gthumb($upload->file_name, 'mid');
            } else {
                $data['result'] = false;
    ...
    private function profile_menu($menu_key='') {
        $menu_array	= array(
            1=>array('menu_key'=>'groupbuy_list','menu_name'=>L('nc_member_path_group_list'),'menu_url'=>uk86_urlShop('store_groupbuy', 'groupbuy_list'))
        );
        switch ($menu_key){
        case 'groupbuy_add':
        ...
        case 'groupbuy_quota_add':
            $menu_array[] = array('menu_key'=>'groupbuy_quota_add','menu_name'=>'购买套餐','menu_url'=>uk86_urlShop('store_groupbuy', 'groupbuy_quota_add'));
            break;
        case 'groupbuy_edit':
