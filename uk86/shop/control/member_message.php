...
class member_messageControl extends BaseMemberControl {
    public function __construct() {
        parent::__construct();
		//读取语言包
		Uk86Language::uk86_read('member_home_message');
    }
	/**
	 * 收到(普通)站内信列表
	 *
	...
	public function sendmsgOp(){
		...
		if (!$isallowsend){
			uk86_showMessage(Uk86Language::uk86_get('home_message_noallowsend'),$referer_url,'html','error');
		}
	...
	public function savemsgOp() {
		...
		if (uk86_chksubmit()) {
			...
			$obj_validate->validateparam = array(
				array("input"=>$_POST["to_member_name"],"require"=>"true","message"=>Uk86Language::uk86_get('home_message_receiver_null')),
				array("input"=>$_POST["msg_content"],"require"=>"true","message"=>Uk86Language::uk86_get('home_message_content_null')),
			);
			...
			if (empty($membername_arr)){
				showDialog(Uk86Language::uk86_get('home_message_receiver_null'));
			}
			...
			if($_GET['type'] == 'sns_board'){
				...
				showDialog(Uk86Language::uk86_get('home_message_send_success'),'index.php?act=member_message&op=privatemsg','succ');
			}
		}
	}
	/**
	...
	public function savereplyOp() {
		...
		if ($_POST['form_submit'] == 'ok') {
			...
			$obj_validate = new Uk86Validate();
			$obj_validate->validateparam = array(
				array("input"=>$_POST["msg_content"],"require"=>"true","message"=>Uk86Language::uk86_get('home_message_reply_content_null'))
			);
			$error = $obj_validate->uk86_validate();
			if ($error != ''){
				if($_GET['inajax'] == 1){
	...
	private function profile_menu($menu_key='') {
		$menu_array	= array(
    		1=>array('menu_key'=>'message',	'menu_name'=>Uk86Language::uk86_get('home_message_received_message'),'menu_url'=>'index.php?act=member_message&op=message'),
    		2=>array('menu_key'=>'private',	'menu_name'=>Uk86Language::uk86_get('home_message_private_message'),	'menu_url'=>'index.php?act=member_message&op=privatemsg'),
    		3=>array('menu_key'=>'system',	'menu_name'=>Uk86Language::uk86_get('home_message_system_message'),	'menu_url'=>'index.php?act=member_message&op=systemmsg'),
    		4=>array('menu_key'=>'close',	'menu_name'=>Uk86Language::uk86_get('home_message_close'),	'menu_url'=>'index.php?act=member_message&op=personalmsg'),
		    5=>array('menu_key'=>'setting', 'menu_name'=>'接收设置', 'menu_url'=>uk86_urlShop('member_message', 'setting'))
		);
		if($menu_key == 'sendmsg') {
			$menu_array[] = array('menu_key'=>'sendmsg','menu_name'=>Uk86Language::uk86_get('home_message_send_message'),'menu_url'=>'index.php?act=member_message&op=sendmsg');
		}elseif($menu_key == 'showmsg') {
			$menu_array[] = array('menu_key'=>'showmsg','menu_name'=>Uk86Language::uk86_get('home_message_view_message'),'menu_url'=>'#');
		}
