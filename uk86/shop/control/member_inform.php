...
class member_informControl extends BaseMemberControl{
    ...
    public function inform_submitOp() {
        ...
        if($model_inform->isProcessOfInform($goods_id)) {
           uk86_showMessage(Uk86Language::uk86_get('inform_handling'),'','html','error');
        }
        ...
        if(empty($inform_subject_type_list)) {
            uk86_showMessage(Uk86Language::uk86_get('inform_type_null'),'','html','error');
        }
        $this->profile_menu('inform_list');
    ...
    public function inform_saveOp() {
        ...
        $obj_validate->validateparam = array(
            array("input"=>$input["inform_content"], "require"=>"true","validator"=>"Length","min"=>"1","max"=>"100","message"=>Uk86Language::uk86_get('inform_content_null')),
            array("input"=>$input["inform_subject_content"], "require"=>"true", "validator"=>"Length","min"=>"1","max"=>"50","message"=>Uk86Language::uk86_get('para_error')),
        );
    ...
    public function inform_cancelOp() {
        ...
        if(intval($inform_info['inform_state']) === 1) {
           ...
           showDialog(Uk86Language::uk86_get('inform_cancel_success'),'reload','succ');
        }
        else {
           showDialog(Uk86Language::uk86_get('inform_cancel_fail'),'','error');
        }
    }
    ...
    private function get_goods_info_byid($goods_id) {
        ...
        //检查该商品是否存在
        if(empty($goods_info)) {
            uk86_showMessage(Uk86Language::uk86_get('goods_null'),'','html','error');
        }
        return $goods_info;
    }
    ...
    private function check_member_allow_inform() {
        ...
        if(!$model_member->isMemberAllowInform($_SESSION['member_id'])) {
            uk86_showMessage(Uk86Language::uk86_get('deny_inform'),'','html','error');
        }
    }
    ...
    public function get_subject_by_typeidOp() {
        ...
        if(empty($inform_subject_type_id)) {
            ...
            if (strtoupper(CHARSET) == 'GBK'){
                $inform_subject_list = Uk86Language::uk86_getUTF8($inform_subject_list);
		        }
