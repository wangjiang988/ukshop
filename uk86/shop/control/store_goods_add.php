...
class store_goods_addControl extends BaseSellerControl {
    ...
    private function checkStore(){
        ...
        if ($goodsLimit > 0) {
            ...
            if ($goods_num >= $goodsLimit) {
                uk86_showMessage(L('store_goods_index_goods_limit') . $goodsLimit . L('store_goods_index_goods_limit1'), 'index.php?act=store_goods_online&op=goods_list', 'html', 'error');
            }
        }
    }
     ...
     * 异步判断店铺地址是否填写
     */
    public function store_info_addressOp(){
    	if($_POST['isuse']){
    		$info = Model('store')->where(array('store_id' => $_SESSION['store_id']))->field('live_store_name, live_store_address, live_store_tel')->find();
    		if(is_array($info) && !empty($info['live_store_name']) && !empty($info['live_store_address']) && !empty($info['live_store_tel'])){
    			echo 1;
    		}
    	}
    }
    ...
    public function save_goodsOp() {
        if (uk86_chksubmit()) {
            ...
            $common_array['store_floor_id']     = intval($_POST['store_floor_id']);    //楼层id
            $common_array['store_floor_name']   = $_POST['store_floor_name'];         //楼层名称
            // 序列化保存手机端商品描述数据
            if ($_POST['m_body'] != '') {
 		...
 		require_once(BASE_RESOURCE_PATH.DS.'phpqrcode'.DS.'index.php');
                ...
                if (is_array($_POST['spec'])) {
                    foreach ($_POST['spec'] as $value) {
                        ...
                        $goods['store_floor_id']    = $common_array['store_floor_id'];   //楼层id
                        $goods['store_floor_name']  = $common_array['store_floor_name']; //楼层名称
    ...
    public function image_uploadOp() {
        ...
        $result = $upload->uk86_upfile($_POST['name']);
        if (!$result) {
            if (strtoupper(CHARSET) == 'GBK') {
        ...
        $data = array ();
        $data ['thumb_name'] = uk86_cthumb($upload->getSysSetPath() . $upload->thumb_image, 240, $_SESSION['store_id']);
        $data ['name']      = $img_path;
    ...
    public function ajax_stapledelOp() {
        ...
        if ($staple_id < 1) {
            echo json_encode ( array (
                    'done' => false,
                    'msg' => Uk86Language::uk86_get ( 'wrong_argument' )
            ) );
            die ();
        }
        /**
