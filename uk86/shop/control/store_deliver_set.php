...
class store_deliver_setControl extends BaseSellerControl {
    ...
    public function daddress_delOp() {
        ...
        if ($address_id <=  0) {
           showDialog(Uk86Language::uk86_get('store_daddress_del_fail'),'','error');
        }
		$condition = array();
		$condition['address_id'] = $address_id;
		$condition['store_id'] = $_SESSION['store_id'];
        ...
        if ($delete){
           showDialog(Uk86Language::uk86_get('store_daddress_del_succ'),'index.php?act=store_deliver_set&op=daddress_list','succ');
        }else {
           showDialog(Uk86Language::uk86_get('store_daddress_del_fail'),'','error');
        }
    }
   ...
   public function daddress_default_setOp() {
	 ...
	 * 包邮设置
	 */
// 	public function free_deliveryOp(){
// 		if (uk86_chksubmit()) {
// 			$store_free_freight = intval($_POST['store_free_delivery']);
// 			$succ = Model()->table('store')->where('store_id = '.$_SESSION['store_id'])->update(array('store_free_delivery' => $store_free_freight));
// 			if($succ){
// 				showDialog('操作成功', '', 'succ');
// 			}else{
// 				showDialog('系统错误', '', 'error');
// 			}
// 		}
// 		Tpl::output('store_free_delivery', $this->store_info['store_free_delivery']);
// 		self::profile_menu('daddress','free_delivery');
// 		Tpl::showpage('store_free_delivery.index');
// 	}
	...
	public function print_setOp(){
	    ...
	    if(uk86_chksubmit()){
	        ...
	        if ($rs){
	           ...
	           showDialog(Uk86Language::uk86_get('nc_common_save_fail'),'','error');
	        }
	    }else{
	...
	private function profile_menu($menu_type, $menu_key = '') {
		...
		switch ($menu_type) {
			case 'daddress':
				$menu_array = array(
				array('menu_key'=>'daddress',	'menu_name'=>Uk86Language::uk86_get('store_deliver_daddress_list'),	'menu_url'=>'index.php?act=store_deliver_set&op=daddress_list'),
				array('menu_key'=>'express',		'menu_name'=>Uk86Language::uk86_get('store_deliver_default_express'),	'menu_url'=>'index.php?act=store_deliver_set&op=express'),
//				array('menu_key'=>'free_delivery',  'menu_name'=>'包邮设置',  'menu_url'=>'index.php?act=store_deliver_set&op=free_delivery'),
