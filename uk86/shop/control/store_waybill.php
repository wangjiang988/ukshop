...
class store_waybillControl extends BaseSellerControl{
    ...
    public function waybill_bind_saveOp() {
        ...
        if($result) {
            uk86_showMessage('绑定成功', uk86_urlShop('store_waybill', 'waybill_manage'));
        } else {
            uk86_showMessage('绑定失败', '', '', 'error');
        }
    }
    ...
    public function waybill_unbindOp() {
        ...
        if($result) {
            uk86_showMessage('解绑成功', '');
        } else {
            uk86_showMessage('解绑失败', '', '', 'error');
        }
    }
    ...
    public function waybill_setting_saveOp() {
        ...
        if($result) {
           showDialog(L('nc_common_save_succ'), uk86_urlShop('store_waybill', 'waybill_manage'), 'succ');
        } else {
    ...
    public function waybill_set_defaultOp() {
        ...
        if($result) {
            ...
            uk86_showMessage(L('nc_common_save_fail'), '', '', 'error');
        }
    }
    ...
    public function waybill_saveOp() {
        ...
        if(!isset($result['error'])) {
           showDialog(L('nc_common_save_succ'), uk86_urlShop('store_waybill', 'waybill_list'), 'succ');
        } else {
           showDialog(L('nc_common_save_fail'), uk86_urlShop('store_waybill', 'waybill_list'));
        }
    }
    ...
    public function waybill_delOp() {
        ...
        if($result) {
            uk86_showMessage(L('nc_common_del_succ'));
        } else {
            uk86_showMessage(L('nc_common_del_fail'));
        }
    }
    ...
    public function waybill_designOp() {
        ...
        if(isset($result['error'])) {
            uk86_showMessage($result['error'], '', '', 'error');
        }
    ...
    private function profile_menu($menu_key = '') {
        ...
        $menu_array[] = array(
            ...
            'menu_name' => '模板绑定',
            'menu_url' => uk86_urlShop('store_waybill', 'waybill_manage')
        );
        $menu_array[] = array(
            'menu_key' => 'waybill_list',
            'menu_name' => '自建模板',
            'menu_url' => uk86_urlShop('store_waybill', 'waybill_list')
        );
        if($menu_key == 'waybill_bind') {
            $menu_array[] = array(
                'menu_key' => 'waybill_bind',
                'menu_name' => '选择模板',
                'menu_url' => uk86_urlShop('store_waybill', 'waybill_bind')
            );
        }
        if($menu_key == 'waybill_setting') {
            $menu_array[] = array(
                ...
                'menu_name' => '模板设置',
                'menu_url' => uk86_urlShop('store_waybill', 'waybill_setting')
            );
        }
        if($menu_key == 'waybill_add') {
            $menu_array[] = array(
                ...
                'menu_name' => '添加模板',
                'menu_url' => uk86_urlShop('store_waybill', 'waybill_add')
            );
        }
        if($menu_key == 'waybill_edit') {
            $menu_array[] = array(
                ...
                'menu_name' => '编辑模板',
                'menu_url' => uk86_urlShop('store_waybill', 'waybill_edit')
            );
        }
        if($menu_key == 'waybill_design') {
            $menu_array[] = array(
                ...
                'menu_name' => '设计模板',
                'menu_url' => uk86_urlShop('store_waybill', 'waybill_design')
            );
        }
