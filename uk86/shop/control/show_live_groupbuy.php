...
class show_live_groupbuyControl extends BaseHomeControl {
    ...
    public function __construct() {
        ...
        //读取语言包
        Uk86Language::uk86_read('member_groupbuy,home_cart_index,live_groupbuy');

		//分类导航
		$nav_link = array(
			0=>array(
				'title'=>Uk86Language::uk86_get('homepage'),
				'link'=>'index.php'
			),
			1=>array(
				'title'=>Uk86Language::uk86_get('nc_groupbuy')
			)
		);
    ...
    public function indexOp() {
		...
		//区域城市商区
		if(uk86_cookie('city_id')){
			$condition_live_groupbuy['city_id']	= uk86_cookie('city_id');
		}
	...
	public function live_groupbuy_listOp(){
		...
		//区域城市商区
		if(uk86_cookie('city_id')){
			$condition['city_id']	= uk86_cookie('city_id');
		}
	...
	public function livegroupbuyorderOp(){
		if($_SESSION['is_login'] !== '1'){//检测是否登录
			uk86_showMessage('请先登录','index.php?act=login&op=index','','error');
		}
		...
		if(empty($live_groupbuy)){
			uk86_showMessage('该抢购不存在','','','error');
		}

		if($live_groupbuy['start_time']>time()){
			uk86_showMessage('抢购即将开始','','','error');
		}

		if($live_groupbuy['end_time']<time()){
			uk86_showMessage('抢购已经结束','','','error');
		}
		Tpl::output('live_groupbuy',$live_groupbuy);
		...
		if(uk86_chksubmit()){
			...
			if(isset($_POST['pd_pay']) && $_POST['pd_pay']==1){
				...
				if($params['price']>$member['available_predeposit']){//使用预存款支付部分
					...
					if(!$res){
						uk86_showMessage('订单支付失败','index.php?act=member_live','html','error');
					}
	...
	public function paymentOp(){
		...
		if(empty($order_info)){
			uk86_showMessage('该订单不存在','index.php?act=member_live','html','error');
		}
	...
	private function _api_pay($order_info, $payment_info) {
    	...
    	if(!file_exists($inc_file)){
    		uk86_showMessage(Uk86Language::uk86_get('payment_index_lose_file'),'','html','error');
    	}
