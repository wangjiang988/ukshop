...
class store_promotion_xianshiControl extends BaseSellerControl {
    ...
    public function __construct() {
        ...
        //读取语言包
        Uk86Language::uk86_read('member_layout,promotion_xianshi');
        //检查限时折扣是否开启
        if (intval(C('promotion_allow')) !== 1){
            uk86_showMessage(Uk86Language::uk86_get('promotion_unavailable'),'index.php?act=store','','error');
        }
    ...
    public function xianshi_addOp() {
        if (uk86_checkPlatformStore()) {
            ...
            if(empty($current_xianshi_quota)) {
                uk86_showMessage(Uk86Language::uk86_get('xianshi_quota_current_error1'),'','','error');
            }
            Tpl::output('current_xianshi_quota',$current_xianshi_quota);
        }
    ...
    public function xianshi_edit_saveOp() {
        ...
        if($result && $result) {
           ...
           showDialog(Uk86Language::uk86_get('nc_common_op_succ'),self::LINK_XIANSHI_LIST,'succ','',3);
        }else {
    ...
    public function xianshi_delOp() {
        ...
        if($result) {
           ...
           showDialog(L('nc_common_op_succ'), uk86_urlShop('store_promotion_xianshi', 'xianshi_list'), 'succ');
        } else {
...
//         showDialog(Uk86Language::uk86_get('xianshi_quota_quantity_error'));
//      }
		...
		$xianshi_quota_quantity = isset($order['goods_num'])?$order['goods_num']:0; //餐套数量
       ...
       showDialog(Uk86Language::uk86_get('xianshi_quota_add_success'),self::LINK_XIANSHI_LIST,'succ');
    }
...
//		  `goods_type` int(4) NOT NULL DEFAULT '1' COMMENT '1抢购2.限时，3满送，4优惠套餐，5推荐站位，6卡卷包',
        $result = $logic_buy->createPromotionVrOrder($_POST, '2',$_SESSION['member_id'], $_SESSION['member_name'],
    ...
    public function xianshi_goods_price_editOp() {
        ...
        if($result) {
            ...
            // 添加对列修改商品促销价格
            Uk86QueueClient::push('updateGoodsPromotionPriceByGoodsId', $xianshi_goods_info['goods_id']);
    ...
    private function profile_menu($menu_key='') {
        $menu_array = array(
            1=>array('menu_key'=>'xianshi_list','menu_name'=>Uk86Language::uk86_get('promotion_active_list'),'menu_url'=>'index.php?act=store_promotion_xianshi&op=xianshi_list'),
        );
        switch ($menu_key){
        	case 'xianshi_add':
                $menu_array[] = array('menu_key'=>'xianshi_add','menu_name'=>Uk86Language::uk86_get('promotion_join_active'),'menu_url'=>'index.php?act=store_promotion_xianshi&op=xianshi_add');
        		break;
        	case 'xianshi_edit':
                $menu_array[] = array('menu_key'=>'xianshi_edit','menu_name'=>'编辑活动','menu_url'=>'javascript:;');
        		break;
        	case 'xianshi_quota_add':
                $menu_array[] = array('menu_key'=>'xianshi_quota_add','menu_name'=>Uk86Language::uk86_get('promotion_buy_product'),'menu_url'=>'index.php?act=store_promotion_xianshi&op=xianshi_quota_add');
        		break;
        	case 'xianshi_manage':
                $menu_array[] = array('menu_key'=>'xianshi_manage','menu_name'=>Uk86Language::uk86_get('promotion_goods_manage'),'menu_url'=>'index.php?act=store_promotion_xianshi&op=xianshi_manage&xianshi_id='.$_GET['xianshi_id']);
        		break;
        }
