...
class buyControl extends BaseBuyControl {
    ...
    public function buy_step1Op() {
        ...
        if(!empty($_POST['cart_id']) && count($_POST['cart_id']) == 1){
    ...
    public function pd_payOp() {
        $pay_sn	= $_GET['pay_sn'];
        if (!preg_match('/^\d{18}$/',$pay_sn)){
            uk86_showMessage(Uk86Language::uk86_get('para_error'),'index.php?act=predeposit','html','error');
        }
        ...
        if (intval($pd_info['pdr_payment_state'])) {
            uk86_showMessage('您的订单已经支付，请勿重复支付','index.php?act=predeposit','html','error');
        }
        ...
        if (empty($payment_list)) {
            uk86_showMessage('暂未找到合适的支付方式','index.php?act=predeposit&op=index','html','error');
        }
        Tpl::output('payment_list',$payment_list);
