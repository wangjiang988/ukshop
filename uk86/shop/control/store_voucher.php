...
class store_voucherControl extends BaseSellerControl{
	...
	public function __construct() {
		parent::__construct() ;
		//读取语言包
		Uk86Language::uk86_read('member_layout,member_voucher');
		//判断系统是否开启卡券包功能
		if (C('voucher_allow') != 1){
			uk86_showMessage(Uk86Language::uk86_get('voucher_unavailable'),'index.php?act=store','html','error');
		}
		//申请记录状态
		$this->applystate_arr = array('new'=>array(1,Uk86Language::uk86_get('voucher_applystate_new')),'verify'=>array(2,Uk86Language::uk86_get('voucher_applystate_verify')),'cancel'=>array(3,Uk86Language::uk86_get('voucher_applystate_cancel')));
		//套餐状态
		$this->quotastate_arr = array('activity'=>array(1,Uk86Language::uk86_get('voucher_quotastate_activity')),'cancel'=>array(2,Uk86Language::uk86_get('voucher_quotastate_cancel')),'expire'=>array(3,Uk86Language::uk86_get('voucher_quotastate_expire')));
		//卡券包模板状态
		$this->templatestate_arr = array('usable'=>array(1,Uk86Language::uk86_get('voucher_templatestate_usable')),'disabled'=>array(2,Uk86Language::uk86_get('voucher_templatestate_disabled')));
		...
		if(is_array($list)){
			foreach ($list as $key=>$val){
				if (!$val['voucher_t_customimg'] || !file_exists(BASE_UPLOAD_PATH.DS.ATTACH_VOUCHER.DS.$_SESSION['store_id'].DS.$val['voucher_t_customimg'])){
					$list[$key]['voucher_t_customimg'] = UPLOAD_SITE_URL.DS.uk86_defaultGoodsImage(60);
				}else{
	...
	public function quota_add_saveOp(){
		...
		if (!preg_match('/^\d{18}$/',$order_sn)){
            uk86_showMessage('该订单不存在','index.php?act=store_promotion_xianshi&op=index','html','error');
			exit;
        }
		...
		$quota_quantity = isset($order['goods_num'])?$order['goods_num']:0; //餐套数量
//		$quota_quantity = intval($_POST['quota_quantity']);
//      if($quota_quantity <= 0 || $quota_quantity > 12) {
//         showDialog(Uk86Language::uk86_get('voucher_apply_num_error'));
//      }
        //获取当前价格
        ...
        //记录店铺费用
        $this->recordStoreCost($current_price * $quota_quantity, '购买卡券包套餐');

        $this->recordSellerLog('购买'.$quota_quantity.'份卡券包套餐，单价'.$current_price.L('nc_yuan'));

        if($reault){
           showDialog(Uk86Language::uk86_get('voucher_apply_buy_succ'),'index.php?act=store_voucher&op=quotalist','succ');
        } else {
           showDialog(Uk86Language::uk86_get('nc_common_op_fail'),'index.php?act=store_voucher&op=quotalist');
        }
	}
    ...
    public function templateaddOp(){
        ...
        if ($isOwnShop = uk86_checkPlatformStore()) {
            ...
            if ($count >= C('promotion_voucher_storetimes_limit')){
                $message = sprintf(Uk86Language::uk86_get('voucher_template_noresidual'),C('promotion_voucher_storetimes_limit'));
                uk86_showMessage($message,'index.php?act=store_voucher&op=templatelist','html','error');
            }
        }
        ...
        if(uk86_chksubmit()){
	        ...
	        $obj_validate->validateparam = array(
	            array("input"=>$_POST['txt_template_title'], "require"=>"true","validator"=>"Length","min"=>"1","max"=>"50","message"=>Uk86Language::uk86_get('voucher_template_title_error')),
	            array("input"=>$_POST['txt_template_total'], "require"=>"true","validator"=>"Number","message"=>Uk86Language::uk86_get('voucher_template_total_error')),
	            array("input"=>$_POST['select_template_price'], "require"=>"true","validator"=>"Number","message"=>Uk86Language::uk86_get('voucher_template_price_error')),
	            array("input"=>$_POST['txt_template_limit'], "require"=>"true","validator"=>"Double","message"=>Uk86Language::uk86_get('voucher_template_limit_error')),
	            array("input"=>$_POST['txt_template_describe'], "require"=>"true","validator"=>"Length","min"=>"1","max"=>"255","message"=>Uk86Language::uk86_get('voucher_template_describe_error')),
	        );
	        ...
	        if($price>=$limit) $error.=Uk86Language::uk86_get('voucher_template_price_error');
	        if ($error){
	           showDialog($error,'reload','error');
	        	...
	        	$insert_arr['voucher_t_start_date'] = time();//默认卡券包模板的有效期为当前时间
	            ...
	            if($rs){
	               showDialog(Uk86Language::uk86_get('nc_common_save_succ'),'index.php?act=store_voucher&op=templatelist','succ');
	            }else{
	               showDialog(Uk86Language::uk86_get('nc_common_save_fail'),'index.php?act=store_voucher&op=templatelist','error');
	            }
	        }
        }else{
            //店铺分类
    ...
    public function templatedelOp(){
        ...
        $param['voucher_t_id'] = $t_id;
        $param['voucher_t_store_id'] = $_SESSION['store_id'];
        $param['voucher_t_giveout'] = array('elt','0');//会员没领取过卡券包才可删除
        $t_info = $model->table('voucher_template')->where($param)->find();
    	if (empty($t_info)){
    		uk86_showMessage(Uk86Language::uk86_get('wrong_argument'),'index.php?act=store_voucher&op=templatelist','html','error');
    	}
        ...
        if ($rs){
        	...
        	showDialog(Uk86Language::uk86_get('nc_common_del_succ'),'reload','succ');
        }else {
	...
	private function profile_menu($menu_type,$menu_key='') {
		...
		switch ($menu_type) {
			case 'voucher':
				$menu_array = array(
				1=>array('menu_key'=>'templatelist','menu_name'=>Uk86Language::uk86_get('nc_member_path_store_voucher'), 'menu_url'=>'index.php?act=store_voucher&op=templatelist'),
				);
				break;
			case 'quota_add':
				$menu_array = array(
				1=>array('menu_key'=>'templatelist','menu_name'=>Uk86Language::uk86_get('nc_member_path_store_voucher'),	'menu_url'=>'index.php?act=store_voucher&op=templatelist'),
				4=>array('menu_key'=>'quotaadd','menu_name'=>Uk86Language::uk86_get('voucher_applyadd'),	'menu_url'=>'index.php?act=store_voucher&op=quotaadd')
				);
				break;
			case 'template':
				$menu_array = array(
				1=>array('menu_key'=>'templatelist','menu_name'=>Uk86Language::uk86_get('nc_member_path_store_voucher'),	'menu_url'=>'index.php?act=store_voucher&op=templatelist'),
				2=>array('menu_key'=>'templateadd','menu_name'=>Uk86Language::uk86_get('voucher_templateadd'),	'menu_url'=>'index.php?act=store_voucher&op=templateadd'),
				);
				break;
			case 'templateedit':
				$menu_array = array(
				1=>array('menu_key'=>'templatelist','menu_name'=>Uk86Language::uk86_get('nc_member_path_store_voucher'),	'menu_url'=>'index.php?act=store_voucher&op=templatelist'),
				2=>array('menu_key'=>'templateedit','menu_name'=>Uk86Language::uk86_get('voucher_templateedit'),	'menu_url'=>''),
				);
				break;
			case 'templateinfo':
				$menu_array = array(
				1=>array('menu_key'=>'templatelist','menu_name'=>Uk86Language::uk86_get('nc_member_path_store_voucher'),	'menu_url'=>'index.php?act=store_voucher&op=templatelist'),
				2=>array('menu_key'=>'templateinfo','menu_name'=>Uk86Language::uk86_get('voucher_templateinfo'), 'menu_url'=>''),
				);
				break;
		}
