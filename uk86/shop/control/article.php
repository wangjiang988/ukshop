...
class articleControl extends BaseHomeControl {
	...
	public function indexOp(){
		...
		uk86_showMessage(Uk86Language::uk86_get('article_article_not_found'),'','html','error');//'没有符合条件的文章'
	}
	/**
	 * 文章列表显示页面
	 */
	public function articleOp(){
		...
		if(empty($_GET['ac_id'])){
			uk86_showMessage($lang['para_error'],'','html','error');//'缺少参数:文章类别编号'
		}
		/**
		 * 得到导航ID
		 */
		...
		if(empty($article_class) || !is_array($article_class)){
			uk86_showMessage($lang['article_article_class_not_exists'],'','html','error');//'该文章分类并不存在'
		}
	...
	public function showOp(){
		...
		$lang	= Uk86Language::uk86_getLangContent();
		if(empty($_GET['article_id'])){
			uk86_showMessage($lang['para_error'],'','html','error');//'缺少参数:文章编号'
		}
		/**
		 * 根据文章编号获取文章信息
		 */
		...
		if(empty($article) || !is_array($article) || $article['article_show']=='0'){
			uk86_showMessage($lang['article_show_not_exists'],'','html','error');//'该文章并不存在'
		}
		...
		if(empty($article_class) || !is_array($article_class)){
			uk86_showMessage($lang['article_show_delete'],'','html','error');//'该文章已随所属类别被删除'
		}
		$default_count	= 5;//定义最新文章列表显示文章的数量
		/**
		 * 分类导航
		...
		$nav_link = array(
			...
			array(
				'title'=>$article_class['ac_name'],
			    'link' => uk86_urlShop('article', 'article', array('ac_id' => $article_class['ac_id']))
			),
			array(
