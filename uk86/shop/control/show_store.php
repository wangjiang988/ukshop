...
class show_storeControl extends BaseStoreControl {
	...
	public function indexOp(){
        if(!$this->store_decoration_only) {
			...
			$floors=$floor_model->table('store_floor')->where(array('store_id'=>$condition['store_id'],'is_hidden'=>0))->select();

			foreach($floors as $key => $item){
				$where['store_floor_id']=$item['id'];
				$goods_list=$model_goods->getGoodsListByColorDistinct($where, $fieldstr, 'goods_id desc', 12);
				$floors[$key]['goods_list']=$goods_list;
				unset($goods_list);
			}
			Tpl::output("floors_goods_list",$floors);
			//var_dump($floors);
            //幻灯片图片
