...
class member_snshomeControl extends BaseSNSControl {
	...
	public function __construct(){
		parent::__construct();
		Uk86Language::uk86_read('member_sns,sns_home');
		$where = array();
	...
	public function indexOp(){
		...
		if (!empty($storelist)){
			...
			if (!empty($goodslist)){
				foreach ($goodslist as $k=>$v){
					$v['goodsurl'] = uk86_urlShop('goods', 'index', array('goods_id'=>$v['goods_id']));
	...
	public function goodsinfoOp(){
		...
		if ($share_id <= 0){
			showDialog(Uk86Language::uk86_get('wrong_argument'),"index.php?act=member_snshome&mid={$this->master_id}",'error');
		}
		//查询分享和喜欢商品信息
		$sharegoods_model = Model('sns_sharegoods');
		$condition = array();
		...
		$sharegoods_info['snsgoods_goodsurl'] = uk86_urlShop('goods', 'index', array('goods_id'=>$sharegoods_info['snsgoods_goodsid']));
	...
	public function image_uploadOp(){
		...
		if($ap_id > 0){
			...
			if(!empty($pic_info)) $upload->uk86_set('file_name',$pic_info['ap_cover']);		// 原图存在设置图片名称为原图名称
		}
