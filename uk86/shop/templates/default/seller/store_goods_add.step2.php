...
<div class="item-publish">
  <form method="post" id="goods_form" action="<?php if ($output['edit_goods_sign']) { echo uk86_urlShop('store_goods_online', 'edit_save_goods');} else { echo uk86_urlShop('store_goods_add', 'save_goods');}?>">
    ...
    <input type="hidden" name="ref_url" value="<?php echo $_GET['ref_url'] ? $_GET['ref_url'] : uk86_getReferer();?>" />
    <div class="ncsc-form-goods">
      ...
      <dl>
        ...
        <dd id="gcategory"> <?php echo $output['goods_class']['gc_tag_name'];?> <a class="ncsc-btn" href="<?php if ($output['edit_goods_sign']) { echo uk86_urlShop('store_goods_online', 'edit_class', array('commonid' => $output['goods']['goods_commonid'], 'ref_url' => uk86_getReferer())); } else { echo uk86_urlShop('store_goods_add', 'add_step_one'); }?>"><?php echo $lang['nc_edit'];?></a>
      ...
      <dl nc_type="spec_group_dl_<?php echo $i;?>" nctype="spec_group_dl" class="spec-bg" <?php if($k == '1'){?>spec_img="t"<?php }?>>
        ...
        <dd <?php if($k == '1'){?>nctype="sp_group_val"<?php }?>>
          <ul class="spec">
            ...
            <?php }?>
            <?php }?>
            <li data-param="{gc_id:<?php echo $output['goods_class']['gc_id'];?>,sp_id:<?php echo $k;?>,url:'<?php echo uk86_urlShop('store_goods_add', 'ajax_add_spec');?>'}">
        ...
        <dd>
          <div class="ncsc-goods-default-pic">
            <div class="goodspic-uplaod">
              <div class="upload-thumb"> <img nctype="goods_image" src="<?php echo uk86_thumb($output['goods'], 240);?>"/> </div>
              ...
              <div class="handle">
                ...
                <a class="ncsc-btn mt5" nctype="show_image" href="<?php echo uk86_urlShop('store_album', 'pic_list', array('item'=>'goods'));?>"><i class="icon-picture"></i>从图片空间选择</a> <a href="javascript:void(0);" nctype="del_goods_demo" class="ncsc-btn mt5" style="display: none;"><i class="icon-circle-arrow-up"></i>关闭相册</a></div>
            </div>
          </div>
          <div id="demo"></div>
        </dd>
        ...
        <dd>
          <div class="ncsc-brand-select">
            ...
            <div class="ncsc-brand-select-container">
              <div class="brand-index" data-tid="<?php echo $output['goods_class']['type_id'];?>" data-url="<?php echo uk86_urlShop('store_goods_add', 'ajax_get_brand');?>">
        ...
        <dd id="ncProductDetails">
          <div class="tabs">
            ...
            <div id="panel-1" class="ui-tabs-panel" jquery1239647486215="4">
              <?php uk86_showEditor('g_body',$output['goods']['goods_body'],'100%','480px','visibility:hidden;',"false",$output['editor_multimedia']);?>
              <div class="hr8">
                <div class="ncsc-upload-btn"> <a href="javascript:void(0);"><span>
      ...
      <dl class="special-02" nctype="fcode_valid">
        <dt>
            <a class="ncsc-btn-mini ncsc-btn-red" href="<?php echo uk86_urlShop('store_goods_online', 'download_f_code_excel', array('commonid' => $val['goods_commonid']));?>">下载F码</a>&nbsp;&nbsp;F码<?php echo $lang['nc_colon'];?></dt>
        <dd>
          <ul class="ncsc-form-radio-list">
      ...
      <dl>
        ...
        <dd>
        	<ul class="ncsc-form-radio-list">
        		<li>
        			<label><input type="radio" name="g_mentioning" value="1" id="g_mentioning" <?php if($output['goods']['store_mentioning'] == 1){ echo 'checked="checked"'; } ?>><?php echo $lang['nc_yes'];?></label>
        		</li>
        		<li>
        			<label><input type="radio" name="g_mentioning" value="0" <?php if($output['goods']['store_mentioning'] == 0){ echo 'checked="checked"'; } ?>><?php echo $lang['nc_no'];?></label>
        		</li>
        	</ul>
        </dd>
      </dl>
      ...
      <?php if(!empty($output['store_floors'])){?>
      <dl>
        <dt>店铺楼层</dt>
        <dd>
            <select name="store_floor_id">
              <option value="0">请选择楼层</option>
              <?php foreach($output['store_floors'] as $key=>$item){?>
                <option value="<?php echo $item['id'];?>" <?php if($item['id']==$output['goods']['store_floor_id']){echo "selected";};?>><?php echo $item['floor_name'];?></option>
              <?php }?>
            </select>
            <input type="hidden" name="store_floor_name" value="<?php echo $output['goods']['store_floor_name'];?>"/>
          <p class="hint">选中后商品将放到店铺首页相应楼层成，不选同时不被推荐的商品不放在首页</p>
        </dd>
      </dl>
      <?php }?>
    </div>
...
var DEFAULT_GOODS_IMAGE = "<?php echo uk86_thumb(array(), 60);?>";
...
$(function(){
    ...
    $('select[name="store_floor_id"]').on('change',function(){
        var floor_name=$(this).find("option:checked").text();
        console.log(floor_name);
        var id=$(this).find("option:checked").val();
        if(id!=0){
          $("input[name=store_floor_name]").val(floor_name);
        }else{
          $("input[name=store_floor_name]").val("");
        }
...
<script type="text/javascript">
	...
	$(function(){
// 		$('#g_mentioning').checke(function(){
// 			alert(1);
// 		});//.attr('checked')
		$('input[name=g_mentioning]').change(function(){
			if($(this).val() == 1){
				$.post('index.php?act=store_goods_add&op=store_info_address', {isuse:1}, function(msg){
					if(msg != 1){
						var yes = confirm('您的线下店铺信息没有填写完整\n请前往“店铺 -> 线下商铺”吧店铺信息填写完整\n是否立即前往');
						if(yes){
							window.location.href="index.php?act=store_live&op=store_live";
						}else{
							$('input[name=g_mentioning][value=1]').removeAttr('checked');
							$('input[name=g_mentioning][value=0]').attr('checked', 'checked');
						}
					}
				});
			}
