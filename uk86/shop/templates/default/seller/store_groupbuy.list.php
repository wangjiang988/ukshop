...
<?php if ($output['isOwnShop']) { ?>
  <a href="<?php echo uk86_urlShop('store_groupbuy', 'groupbuy_add_vr'); ?>" style="right:100px" class="ncsc-btn ncsc-btn-green" title="新增虚拟商品抢购"><i class="icon-plus-sign"></i>新增虚拟抢购</a>
  <a href="<?php echo uk86_urlShop('store_groupbuy', 'groupbuy_add');?>" class="ncsc-btn ncsc-btn-green" title="<?php echo $lang['groupbuy_index_new_group'];?>"><i class="icon-plus-sign"></i><?php echo $lang['groupbuy_index_new_group'];?></a>
<?php } else { ?>

  <?php if(!empty($output['current_groupbuy_quota'])) { ?>
  <a href="<?php echo uk86_urlShop('store_groupbuy', 'groupbuy_add_vr'); ?>" style="right:200px" class="ncsc-btn ncsc-btn-green" title="新增虚拟商品抢购"><i class="icon-plus-sign"></i>新增虚拟抢购</a>
  <a href="<?php echo uk86_urlShop('store_groupbuy', 'groupbuy_add');?>" style="right:100px" class="ncsc-btn ncsc-btn-green" title="<?php echo $lang['groupbuy_index_new_group'];?>"><i class="icon-plus-sign"></i><?php echo $lang['groupbuy_index_new_group'];?></a>
  <a class="ncsc-btn ncsc-btn-acidblue" href="<?php echo uk86_urlShop('store_groupbuy', 'groupbuy_quota_add');?>" title="套餐续费"><i class="icon-money"></i>套餐续费</a>
  <?php } else { ?>
  <a class="ncsc-btn ncsc-btn-acidblue" href="<?php echo uk86_urlShop('store_groupbuy', 'groupbuy_quota_add');?>" title="购买套餐"><i class="icon-money"></i>购买套餐</a>
  <?php } ?>
<?php } ?>
...
<table class="ncsc-default-table">
  ...
  <tbody>
    ...
    <tr class="bd-line">
      <td></td>
      <td><div class="pic-thumb"><a href="<?php echo $group['groupbuy_url'];?>" target="_blank"><img src="<?php echo uk86_gthumb($group['groupbuy_image'], 'small');?>"/></a></div></td>
      <td class="tl">
        <dl class="goods-name">
          <dt>
<?php if ($group['is_vr']) { ?>
...
<?php } ?>
      ...
      <td><?php if($group['state'] != 30){echo $group['groupbuy_state_text'];}else{echo "申请失败，理由是".$group['reject_reason'].'，请<a href="'.uk86_urlShop('store_groupbuy', 'groupbuy_edit').'&groupbuy_id='.$group['groupbuy_id'].'">重新申请</a>';}?></td>
    </tr>
    <?php }?>
    <?php }else{?>
    <tr>
