...
<div class="ncsc-oredr-show">
        ...
        <dl>
          ...
          <dd><?php echo $output['order_info']['order_sn'];?><a href="javascript:void(0);">更多<i class="icon-angle-down"></i>
            <div class="more"><span class="arrow"></span>
              <ul>
                <li>支付方式：<?php echo uk86_orderPaymentName($output['order_info']['payment_code']);?></li>
    ...
    <div class="code-list tip" title="如列表过长超出显示区域时可滚动鼠标进行查看"><i class="arrow"></i>
      ...
      <div id="codeList">
        <ul>
          ...
          <li class="<?php echo $code_info['vr_state'] == 1 ? 'used' : null;?>"><strong><?php echo $code_info['vr_state'] == '0' ? ($code_info['vr_indate'] < TIMESTAMP ? $code_info['vr_code'] : uk86_encryptShow($code_info['vr_code'],7,12)) : $code_info['vr_code'];?></strong> <?php echo $code_info['vr_code_desc'];?> </li>
          <?php } ?>
        </ul>
      </div>
    </div>
  ...
  <div class="ncsc-order-contnet">
    <table class="ncsc-default-table order">
      ...
      <tbody>
        <tr>
          <th colspan="20"><span class="ml10" title="虚拟订单号">虚拟订单号：<?php echo $output['order_info']['order_sn'];?></span><span>下单时间：<?php echo date("Y-m-d H:i",$output['order_info']['add_time']);?></span><span><a href="<?php echo uk86_urlShop('show_store','index',array('store_id'=>$output['order_info']['store_id']));?>" title="<?php echo $output['order_info']['store_name'];?>"><?php echo $output['store']['store_name'];?></a></span></th>
        </tr>
        <tr>
          <td class="bdl"></td>
          <td><div class="pic-thumb"><a href="<?php echo uk86_urlShop('goods','index',array('goods_id' => $output['order_info']['goods_id']));?>" target="_blank" onMouseOver="toolTip('<img src=<?php echo uk86_thumb($output['order_info'], 240);?>>')" onMouseOut="toolTip()"/><img src="<?php echo uk86_thumb($output['order_info'], 60);?>"/></a></div></td>
          ...
          <?php echo uk86_ncPriceFormat($output['order_info']['order_amount']);?> * <?php echo $output['order_info']['commis_rate'];?>% = <?php echo uk86_ncPriceFormat($output['order_info']['order_amount'] * $output['order_info']['commis_rate']/100);?>
          <?php } ?>
          </td>
      ...
      <tfoot>
        ...
        <tr>
          <th colspan="20"><dl class="ncsc-store-sales">
              <?php if(!empty($output['order_info']['voucher_code'])){ ?>
              <dd><span>&emsp;使用了 <strong><?php echo $output['order_info']['voucher_price'];?></strong> 元 卡券包（编码：<?php echo $output['order_info']['voucher_code'];?>）</span></dd>
              <?php } ?>
            </dl>
          </th>
        </tr>
