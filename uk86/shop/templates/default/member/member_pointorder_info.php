  ...
  <div class="ncm-order-contnet">
    <table class="ncm-default-table order">
      ...
      <tbody>
        ...
        <tr>
          <td></td>
          <td class="w50"><div class="pic-thumb"><a href="<?php echo uk86_urlShop('pointprod', 'pinfo', array('id' => $val['point_goodsid']));?>" target="_blank"> <img src="<?php echo $val['point_goodsimage_small'];?>" onMouseOver="toolTip('<img src=<?php echo $val['point_goodsimage'];?> />')" onMouseOut="toolTip()" /></a></div></td>
          <td class="tl"><dl class="goods-name">
              <dt><a href="<?php echo uk86_urlShop('pointprod', 'pinfo', array('id' => $val['point_goodsid']));?>" target="_blank"><?php echo $val['point_goodsname']; ?></a></dt>
            </dl></td>
          ...
          <p><a href="javascript:void(0)" class="ncm-btn" onclick="ajax_get_confirm('<?php echo $lang['member_pointorder_confirmreceivingtip']; ?>','index.php?act=member_pointorder&op=receiving_order&order_id=<?php echo $output['order_info']['point_orderid']; ?>');" class="" ><?php echo $lang['member_pointorder_confirmreceiving']; ?></a></p>
          <?php } ?>
          </td>
          <?php } ?>
        </tr>
...
$(function(){
    $('#show_shipping').on('hover',function(){
    	...
    	$.getJSON('index.php?act=member_order&op=get_express&e_code=<?php echo $output['express_info']['e_code']?>&shipping_code=<?php echo $output['order_info']['point_shippingcode']?>&t=<?php echo uk86_random(7);?>',function(data){
    		if(data){
