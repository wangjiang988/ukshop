...
<style type="text/css">
.order-code{display:block; position:relative; color:#0279b9; cursor:pointer;}
.order-code img{position:absolute; top:30px; left:-40px; display:none; z-index:5;}
.order-code:hover{color:red;}
.order-code:hover img{display:block;}
</style>
<div class="wrap">
  <div class="tabmenu">
  ...
  <table class="ncm-default-table order">
    ...
    <tbody order_id="" <?php if (!empty($group_info['pay_amount']) && $p == 0) {?> class="pay" <?php }?>>
      ...
      <tr>
        <td colspan="19" class="pay-td"><span class="ml15">在线支付金额：<em>￥<?php echo uk86_ncPriceFormat($group_info['pay_amount']);?></em></span> </td>
      </tr>
      <?php }?>
      <?php $p++;?>
      <tr>
        <th colspan="19"> <span class="ml10">
          ...
          <!-- 订单二维码 -->
          <span class="order-code">二维码<img src="<?php echo UPLOAD_SITE_URL.DS.'shop/mention_code/'.$order_info['order_code'] ?>" /></span>
          <!-- 订单二维码 -->
          <!-- order_time -->
          <span><?php echo $lang['member_order_time'].$lang['nc_colon'];?><?php echo date("Y-m-d H:i:s",$order_info['add_time']); ?></span>

          <!-- store_name -->
          <span><a href="<?php echo uk86_urlShop('show_store','index',array('store_id'=>$order_info['store_id']), $order_info['extend_store']['store_domain']);?>" title="<?php echo $order_info['store_name'];?>"><?php echo $order_info['store_name']; ?></a></span>
      ...
      <!-- S 商品列表 -->
      <?php $i = 0;?>
      <?php foreach ((array)$order_info['goods_list'] as $k => $goods_info) {?>
      <?php $i++;?>
      <tr>
        <td class="bdl"></td>
        ...
        <td class="bdl" rowspan="<?php echo $order_info['goods_count'];?>"><p class=""><strong><?php echo $order_info['order_amount']; ?></strong></p>
          <p class="goods-freight">
            ...
            <?php }else{?>
            	<?php if($order_info['store_mentioning'] == 1) {?>
            		<?php echo '( 店铺自提 )'; ?>
            	<?php }else{ ?>
        ...
        <td class="bdl" rowspan="<?php echo $order_info['goods_count'];?>"><p><?php if ($order_info['if_lock']) {echo "退款审核中";}else{echo $order_info['state_desc'];} ?> <?php echo $order_info['evaluation_status'] ? $lang['member_order_evaluated'] : '';?></p>
        ...
        <td class="bdl bdr" rowspan="<?php echo $order_info['goods_count'];?>"><!-- 永久删除 -->
          ...
          <ul>
          <li><p><a style="margin-right:27px !important; width:67px;" class="ncm-btn ncm-btn-orange fr mr15" href="index.php?act=buy&op=pay&pay_sn=<?php echo $order_pay_sn; ?>"><i class="icon-shield"></i>订单支付</a></p>
    ...
    <tbody>
      <tr>
        <td colspan="20" class="norecord"><div class="warning-option"><i>&nbsp;</i><span>你还没有购买商品哦，先去逛逛吧！</span></div></td>
      </tr>
    </tbody>
    <?php } ?>
