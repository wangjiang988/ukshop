...
<div class="ncm-oredr-show">
  <div class="ncm-order-info">
      ...
      <div class="content">
        <dl>
          ...
          <dd><?php echo $output['order_info']['buyer_phone'];?>
            ...
            <a href="javascript:void(0);" class="ncm-btn-mini ncm-btn-orange" dialog_id="vr_code_resend" dialog_title="发送电子兑换码" dialog_width="480" nc_type="dialog" uri="<?php echo uk86_urlShop('member_vr_order', 'resend',array('buyer_phone'=>$output['order_info']['buyer_phone'],'order_id'=>$output['order_info']['order_id']));?>"><i class="icon-mobile-phone"></i>重新发送</a>
            <?php } ?>
          </dd>
        </dl>
        <dl class="line">
          ...
          <dd><?php echo $output['order_info']['order_sn'];?><a href="javascript:void(0);">更多<i class="icon-angle-down"></i>
            <div class="more"><span class="arrow"></span>
              <ul>
                <li>支付方式：<span><?php echo uk86_orderPaymentName($output['order_info']['payment_code']);?></span></li>
    ...
    <div class="ncm-order-condition">
      ...
      <ul>
        ...
        <li>2. 去商城看看有没有新的<a href="<?php echo uk86_urlShop('search','index');?>" class="ncm-btn-mini ncm-btn-green" target="_blank">虚拟商品</a>。</li>
      </ul>
    </div>
    <?php }?>
    <div class="mall-msg">有疑问可咨询<a href="javascript:void(0);" onclick="ajax_form('mall_consult', '平台客服', '<?php echo uk86_urlShop('member_mallconsult', 'add_mallconsult', array('inajax' => 1));?>', 640);"><i class="icon-comments-alt"></i>平台客服</a></div>
  </div>
  <?php if ( $output['order_info']['order_state'] != ORDER_STATE_CANCEL){ ?>
  ...
  <div class="ncm-order-contnet" id="order-list">
    <table class="ncm-default-table order">
      ...
      <tbody>
        <tr>
          <th colspan="20"><span class="ml10" title="虚拟订单号">虚拟订单号：<?php echo $output['order_info']['order_sn'];?></span><span>下单时间：<?php echo date("Y-m-d H:i",$output['order_info']['add_time']);?></span><span><a href="<?php echo uk86_urlShop('show_store','index',array('store_id'=>$output['order_info']['store_id']));?>" title="<?php echo $output['order_info']['store_name'];?>"><?php echo $output['store_info']['store_name'];?></a></span>
        ...
        <tr>
          <td class="bdl"></td>
          <td class="w50"><div class="pic-thumb"><a href="<?php echo uk86_urlShop('goods','index',array('goods_id' => $output['order_info']['goods_id']));?>" target="_blank" onMouseOver="toolTip('<img src=<?php echo uk86_thumb($output['order_info'], 240);?>>')" onMouseOut="toolTip()"/><img src="<?php echo uk86_thumb($output['order_info'], 60);?>"/></a></div></td>
          <td class="tl"><dl class="goods-name">
              <dt><a href="<?php echo uk86_urlShop('goods','index',array('goods_id'=>$output['order_info']['goods_id']));?>" target="_blank" title="<?php echo $output['order_info']['goods_name'];?>"><?php echo $output['order_info']['goods_name'];?></a></dt>
      ...
      <tfoot>
        ...
        <tr>
          <th colspan="20"><dl class="ncm-store-sales">
              <?php if(!empty($output['order_info']['voucher_code'])){ ?>
              <dd><span>使用了 <strong><?php echo $output['order_info']['voucher_price'];?></strong> 元 卡券包（编码：<?php echo $output['order_info']['voucher_code'];?>）</span></dd>
              <?php } ?>
            </dl>
          </th>
        </tr>
