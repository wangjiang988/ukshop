...
<div class="ncm-oredr-show">
  <div class="ncm-order-info">
    <div class="ncm-order-details">
      ...
      <div class="content">
        <dl>
          <?php if($output['order_info']['store_mentioning'] == 1){ ?>
          	<dt><?php echo '店铺信息'.$lang['nc_colon'];?></dt>
          	<dd><span><?php echo $output['order_info']['extend_store']['live_store_name'] ?></span>，<span><?php echo $output['order_info']['extend_store']['live_store_address'] ?></span>，<span><?php echo $output['order_info']['extend_store']['live_store_tel'] ?></span></dd>
          	<dt>公&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;交：</dt>
          	<dd><span><?php echo $output['order_info']['extend_store']['live_store_bus'] ?></span></dd>
          <?php }else{ ?>
    ...
    <div class="ncm-order-condition">
      <dl>
        ...
        <dd>
          ...
          订单已提交，等待卖家发货
          <?php } else if ($output['order_info']['if_lock']) { ?>
          退款审核中
          <?php } else { ?>
          已支付成功
          <?php } ?>
      ...
      <ul>
        ...
        <?php } else { ?>
        <li>1. 您已使用“<?php echo uk86_orderPaymentName($output['order_info']['payment_code']);?>”方式成功对订单进行支付，支付单号 “<?php echo $output['order_info']['pay_sn'];?>”。</li>
        <li>2. 订单已提交商家进行备货发货准备。</li>
    ...
    <div class="ncm-order-condition">
      ...
      <ul>
        ...
        <li>3. 看一看<a href="<?php echo uk86_urlShop('show_store','index',array('store_id'=>$output['order_info']['store_id']), $output['order_info']['extend_store']['store_domain']);?>" class="ncm-btn-mini">该店铺</a>中有什么新商品上架。</li>
      </ul>
    </div>
    <?php } else { ?>
    <div class="ncm-order-condition">
    ...
    <table class="ncm-default-table order">
      ...
      <tbody>
        ...
        <tr class="bd-line">
          ...
          <td class="bdl bdr" rowspan="<?php echo $output['order_info']['goods_count'];?>"><?php if($output['order_info']['if_lock']){echo "退款审核中";}else{echo $output['order_info']['state_desc'];} ?></td>
      ...
      <tfoot>
        ...
        <tr>
          <th colspan="20"><dl class="ncm-store-sales">
              ...
              <dd>使用了面额为 <strong><?php echo $output['order_info']['extend_order_common']['voucher_price'];?></strong> 元的卡券包，编码：<?php echo $output['order_info']['extend_order_common']['voucher_code'];?></span></dd>
              <?php } ?>
            </dl>
          </th>
        </tr>
...
$(function(){
    $('#show_shipping').on('hover',function(){
    	...
    	$.getJSON('index.php?act=member_order&op=get_express&e_code=<?php echo $output['order_info']['express_info']['e_code']?>&shipping_code=<?php echo $output['order_info']['shipping_code']?>&t=<?php echo uk86_random(7);?>',function(data){
    		if(data){
