...
<style type="text/css">
a.btn-this:hover{ background:#bd362f; width:26px !important; height:32px !important; margin-left:-1px !important; border:1px solid #8e2823;}
</style>
<div class="wrap">
  <div class="tabmenu">
  ...
  <table class="ncm-default-table">
    ...
    <tbody>
      ...
      <tr class="bd-line">
        ...
        <td class="w70"><div class="ncm-goods-thumb"><a href="<?php echo uk86_urlShop('show_store', 'index', array('store_id' => $favorites['store']['store_id']), $favorites['store']['store_domain']);?>" ><img src="<?php echo uk86_getStoreLogo($favorites['store']['store_avatar']);?>" onMouseOver="toolTip('<img src=<?php echo uk86_getStoreLogo($favorites['store']['store_avatar']);?>>')" onMouseOut="toolTip()"/></a></div></td>
        <td class="tl"><dl class="goods-name">
            <dt><a href="<?php echo uk86_urlShop('show_store', 'index', array('store_id'=>$favorites['store']['store_id']), $favorites['store']['store_domain'])?>" ><?php echo $favorites['store']['store_name'];?></a> </dt>
        ...
        <td class="ncm-table-handle"><span><a href="javascript:void(0);" class="btn-acidblue" nc_type="sharestore" data-param='{"sid":"<?php echo $favorites['store']['store_id'];?>"}'><i class="icon-share"></i>
          ...
          </a></span> <span><a href="javascript:void(0)" class="btn-this" onclick="ajax_get_confirm('<?php echo $lang['nc_ensure_del'];?>', 'index.php?act=member_favorites&op=delfavorites&type=store&fav_id=<?php echo $favorites['fav_id'];?>');"><i class="icon-trash"></i>
    ...
    <tbody>
      <tr>
        <td colspan="20" class="norecord"><div class="warning-option"><i>&nbsp;</i><span>还未收藏过店铺哦，先去逛逛吧！</span></div></td>
      </tr>
    </tbody>
    <?php }?>
    <?php if(!empty($output['favorites_list']) && is_array($output['favorites_list'])){?>
