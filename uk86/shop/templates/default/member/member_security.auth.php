    ...
    <form method="post" id="auth_form" action="index.php?act=member_security&op=auth">
      ...
      <dl>
        ...
        <dd><p>
          <select name="auth_type" id="auth_type">
            ...
            <option value="mobile">手机 [<?php echo uk86_encryptShow($output['member_info']['member_mobile'],4,4);?>]</option>
            <?php } ?>
            <?php if ($output['member_info']['member_email']) {?>
            <option value="email">邮箱 [<?php echo uk86_encryptShow($output['member_info']['member_email'],4,4);?>]</option>
            <?php } ?>
          </select>
      ...
      <dl>
        ...
        <dd>
         ...
         <img src="<?php echo SHOP_SITE_URL?>/index.php?act=seccode&op=makecode&nchash=<?php echo uk86_getNchash();?>" name="codeimage" border="0" id="codeimage" class="ml5 vm"><a href="javascript:void(0)" class="ml5 blue" onclick="javascript:document.getElementById('codeimage').src='<?php echo SHOP_SITE_URL?>/index.php?act=seccode&op=makecode&nchash=<?php echo uk86_getNchash();?>&t=' + Math.random();">看不清？换张图</a>
