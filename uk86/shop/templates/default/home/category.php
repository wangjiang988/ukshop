...
<div class="nch-container wrapper">
  <div class="nch-all-menu">
    <ul class="tab-bar">
      ...
      <li><a href="<?php echo uk86_urlShop('brand', 'index');?>">全部品牌</a></li>
      <li><a href="<?php echo uk86_urlShop('search', 'index');?>">全部商品</a></li>
    </ul>
  </div>
  <div class="nch-category-all">
    ...
    <ul class="nch-category-container" id="categoryList">
      ...
      <li class="classes">
        <div class="title"><i></i>
        <a href="<?php echo uk86_urlShop('search', 'index', array('cate_id' => $gc_list['gc_id']));?>"><?php echo $gc_list['gc_name'];?></a>
        </div>
        <?php if (!empty($gc_list['class2'])) {?>
        <?php foreach ($gc_list['class2'] as $gc_list2) {?>
        <dl>
          <dt><a href="<?php echo uk86_urlShop('search', 'index', array('cate_id' => $gc_list2['gc_id']));?>"><?php echo $gc_list2['gc_name'];?></a></dt>
          <dd>
            <?php if(!empty($gc_list2['class3'])){?>
            <?php foreach($gc_list2['class3'] as $key=>$gc_list3){?>
            <a href="<?php echo uk86_urlShop('search', 'index', array('cate_id' => $gc_list3['gc_id']));?>"><?php echo $gc_list3['gc_name'];?></a>
            <?php }?>
            <?php }?>
          </dd>
        </dl>
