    ...
    <table border="0" cellpadding="0" cellspacing="0" class="all">
      ...
      <tbody>
        <tr>
          ...
          <td><input id="seller_name" name="seller_name" type="text" value="<?php echo $output['joininfo']['seller_name']; ?>" class="w200"/>
            <span></span>
            <p class="emphasis">此账号为日后登录并管理商家中心时使用，注册后不可修改，请牢记。</p></td>
        </tr>
        <tr>
          <th class="w150"><i>*</i>店铺名称：</th>
          <td><input name="store_name" type="text" value="<?php echo $output['joininfo']['store_name']; ?>" class="w200"/>
            <span></span>
        ...
        <tr>
          ...
          <td><select name="sg_id" id="sg_id">
              ...
              <option value="<?php echo $v['sg_id'];?>" <?php if($output['joininfo']['sg_id']==$v['sg_id']) echo 'selected'; ?> ><?php echo $v['sg_name'];?> (<?php echo $explain;?>)</option>
              <?php } ?>
              <?php } ?>
            </select>
              <input id="sg_name" name="sg_name" value="<?php echo $output['joininfo']['sg_name']; ?>" type="hidden"/>
        ...
        <tr>
          ...
          <td><select name="joinin_year" id="joinin_year">
              <option value="1" <?php if($output['joininfo']['joinin_year']==1) echo 'selected' ?>>1 年</option>
              <option value="2" <?php if($output['joininfo']['joinin_year']==2) echo 'selected' ?>>2 年</option>
            </select></td>
        </tr>
        <tr>
          <th><i>*</i>店铺分类：</th>
          <td><select name="sc_id" id="sc_id">
              ...
              <option value="<?php echo $v['sc_id'];?>" <?php if($output['joininfo']['sc_id']==$v['sc_id']) echo 'selected'; ?> ><?php echo $v['sc_name'];?> (保证金：<?php echo $v['sc_bail'];?> 元)</option>
              <?php } ?>
              <?php } ?>
            </select>
            <span></span>
              <input id="sc_name" name="sc_name" value="<?php echo $output['joininfo']['sc_name']; ?>" type="hidden"/>
        ...
        <tr>
          ...
          <td><a href="###" id="btn_select_category" class="btn">+选择添加类目</a>
            <div id="gcategory" style="display:none;">
              ...
              <input id="store_class" name="store_class" type="hidden" value="<?php if(!empty($output['joininfo']['store_class_ids'])) echo 1;?>"/>
            <span></span></td>
        </tr>
        <tr>
          <td colspan="2"><table border="0" cellpadding="0" cellspacing="0" id="table_category" class="type">
              <thead>
                <tr>
                  ...
                  <tbody>
                  <?php $store_class_names = unserialize($output['joininfo']['store_class_names']); ?>
                  <?php $store_class_ids = unserialize($output['joininfo']['store_class_ids']); ?>
                  <?php if (!empty($store_class_names) && is_array($store_class_names)) { ?>
                      <?php for ($i = 0, $length = count($store_class_names); $i < $length; $i++) { ?>
                          ...
                          <tr>
                              ...
                              <td><a nctype="btn_drop_category" href="javascript:;">删除</a></td>
                              <input name="store_class_ids[]" type="hidden" value="<?php echo $store_class_ids[$i];?>">
                              <input name="store_class_names[]" type="hidden" value="<?php echo $class1; ?>,<?php echo $class2; ?>,<?php echo $class3; ?>,">
                          </tr>
                      <?php } ?>
                  <?php } ?>
                  </tbody>
...
$(document).ready(function(){
    ...
    jQuery.validator.addMethod("seller_name_exist", function(value, element, params) {
        ...
        $.ajax({
            type:"GET",
            url:'<?php echo uk86_urlShop('store_joinin', 'check_seller_name_exist');?>',
            async:false,
    ...
    $('#form_store_info').validate({
        ...
        rules : {
            ...
            store_name: {
                required: true,
                maxlength: 50,
                remote : '<?php echo uk86_urlShop('store_joinin', 'checkname');?>'
            },
            sg_id: {
                required: true
            },
