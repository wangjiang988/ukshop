...
<div class="addshare">
<div class="goods-pic"><span class="thumb size120"><i></i><img src="<?php echo uk86_cthumb($output['sharegoods_info']['snsgoods_goodsimage'], 240,$output['sharegoods_info']['snsgoods_storeid']);?>"/></span></div>
  <ul id="goods_images">
    <?php for($i=0;$i<5;$i++){?>
    <li nc_type="handle_pic" data-param="{'apid':'<?php echo isset($output['pic_list'][$i])?$output['pic_list'][$i]['ap_id']:0;?>'}" >
      <div class="picture"><span class="thumb size60"><i></i> <img src="<?php echo isset($output['pic_list'][$i])?$output['pic_list'][$i]['ap_cover']:UPLOAD_SITE_URL.DS.uk86_defaultGoodsImage(60);?>" onload="javascript:DrawImage(this,60,60);"/> </span></div>
	...
	$('span[nctype="drop_image"]').click(function(){
		...
		$.getJSON('index.php?act=member_snshome&op=del_sharepic', {apid:data_str.apid}, function(data){
			if(data.type == 'false'){
				...
				p_li.find('img:first').attr('src','<?php echo UPLOAD_SITE_URL.DS.uk86_defaultGoodsImage(60)?>');
...
function img_handle(){
    $('*[nc_type="handle_pic"]').each(function(){
    	$(this).unbind();
    	if($(this).find('img:first').attr('src') != '<?php echo UPLOAD_SITE_URL.DS.uk86_defaultGoodsImage(60);?>'){
    		$(this).hover(function(){
