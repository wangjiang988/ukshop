...
if (($ua == '' || preg_match($uachar, $ua)) && !strpos(strtolower($_SERVER['REQUEST_URI']), 'wap')) {
    ...
    if (!empty($config['wap_site_url'])) {
        $url = SHOP_SITE_URL . '/index.php?act=wap_index';
    } else {
        $url = $config['site_url'];
    }
    header('Location:' . $url);
...
<html lang="zh">
<head>
    <meta property="qc:admins" content="175221722066752766637575306"/>
    <meta property="wb:webmaster" content="4d02a4ed22cad8e3"/>
    ...
    <link href="<?php echo SHOP_TEMPLATES_URL; ?>/css/pointprod/register.css" rel="stylesheet" type="text/css">
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            /* var menu = $(this).find("div[cat_menu_id='"+cat_id+"']");
             menu.show();*/
            //$(".category ul.menu").first('li').addClass("hover");
                    ...
                    $(this).hover(
                        function () {
                           ...
                           // menu.css('display','block');
                            ...
                            // console.log("menu　高度"+menu_height);
                            // if (menu_height < 60) menu.height(80);
                            // menu_height = menu.height();
                            // var li_top = $(this).position().top;
                            // console.log("li_top高度"+li_top);
        ...
        $(function () {
            ...
            /*$("img").lazyload({
             placeholder : "
            <?php echo SHOP_SITE_URL;?>/uk86_templates/default/images/grey.gif",
             effect : "fadeIn",
             failurelimit : 10
...
<?php require_once uk86_template('layout/layout_top'); ?><!--<!--右侧登陆聊天购物车bar和顶部-->
<!-- PublicHeadLayout Begin -->
<!-- 顶部广告展开效果-->
<!-- 顶部广告展开效果-->
<div class="header-wrap">
  <header class="public-head-layout wrapper">
    <h1 style="float: left; width:420px;" >
    <div class="header-logo">
       <a href="<?php echo BASE_SITE_URL;?>"><img src="<?php echo UPLOAD_SITE_URL.DS.ATTACH_COMMON.DS.$output['setting_config']['site_logo']; ?>" class="pngFix"></a>
    </div>
    </h1>
	...
	<!--商品和店铺-->
      <ul class="tab">
        <li title="请输入商品关键字" act="search" class="current">商品</li>
        <li title="请输入店铺关键字" act="store_list">店铺</li>
      </ul>
      ...
      <div class="keyword">
            <ul>
                <?php if(is_array($output['hot_search']) && !empty($output['hot_search'])) { foreach($output['hot_search'] as $val) { ?>
                    <li><a href="<?php echo uk86_urlShop('search', 'index', array('keyword' => $val));?>"><?php echo $val; ?></a></li>
                <?php } }?>
            </ul>
        </div>
     </div>
    ...
    <div class="head-user-menu">
      <dl class="my-mall">
        <dt><span class="ico"></span>我的享生活<i class="arrow"></i></dt>
        <dd>
          <div class="sub-title">
            ...
            <a href="<?php echo uk86_urlShop('member', 'home');?>" class="arrow">我的用户中心<i></i></a></div>
          <div class="user-centent-menu">
            <ul>
              ...
              <li><a href="<?php echo SHOP_SITE_URL;?>/index.php?act=member_voucher">卡券包(<span id="member_voucher">0</span>)</a></li>
              <?php } ?>
              <?php if (C('points_isuse') == 1){ ?>
              <li><a href="<?php echo SHOP_SITE_URL;?>/index.php?act=member_points" class="arrow">我的U币<i></i></a></li>
              <?php } ?>
            </ul>
          </div>
      ...
      <dl class="my-cart">
        ...
        <?php } ?>
        <dt><span class="ico"></span>去购物车<i class="arrow"></i></dt>
        <dd>
          <div class="sub-title">
            <h4>最新加入的商品</h4>
          </div>
...
<nav class="public-nav-layout">
    <div class="wrapper">
        <div class="all-category">
            <?php require uk86_template('layout/home_goods_class'); ?><!--  左侧商品分类栏-->
        </div>
        <ul class="site-menu">    <!--分类栏旁边的右侧导航栏，首页抢购，u币中心，专题，店铺，资讯等-->
            <li>
                <a href="<?php echo BASE_SITE_URL; ?>" <?php if ($output['index_sign'] == 'index' && $output['index_sign'] != '0') {
                    echo 'class="current"';
                } ?>><?php echo $lang['nc_index']; ?></a></li>
            <!--   <li><a href="<?php /*echo uk86_urlShop('search', 'local_search');*/ ?>" <?php /*if($output['index_sign'] == 'search_local' && $output['index_sign'] != '0') {echo 'class="current"';} */ ?>>本地生活</a></li>-->
                        ...
                        <li><a
                                ...
                                switch ($nav['nav_type']) {
                                    case '0':
                                        ...
                                        break;
                                    case '1':
                                        echo ' href="' . uk86_urlShop('search', 'index', array('cate_id' => $nav['item_id'])) . '"';
                                        if (isset($_GET['cate_id']) && $_GET['cate_id'] == $nav['item_id']) {
                                            echo ' class="current"';
                                        }
                                        break;
                                    case '2':
                                        echo ' href="' . uk86_urlShop('article', 'article', array('ac_id' => $nav['item_id'])) . '"';
                                        if (isset($_GET['ac_id']) && $_GET['ac_id'] == $nav['item_id']) {
                                            echo ' class="current"';
                                        }
                                        break;
                                    case '3':
                                        echo ' href="' . uk86_urlShop('activity', 'index', array('activity_id' => $nav['item_id'])) . '"';
