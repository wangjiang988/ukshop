...
<!--搜索标题-->
	<div class="weiquan_list_top_tit clearfixd">		
    	<ul>
        	<li nctype="0" class="hovered <?php if($_GET['type'] == 0 || empty($_GET['type'])){echo 'l_top_c'; } ?>">全部</li>
			<li nctype="1" class="hovered <?php if($_GET['type'] == 1){echo 'l_top_c'; } ?>">热门圈子</li>
			<li nctype="2" class="hovered <?php if($_GET['type'] == 2){echo 'l_top_c'; } ?>">推荐圈子</li>
    	</ul>
	</div>
	<!--搜索标题结束-->
...
<!--默认列表-->
	<div class="list_main_all">
	  ...
	  <?php }else{ ?>
	    <div style="color:#999; width:100%; height:83%; text-align:center; background:#FFF;"><span style="margin-top:2.9rem; display:inline-block;">还没有圈子<span></span><br /></div>
  	  <?php } ?>
	</div>
	<!--默认结束-->
</div>
...
$(document).ready(function(){
	...
	$('.weiquan_list_top_tit ul li').click(function(){
		...
		window.location.href="index.php?act=wap_circle&type="+type;
	});
	$('.list_weiquan_box ul').click(function(){
		if(<?php echo $_SESSION['is_login']?$_SESSION['is_login']:'0'; ?> == 0){
			if(confirm('您还没有登录，是否前去登录？')){
