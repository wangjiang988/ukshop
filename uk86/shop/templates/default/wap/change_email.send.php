...
<title>优康_<?php if(empty($_GET['type'])){ ?>安全校验<?php }else{ ?>绑定邮箱<?php } ?></title>
<link rel="stylesheet" href="<?php echo SHOP_TEMPLATES_URL ?>/css/wap/base.css" type="text/css" media="all">
<style type="text/css">
input[type="number"], input[type="text"]{ line-height:0.6rem; height:0.6rem;border:0.02rem solid #ccc; }
...
<div class="header_con">
	...
	<div class="top-hk-div">
		<span><?php if(empty($_GET['type'])){ ?>安全校验<?php }else{ ?>绑定邮箱<?php } ?></span>
	</div>
...
<div id="content" class="p_bottom_b" style="background:#F5F5F5;">
  <div class="bd_div_input">
    <p>绑定<?php if(empty($_GET['type'])){ ?>新<?php } ?>邮箱：</p>
    <input type="text" id="mobile" placeholder="请输入新邮箱地址"/>
...
$(function(){
	...
	$('.bd_div_input a').click(function(){
		...
		$.getJSON('index.php?act=wap_member_change&op=get_email_code', {email:email}, function(data){
			if(data.state){
				showDialog(data.msg);
	...
	$('.bd_submit').click(function(){
		...
		$.getJSON('index.php?act=wap_member_change&op=getEmailCode', {email:email, code:code}, function(data){
			if(data.state){
