...
.free_delivery{width:32px; height:18px; color:#FFF; background:#D93600; border-radius:5px; text-align:center; line-height:18px;}
</style>
  ...
  <div class="ncs-detail<?php if ($output['store_info']['is_own_shop']) echo ' ownshop'; ?>">
    ...
    <div class="ncs-goods-summary">
      ...
      <div class="ncs-plus">
        ...
        <dl class="ncs-freight">
          ...
          <dd id="transport_price">
            ...
            <div class="free_delivery">包邮</div>
            <?php }else{?>
            <!-- 如果买家承担运费 -->
        ...
        <dl>
          ...
          <dd class="goods-gift" id="ncsGoodsGift">
            ...
            <ul>
              ...
              <li>
                <div class="goods-gift-thumb"><span><img src="<?php echo uk86_cthumb($val['gift_goodsimage'], '60', $output['goods']['store_id']);?>"></span></div>
                <a href="<?php echo uk86_urlShop('goods', 'index', array('goods_id' => $val['gift_goodsid']));?>" class="goods-gift-name" target="_blank"><?php echo $val['gift_goodsname']?></a><em>x<?php echo $val['gift_amount'];?></em> </li>
              <?php }?>
            </ul>
            <?php }?>
          </dd>
      ...
      <div class="ncs-key">
        ...
        <dl>
          <dt>门店服务：</dt>
          <dd><span style="color:#BA7538; font-size:16px;">门店自提</span><span>&nbsp;&nbsp;·选择有现货的门店下单，可立即提货</span></dd>
        </dl>
        <?php } ?>
        <!-- E 购买数量及库存 -->
      </div>
      ...
      <div class="ncs-saleout">
        <dl>
          ...
          <dd><?php echo $lang['goods_index_is_no_show_message_two_1'];?>&nbsp;<a href="<?php echo uk86_urlShop('show_store', 'index', array('store_id'=>$output['goods']['store_id']), $output['store_info']['store_domain']);?>" class="ncs-btn-mini"><?php echo $lang['goods_index_is_no_show_message_two_2'];?></a>&nbsp;<?php echo $lang['goods_index_is_no_show_message_two_3'];?> </dd>
        </dl>
      </div>
      <?php }?>
      <!--E 商品信息 -->
    ...
    <div style=" position: absolute; z-index: 1; top: -1px; right: -1px;">
      <?php include uk86_template('store/info');?>
    </div>
    <!--E 店铺信息 -->
    <div class="clear"></div>
  </div>
          ...
          <div class="nc-mansong">
            ...
            <dl class="nc-mansong-content">
              ...
              <dd>
                ...
                <span><?php echo $lang['nc_man'];?><em><?php echo uk86_ncPriceFormat($rule['price']);?></em><?php echo $lang['nc_yuan'];?>
                <?php if(!empty($rule['discount'])) { ?>
                ， <?php echo $lang['nc_reduce'];?><i><?php echo uk86_ncPriceFormat($rule['discount']);?></i><?php echo $lang['nc_yuan'];?>
                <?php } ?>
                <?php if(!empty($rule['goods_id'])) { ?>
                ， <?php echo $lang['nc_gift'];?> <a href="<?php echo $rule['goods_url'];?>" title="<?php echo $rule['mansong_goods_name'];?>" target="_blank"> <img src="<?php echo uk86_cthumb($rule['goods_image'], 60);?>" alt="<?php echo $rule['mansong_goods_name'];?>"> </a>&nbsp;。
                <?php } ?>
                </span>
                <?php } ?>
              </dd>
        ...
        <div class="ncs-goods-info-content bd" id="ncGoodsRate">
          <div class="top">
            ...
            <div class="btns"><span>您可对已购商品进行评价</span>
              <p><a href="<?php if ($output['goods']['is_virtual']) { echo uk86_urlShop('member_vr_order', 'index');} else { echo uk86_urlShop('member_order', 'index');}?>" class="ncs-btn ncs-btn-red" target="_blank"><i class="icon-comment-alt"></i>评价商品</a></p>
            </div>
          </div>
          <div class="ncs-goods-title-nav">
            <ul id="comment_tab">
      ...
      <div class="ncs-recommend">
        ...
        <div class="content">
          <ul>
            ...
            <li>
              <dl>
                <dt class="goods-name"><a href="<?php echo uk86_urlShop('goods', 'index', array('goods_id' => $goods_commend['goods_id']));?>" target="_blank" title="<?php echo $goods_commend['goods_jingle'];?>"><?php echo $goods_commend['goods_name'];?><em><?php echo $goods_commend['goods_jingle'];?></em></a></dt>
                <dd class="goods-pic"><a href="<?php echo uk86_urlShop('goods', 'index', array('goods_id' => $goods_commend['goods_id']));?>" target="_blank" title="<?php echo $goods_commend['goods_jingle'];?>"><img src="<?php echo uk86_thumb($goods_commend, 240);?>" alt="<?php echo $goods_commend['goods_name'];?>"/></a></dd>
      ...
      <div class="ncs-sidebar-container">
        ...
        <div class="content">
          <div class="ncs-goods-code">
            <p><img src="<?php echo uk86_goodsQRCode($output['goods']);?>"  title="商品原始地址：<?php echo uk86_urlShop('goods', 'index', array('goods_id'=>$output['goods']['goods_id']));?>"></p>
            <span class="ncs-goods-code-note"><i></i>扫描二维码，手机查看分享</span> </div>
        </div>
      </div>
      <?php include uk86_template('store/callcenter');?>
      <?php include uk86_template('store/left');?>
    </div>
  </div>
</div>
...
$(function(){
    ...
    $('a[nctype="addcart_submit"]').click(function(){
        ...
        $('a[nctype="buynow_submit"]').click(function(){
            if($(this).attr('storetype') == 'store_mention'){
				$('#store_men').val(1);
            }
    ...
    $('a[nctype="arrival_notice"]').click(function(){
        ...
        login_dialog();
        <?php }else{?>
        ajax_form('arrival_notice', '到货通知','<?php echo uk86_urlShop('goods', 'arrival_notice', array('goods_id' => $output['goods']['goods_id']));?>', 350);
        <?php }?>
    });
    <?php }?>
    <?php if (($output['goods']['goods_state'] == 0 || $output['goods']['goods_storage'] <= 0) && $output['goods']['is_appoint'] == 1) {?>
    $('a[nctype="appoint_submit"]').click(function(){
        ...
        login_dialog();
        <?php }else{?>
        ajax_form('arrival_notice', '立即预约', '<?php echo uk86_urlShop('goods', 'arrival_notice', array('goods_id' => $output['goods']['goods_id'], 'type' => 2));?>', 350);
        <?php }?>
    });
    <?php }?>
    //浮动导航  waypoints.js
...
//触及显示缩略图
    ...
    function load_goodseval(type) {
        var url = '<?php echo uk86_urlShop('goods', 'comments', array('goods_id' => $output['goods']['goods_id']));?>';
        url += '&type=' + type;
