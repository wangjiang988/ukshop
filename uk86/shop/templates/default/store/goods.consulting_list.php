    ...
    <div class="ncs-goods-main">
      ...
      <div class="ncs-goods-info-content bd" id="ncGoodsRate">
        ...
        <div class="ncs-goods-title-nav">
          <ul id="comment_tab">
            <li class="<?php if (intval($_GET['ctid']) == 0) {?>current<?php }?>"><a href="<?php echo uk86_urlShop('goods', 'consulting_list', array('goods_id' => $_GET['goods_id']));?>">全部</a></li>
            <?php if (!empty($output['consult_type'])) {?>
            <?php foreach ($output['consult_type'] as $val) {?>
            <li class="<?php if (intval($_GET['ctid']) == $val['ct_id']) {?>current<?php }?>"><a href="<?php echo uk86_urlShop('goods', 'consulting_list', array('goods_id' => $_GET['goods_id'], 'ctid' => $val['ct_id']));?>"><?php echo $val['ct_name'];?></a></li>
            <?php }?>
            <?php }?>
          </ul>
        </div>
          ...
          <div class="ncs-cosult-list">
            <dl class="asker">
              ...
              <dd>
                ...
                <?php echo uk86_str_cut($v['member_name'],2).'***';?>
                <?php }else{?>
                <a href="index.php?act=member_snshome&mid=<?php echo $v['member_id'];?>" target="_blank" data-param="{'id':<?php echo $v['member_id'];?>}" nctype="mcard"><?php echo uk86_str_cut($v['member_name'],8);?></a>
                <?php }?>
        ...
        <div class="ncs-consult-form">
          ...
          <dl>
            ...
            <dd>
              ...
              <div class="code">
                <div class="arrow"></div>
                <div class="code-img"><a href="javascript:void(0)" onclick="javascript:document.getElementById('codeimage').src='index.php?act=seccode&op=makecode&nchash=<?php echo uk86_getNchash();?>&t=' + Math.random();"><img src="index.php?act=seccode&op=makecode&nchash=<?php echo uk86_getNchash();?>" name="codeimage" border="0" id="codeimage" onclick="this.src='index.php?act=seccode&op=makecode&nchash=<?php echo uk86_getNchash();?>&t=' + Math.random()"/></a></div>
                <a href="JavaScript:void(0);" id="hide" class="close" title="关闭"><i></i></a> <a href="JavaScript:void(0);" onclick="javascript:document.getElementById('codeimage').src='index.php?act=seccode&op=makecode&nchash=<?php echo uk86_getNchash();?>&t=' + Math.random();" class="change" title="<?php echo $lang['goods_index_change_checkcode'];?>"><i></i></a> </div>
              <?php } ?>
            		...
            		remote   : {
                        ...
                        complete: function(data) {
                            if(data.responseText == 'false') {
                            	document.getElementById('codeimage').src='index.php?act=seccode&op=makecode&nchash=<?php echo uk86_getNchash();?>&t=' + Math.random();
                            }
                        }
                    }
            	}
