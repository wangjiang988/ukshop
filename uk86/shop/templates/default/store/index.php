...
<div class="wrapper mt10">
  <div class="ncs-main">
    ...
    <div class="ncs-main-container">
      <div class="title"> <span><a href="<?php echo uk86_urlShop('show_store', 'goods_all', array('store_id' => $_GET['store_id']));?>" class="more"><?php echo $lang['nc_more'];?></a></span>
      ...
      <div class="content ncs-goods-list">
        ...
        <ul>
          ...
          <li>
            <dl>
              <dt><a href="<?php echo uk86_urlShop('goods', 'index', array('goods_id'=>$value['goods_id']));?>" class="goods-thumb" target="_blank"><img src="<?php echo uk86_thumb($value, 240);?>" alt="<?php echo $value['goods_name'];?>"/></a>
                <ul class="goods-thumb-scroll-show">
                <?php if (is_array($value['image'])) {?>
                  <?php $i=0;foreach ($value['image'] as $val) {$i++?>
                  <li<?php if($i==1) {?> class="selected"<?php }?>><a href="javascript:void(0);"><img src="<?php echo uk86_thumb($val, 60);?>"/></a></li>
                  <?php }?>
                <?php } else {?>
              ...
              <dd class="goods-name"><a href="<?php echo uk86_urlShop('goods', 'index', array('goods_id'=>$value['goods_id']));?>" title="<?php echo $value['goods_name'];?>" target="_blank"><?php echo $value['goods_name']?></a></dd>
    ...
    <?php if(!empty($output['floors_goods_list'])){?>
        <?php foreach($output['floors_goods_list'] as $key=>$item){?>
      <div class="ncs-main-container">
        <div class="title"><span><a href="<?php echo uk86_urlShop('show_store', 'goods_all', array('store_id' => $_GET['store_id']));?>" class="more"><?php echo $lang['nc_more'];?></a></span>
          <h4><?php echo $item['floor_name'];?></h4>
        </div>
        <div class="content ncs-goods-list">
          <?php if(!empty($item['goods_list']) && is_array($item['goods_list'])){?>
            <ul>
              <?php foreach($item['goods_list'] as $value){?>
                <li>
                  <dl>
                    <dt><a href="<?php echo uk86_urlShop('goods', 'index', array('goods_id'=>$value['goods_id']));?>" class="goods-thumb" target="_blank"><img src="<?php echo uk86_thumb($value, 240)?>" alt="<?php echo $value['goods_name'];?>"/></a>
                    <ul class="goods-thumb-scroll-show">
