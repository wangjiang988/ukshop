  ...
  <div class="ncs-cosult-list">
    <dl class="asker">
	  ...
	  <?php if($v['member_id']== '0') echo $lang['nc_guest']; else if($v['isanonymous'] == 1){?>
        <?php echo uk86_str_cut($v['cmember_name'],2).'***';?>
        <?php }else{?>
        <a href="index.php?act=member_snshome&mid=<?php echo $v['member_id'];?>" target="_blank" data-param="{'id':<?php echo $v['member_id'];?>}" nctype="mcard"><?php echo uk86_str_cut($v['cmember_name'],8);?></a>
        <?php }?>        
  ...
  <div class="ncs-consult-form">
    <div class="asker-info">
      ...
      <label for="captcha"><strong><?php echo $lang['goods_index_checkcode'];?></strong>
        ...
        <img src="index.php?act=seccode&op=makecode&nchash=<?php echo uk86_getNchash();?>" name="codeimage" border="0" id="codeimage" onclick="this.src='index.php?act=seccode&op=makecode&nchash=<?php echo $output['nchash'];?>&t=' + Math.random()"/><span><?php echo $lang['goods_index_change_checkcode'];?></span></label>
      <?php } ?>
    </div>
	    ...
	    if($("#message").valid()){
			$.getJSON('index.php?act=goods&op=save_consultajax',{
				'form_submit':'ok','formhash':$('input[name="formhash"]').val(),
					...
					<?php if($output['setting_config']['captcha_status_goodsqa'] == '1') { ?>
					,'captcha':$("#captcha").val()
					,'nchash':'<?php echo uk86_getNchash();?>'
					<?php }?>
