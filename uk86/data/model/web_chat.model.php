...
class web_chatModel extends Model{
	...
	public function getMemberInfo($condition) {
		...
		$member['store_name'] = '';
		$member['grade_id'] = '';
		$member['member_avatar'] = uk86_getMemberAvatar($member['member_avatar']);
	    $model_seller = Model('seller');
	...
	public function getGoodsInfo($goods_id) {
	    ...
	    if (is_array($goods) && !empty($goods)) {
	        $goods['url'] = uk86_urlShop('goods','index',array('goods_id'=> $goods['goods_id']));
	        $goods['pic'] = uk86_thumb($goods,60);
	    }
	    return $goods;
	}
}