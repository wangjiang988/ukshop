...
class pointsModel {
	 ...
	 * @param  string $stage 操作阶段 regist(注册),login(登录),comments(评论),order(下单),system(系统),other(其他),pointorder(U币礼品兑换),app(同步U币兑换)
	 * @param  array $insertarr 该数组可能包含信息 array('pl_memberid'=>'会员编号','pl_membername'=>'会员名称','pl_adminid'=>'管理员编号','pl_adminname'=>'管理员名称','pl_points'=>'U币','pl_desc'=>'描述','orderprice'=>'订单金额','order_sn'=>'订单编号','order_id'=>'订单序号','point_ordersn'=>'U币兑换订单编号');
	...
	function savePointsLog($stage,$insertarr,$if_repeat = true){
		...
		switch ($stage){
			...
			case 'pointorder':
				if (!$insertarr['pl_desc']){
					$insertarr['pl_desc'] = '兑换礼品信息'.$insertarr['point_ordersn'].'消耗U币';
				}
				break;
            case 'app':
				if (!$insertarr['pl_desc']){
					$insertarr['pl_desc'] = Uk86Language::uk86_get('points_pointorderdesc_app');
				}
				//邀请U币返利 33 hao.co m v3
				break;
			case 'inviter':
				if (!$insertarr['pl_desc']){
