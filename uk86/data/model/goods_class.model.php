...
class goods_classModel extends Model
{
    ...
    public function get_all_category($update_all = 0) {
        ...
        // 不存在时更新或者强制更新时执行
        if ($update_all == 1 || !($gc_list = uk86_rkcache('all_categories'))) {
            ...
            uk86_wkcache('all_categories', $gc_list);
        }
    ...
    public function getGoodsClassNav($id = 0, $sign = 1) {
        if (intval ( $id ) > 0) {
            ...
            if ($sign == 1) {
                ...
                $nav_link [] = array(
                        'title' => $data[$id]['gc_name'],
                        'link' => uk86_urlShop('search', 'index', array('cate_id' => $data[$id]['gc_id']))
                );
            }
            ...
            // 加上 首页 商品分类导航
            $nav_link[] = array('title' => Uk86Language::uk86_get('goods_class_index_search_results'));
        }
        // 首页导航
        $nav_link[] = array('title' => Uk86Language::uk86_get('homepage'), 'link' => SHOP_SITE_URL);
