...
class goods_browseModel extends Model{
    ...
    public function getViewedGoodsList($member_id = 0,$shownum = 0) {
        ...
        if ($member_id > 0){
            if (!C('cache_open')){//查询数据库
                ...
                $hash_key = $member_id;
				//先查找$hash_key缓存
				$browse_goodsid = uk86_rcache($hash_key,'goodsbrowse','goodsid');
				...
				if ($goodsid_arr){
                    ...
                    //获得缓存的浏览商品信息
                    $browselist_tmp = uk86_rcache($hash_key,'goodsbrowse',implode(',', $goodsid_arr));
        ...
        if ($browselist){
            $goods_list_tmp = Model('goods')->getGoodsList(array('goods_id' => array('in', array_keys($browselist))), 'goods_id, goods_commonid, goods_name, goods_promotion_price, goods_promotion_type, goods_marketprice, goods_image, store_id, gc_id, gc_id_1, gc_id_2, gc_id_3');
            $goods_list = array();
            ...
            foreach ((array)$browselist as $k=>$v){
                if ($goods_list[$k]){
                    $browselist_new[] = array(
                    	...
                    	"goods_commonid"	=> $goods_list[$k]['goods_commonid'],
            ...
            $arr_new = @array_reverse($browselist_new, true);
            foreach ($arr_new as $key => $val){
            	foreach ($arr_new as $ke => $va){
            		if($va['goods_commonid'] == $val['goods_commonid'] && $va['goods_id'] != $val['goods_id']){
            			unset($arr_new[$key]);
            		}
            	}
            }
        }
        return @array_reverse($arr_new, true);
    }
    ...
    public function mergebrowse($member_id, $store_id = 0){
        ...
        //最后清空浏览记录cookie
        uk86_setNcCookie('viewed_goods','',-3600);
        return $result;
    }
    /**
     * 浏览过的商品
    ...
    public function addViewedGoodsToCache($goods_id,$member_id,$store_id = 0,$goods_info = array()) {
        ...
        //处理浏览历史cache中商品ID
        $_cache = uk86_rcache($hash_key,'goodsbrowse','goodsid');
        ...
        //缓存商品信息
        uk86_wcache($hash_key,$_cache,'goodsbrowse');
        return array('state'=>true);
    }
    /**
     * 浏览过的商品加入浏览历史数据库
    ...
    public function addViewedGoodsToCookie($goods_id){
        ...
        if (uk86_cookie('viewed_goods')) {//如果cookie已经存在
            $string_viewed_goods = uk86_decrypt(uk86_cookie('viewed_goods'), MD5_KEY);
