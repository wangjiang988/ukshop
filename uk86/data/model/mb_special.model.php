...
class mb_specialModel extends Model{
    ...
    public function getMbSpecialItemUsableListByID($special_id) {
        ...
        $item_list = uk86_rcache($special_id, $prefix);
        //缓存有效
        if(!empty($item_list)) {
        ...
        uk86_wcache($special_id, $cache, $prefix);
        return $item_list;
    }
    ...
    public function addMbSpecialItem($param) {
        ...
        $param['item_sort'] = 255;
		//2015推荐 2016团购 ukshop.com v3-10
    ...
    public function getMbSpecialModuleList() {
        $module_list = array();
        $module_list['nav_list'] = array('name' => 'nav_list' , 'desc' => '首页导航版块');
        $module_list['adv_list'] = array('name' => 'adv_list' , 'desc' => '广告条版块');
        $module_list['class_list'] = array('name' => 'class_list', 'desc' => '推荐分类版块');
...
// 	}
    ...
    private function _delMbSpecialCache($special_id) {
        //清理缓存
        uk86_dcache($special_id, 'mb_special');
