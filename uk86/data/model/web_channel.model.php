...
class web_channelModel extends Model{
    ...
    public function editChannelCityArea($area_id){
        if (empty($area_id)) {
            return false;
        }
        $condition = array();
        $condition['area_id'] = intval($area_id);
    ...
    public function getChannelByCateId($classId){
       $condition['gc_id']=$classId;
       $condition['channel_show']=1;
       $channel=$this->table('web_channel')->where($condition)->find();
       return $channel;
   }
}
