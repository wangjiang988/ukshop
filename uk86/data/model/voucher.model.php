...
class voucherModel extends Model {
	...
	public function getVoucherState(){
	    return array('unused'=>array(1,Uk86Language::uk86_get('voucher_voucher_state_unused')),'used'=>array(2,Uk86Language::uk86_get('voucher_voucher_state_used')),'expire'=>array(3,Uk86Language::uk86_get('voucher_voucher_state_expire')));
	}

	/**
	 * 返回当前可用的卡券包列表,每种类型(模板)的卡券包里取出一个卡券包码(同一个模板所有码面额和到期时间都一样)
	...
	public function getCurrentAvailableVoucherCount($member_id) {
	    $info = uk86_rcache($member_id, 'm_voucher', 'voucher_count');
	    if (empty($info['voucher_count']) && $info['voucher_count'] !== 0) {
			...
			uk86_wcache($member_id, array('voucher_count' => $voucher_count), 'm_voucher');
	    } else {
	...
	public function getCanChangeTemplateInfo($vid,$member_id,$store_id = 0){
	    ...
	    if (empty($template_info) || $template_info['voucher_t_total']<=$template_info['voucher_t_giveout']){//卡券包不存在或者已兑换完
	        return array('state'=>false,'msg'=>'卡券包信息错误');
	    }
	    //验证是否为店铺自己
	    if ($store_id > 0 && $store_id == $template_info['voucher_t_store_id']){
	        return array('state'=>false,'msg'=>'不可以兑换自己店铺的卡券包');
	    }
	    //查询卡券包对应的店铺信息
        ...
        if (!empty($template_info['voucher_t_customimg'])){
            ...
            $template_info['voucher_t_customimg'] = UPLOAD_SITE_URL.DS.uk86_defaultGoodsImage(240);
        }
	    ...
	    if (!empty($voucher_list)){
	        $voucher_count = 0;//在该店铺兑换的卡券包数量
	        $voucherone_count = 0;//该张卡券包兑换的数量
	        ...
	        //买家最多只能拥有同一个店铺尚未消费抵用的店铺卡券包最大数量的验证
	        if ($voucher_count >= intval(C('promotion_voucher_buyertimes_limit'))){
	            $message = sprintf('您的可用卡券包已有%s张,不可再兑换了',C('promotion_voucher_buyertimes_limit'));
	        ...
	        if (!empty($template_info['voucher_t_eachlimit']) && $voucherone_count >= $template_info['voucher_t_eachlimit']){
	            $message = sprintf('该卡券包您已兑换%s次，不可再兑换了',$template_info['voucher_t_eachlimit']);
	            return array('state'=>false,'msg'=>$message);
	        }
	    }
	    //验证会员U币是否足够
	    ...
	    if (intval($member_info['member_points']) < intval($template_info['voucher_t_points'])){
	        return array('state'=>false,'msg'=>'您的U币不足，暂时不能兑换该卡券包');
	    }
    ...
    public function editVoucher($data,$condition,$member_id = 0 ) {
        ...
        if ($result && $member_id > 0){
            uk86_wcache($member_id, array('voucher_count' => null), 'm_voucher');
        }
        return $result;
    }
    ...
    public function exchangeVoucher($template_info, $member_id, $member_name = ''){
        ...
        if ($result){
            ...
            uk86_wcache($member_id, array('voucher_count' => array('exp','voucher_count+1')), 'm_voucher');
