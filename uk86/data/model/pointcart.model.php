...
class pointcartModel extends Model {
	...
	public function addPointCart($param) {
		...
		if($result) {
		    //清除相关缓存
		    uk86_wcache($param['pmember_id'], array('pointcart_count' => null), 'm_pointcart');
			return $result;
		} else {
			return false;
		}
	...
	public function countPointCart($member_id) {
	    $info = uk86_rcache($member_id, 'm_pointcart', 'pointcart_count');
	    if (empty($info['pointcart_count']) && $info['pointcart_count'] !== 0) {
	        ...
	        uk86_wcache($member_id, array('pointcart_count' => $pointcart_count), 'm_pointcart');
	    } else {
	...
	public function delPointCartById($pc_id,$member_id = 0){
	    ...
	    if ($result && $member_id > 0){
	        uk86_wcache($member_id, array('pointcart_count' => null), 'm_pointcart');
	    }
		return $result;
	}
	/**
	...
	/**
	 * 获得符合条件的购物车商品列表同时计算运费、总U币数等信息，用于下单过程
	 */
	...
	public function checkPointEnough($points, $member_id, $member_info = array()){
	    ...
	    if (intval($member_info['member_points']) < $points){
	        return array('state'=>false,'msg'=>'U币不足，暂时不能兑换');
	    }
	    return array('state'=>true);
	}
}