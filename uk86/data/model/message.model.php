...
class messageModel
{
    ...
    public function listMessage($condition, $page = '')
    {
        //得到条件语句
        //  查询用户的创建时间
//        $to_member_id = $condition['to_member_id_common'];
        $to_member_id = $condition['to_member_id'];
        $member_info = Model('member')->getMemberInfoByID($to_member_id, 'member_time');
        $condition['member_create_time'] = $member_info['member_time'];
    ...
    private function getCondition($condition_array)
    {
        ...
        if ($condition_array['to_member_id'] != '') {
            $condition_sql .= " and ((message.to_member_id ='all' and message.message_time > '{$condition_array['member_create_time']}') or (message.message_ismore=0 and message.to_member_id='{$condition_array['to_member_id']}') or (message.message_ismore=1 and message.to_member_id like '%,{$condition_array['to_member_id']},%'))";
        }
        //发信人
