...
class waybillModel extends Model{
	...
	public function getWaybillList($condition, $page='', $order='waybill_usable desc', $field='*') {
        ...
        foreach ($waybill_list as $key => $value) {
            $waybill_list[$key]['waybill_image_url'] = uk86_getWaybillImageUrl($value['waybill_image']);
    ...
    private function _waybill_image_upload() {
        ...
        $upload->uk86_set('default_dir', ATTACH_WAYBILL);
        $upload->uk86_set('allow_type', array('jpg','jpeg','png'));

        $result = $upload->uk86_upfile('waybill_image');
        if($result) {
    ...
    public function validWaybill($input) {
        ...
        return $obj_validate->uk86_validate();
    }
