...
class memberModel extends Model {
    ...
    public function getMemberInfoByID($member_id, $fields = '*') {
        $member_info = uk86_rcache($member_id, 'member', $fields);
        if (empty($member_info)) {
            $member_info = $this->getMemberInfo(array('member_id'=>$member_id),'*',true);
            uk86_wcache($member_id, $member_info, 'member');
        }
        return $member_info;
    }
    ...
    public function editMember($condition, $data) {
        ...
        if ($update && $condition['member_id']) {
            uk86_dcache($condition['member_id'], 'member');
        }
        return $update;
    }
	...
	public function addPoint($member_info) {
	    ...
	    Uk86QueueClient::push('addPoint',$queue_content);
	}
	...
	/**
	 * desc : get wechat basic info
	 * 
	 * @author jason
	 * @since 2015-12-30 上午10:21:07
	 */
	function get_wx_config() {
		   return $this->table('wechat')->field('appid, appsecret')->limit('1')->find();
	}
	
	/**
	 * desc : add wechat users
	 * @param array $data
	 * 
	 * @author jason
	 * @since 2015-12-30 上午10:22:05
	 */
	function addWxUser($data) {
		  return $this->table('wx_users')->insert($data);
	}
	 ...
	 * 
	 * @author jason
	 * @since 2015-12-30 上午10:54:12
	 */
	public function get_wx_user($openid){
	    $sql = "SELECT member.* FROM `".C('tablepre')."member` member INNER JOIN `".C('tablepre')."wx_users` wxuser ON member.`wx_users_id` = wxuser.`id` 
	    	WHERE wxuser.`openid` = '{$openid}' ";
	...
	function get_member_count() {
		   return $this->table('member')->count();
	}
