...
class pointorderModel extends Model{
	...
	/**
	 * 生成U币兑换订单编号(两位随机 + 从2000-01-01 00:00:00 到现在的秒数+微秒+会员ID%1000)，该值会传给第三方支付接口
	 * 长度 =2位 + 10位 + 3位 + 3位  = 18位
	...
	public function addPointOrder($param) {
		...
		if ($result && $param['point_buyerid'] > 0){
		    uk86_wcache($param['point_buyerid'], array('pointordercount' => null), 'm_pointorder');
		}
		return $result;
	}
	/**
	...
	public function getPointOrderGoodsList($where, $field = '*', $page = 0, $limit = 0,$order = '', $group = '') {
	    ...
	    if ($ordergoods_list){
	    	foreach ($ordergoods_list as $k=>$v){
	    	    ...
	    	    $v['point_goodsimage_small'] = uk86_pointprodThumb($v['point_goodsimage'], 'small');
	    	    $v['point_goodsimage'] = uk86_pointprodThumb($v['point_goodsimage'], 'mid');
	    	    $ordergoods_list[$k] = $v;
	    	}
	    }
	...
	public function cancelPointOrder($point_orderid, $member_id = 0) {
	    ...
	    if ($result){
	        ...
	        //更新会员缓存
	        uk86_wcache($order_info['point_buyerid'], array('pointordercount' => null), 'm_pointorder');
	...
	public function getMemberPointsOrderGoodsCount($member_id){
	    $info = uk86_rcache($member_id, 'm_pointorder', 'pointordercount');
	    if (empty($info['pointordercount']) && $info['pointordercount'] !== 0) {
	        ...
	        uk86_wcache($member_id, array('pointordercount' => $pointordercount), 'm_pointorder');
	    } else {
