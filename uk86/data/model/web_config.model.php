...
class web_configModel extends Model{
	...
	/**
	 * 更新模块html信息
	 * @param $web_style 使用第几套模板
	 * @param $web_id  楼层id
	 */
	public function updateWebHtml($web_id = 1,$style_name = 'orange',$web_style="0"){
		$web_html = '';
		$code_list = $this->getCodeList(array('web_id'=>"$web_id"));    //获取模块里面的内容
		...
		if(!empty($code_list) && is_array($code_list)) {
			...
			foreach ($code_list as $key => $val) {
				...
				$val['code_info'] = $this->get_array($code_info,$code_type);  //获取codeinfo对象
    		...
    		switch ($web_id) {
        	    ...
        	    default:
					if($web_style==2) {
						$style_file = BASE_DATA_PATH.DS.'resource'.DS.'web_config'.DS.'index_style_2.php';
					}else if($web_style==3){
						$style_file = BASE_DATA_PATH.DS.'resource'.DS.'web_config'.DS.'index_style_3.php';
					}else if($web_style==1){
						$style_file = BASE_DATA_PATH.DS.'resource'.DS.'web_config'.DS.'index_style_1.php';
					}else if($web_style==0){
			...
			$web_array = array();
			$web_array['web_html'] = addslashes($web_html);   //生成的html文件
			$web_array['update_time'] = time();
			$web_array['web_style']=$web_style;
	...
	public function getStyleList($style_id = 'index'){
		$style_data = array(
			'red' => Uk86Language::uk86_get('web_config_style_red'),
			'pink' => Uk86Language::uk86_get('web_config_style_pink'),
			'orange' => Uk86Language::uk86_get('web_config_style_orange'),
			'green' => Uk86Language::uk86_get('web_config_style_green'),
			'blue' => Uk86Language::uk86_get('web_config_style_blue'),
			'purple' => Uk86Language::uk86_get('web_config_style_purple'),
			'brown' => Uk86Language::uk86_get('web_config_style_brown'),
			'default' => Uk86Language::uk86_get('web_config_style_default')
			);
	...
	public function get_array($code_info,$code_type){
		...
		switch ($code_type) {
    	    case "array":
    	    	if(is_string($code_info)) $code_info = unserialize($code_info);   //反序列化code_info，得到code内容对象
	...
	public function  getWebById($web_id){
		$web=$this->table('web')->find($web_id);         //获取某个楼层的记录
		return $web;
	}
