...
class pm_orderModel extends Model {
    ...
    public function getOrderInfo($condition = array(), $fields = '*', $master = false) {
        $order_info = $this->table('pm_order')->field($fields)->where($condition)->master($master)->find();
        if (empty($order_info)) {
            return array();
        }
        ...
        if (isset($order_info['payment_code'])) {
            $order_info['payment_name'] = uk86_orderPaymentName($order_info['payment_code']);
        }
        return $order_info;
    }
    ...
    public function addOrder($data) {
        $insert = $this->table('pm_order')->insert($data);
        return $insert;
    }
...
//         $order_code[$i-1]['pay_price'] += $diff_amount;
    ...
    public function editOrder($data, $condition, $limit = '') {
        return $this->table('pm_order')->where($condition)->limit($limit)->update($data);
    }
    ...
    public function getOrderList($condition, $pagesize = '', $field = '*', $order = 'order_id desc', $limit = ''){
        $list = $this->table('pm_order')->field($field)->where($condition)->page($pagesize)->order($order)->limit($limit)->select();
        ...
        foreach ($list as $key => $order) {
            ...
            if (isset($order['payment_code'])) {
                 $list[$key]['payment_name'] = uk86_orderPaymentName($order['payment_code']);
            }
        }
    ...
    public function getOrderCount($condition) {
        return $this->table('pm_order')->where($condition)->count();
    }
	...
	public function delOrder($order_id){
		return $this->table('pm_order')->where('order_id='.$order_id)->delete();
	}
}
