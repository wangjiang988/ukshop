...
class goodsModel extends Model{
     ...
     * @return mixe获取根据条件获取商品列表
     */
    public function getGoodsListCommon($condition, $field = '*',$order = '',$page) {
        $condition['goods_state']   = self::STATE1;
        $condition['goods_verify']  = self::VERIFY1;
        $condition = $this->_getRecursiveClass($condition);
        $goodsList=$this->table('goods')->field($field)->where($condition)->order($order)->page($page)->select();
        return $goodsList;
    }
    /**
     ...
     */
    public function getGoodsListForPromotion($condition, $field = '*', $page = 0, $type = '', $group = '') {
        switch ($type) {
            case 'xianshi':
            case 'bundling':
            case 'combo':
        ...
        return $this->getGoodsList($condition, $field, $group, '', 0, $page);
    }
    ...
    public function getGoodsInfoByID($goods_id, $fields = '*') {
        ...
        if (empty($goods_info)) {
            $goods_info = $this->getGoodsInfo(array('goods_id'=>$goods_id, 'goods_state' => 1));
    ...
    private function _rGoodsCache($goods_id, $fields) {
        return uk86_rcache($goods_id, 'goods', $fields);
    }
    ...
    private function _wGoodsCache($goods_id, $goods_info) {
        return uk86_wcache($goods_id, $goods_info, 'goods');
    }
    ...
    private function _dGoodsCache($goods_id) {
        return uk86_dcache($goods_id, 'goods');
    }
    ...
    private function _rGoodsCommonCache($goods_commonid, $fields) {
        return uk86_rcache($goods_commonid, 'goods_common', $fields);
    }
    ...
    private function _wGoodsCommonCache($goods_commonid, $common_info) {
        return uk86_wcache($goods_commonid, $common_info, 'goods_common');
    }
    ...
    private function _dGoodsCommonCache($goods_commonid) {
        return uk86_dcache($goods_commonid, 'goods_common');
    }
    ...
    private function _rGoodsSpecCache($goods_commonid) {
        return uk86_rcache($goods_commonid, 'goods_spec');
    }
    ...
    private function _wGoodsSpecCache($goods_commonid, $spec_list) {
        return uk86_wcache($goods_commonid, $spec_list, 'goods_spec');
    }
    ...
    private function _dGoodsSpecCache($goods_commonid) {
        return uk86_dcache($goods_commonid, 'goods_spec');
    }
    ...
    private function _rGoodsImageCache($key) {
        return uk86_rcache($key, 'goods_image');
    }
    ...
    private function _wGoodsImageCache($key, $image_list) {
        return uk86_wcache($key, $image_list, 'goods_image');
    }
    ...
    private function _dGoodsImageCache($key) {
        return uk86_dcache($key, 'goods_image');
    }
