...
class buy_1Logic {
    ...
    public function calcCartList($store_cart_list) {
        ...
        foreach ($store_cart_list as $store_id => $store_cart) {
            ...
            foreach ($store_cart as $key => $cart_info) {
                $store_cart[$key]['goods_total'] = uk86_ncPriceFormat($cart_info['goods_price'] * $cart_info['goods_num']);
                $store_cart[$key]['goods_image_url'] = uk86_cthumb($store_cart[$key]['goods_image']);
                $tmp_amount += $store_cart[$key]['goods_total'];
            }
            $store_cart_list[$store_id] = $store_cart;
            $store_goods_total[$store_id] = uk86_ncPriceFormat($tmp_amount);
        }
	...
	/**
	 * 验证传过来的卡券包是否可用有效，如果无效，直接删除
	 * @param array $input_voucher_list 卡券包列表
    ...
    public function getXianshiInfo( & $goods_info, $quantity) {
        ...
        $goods_info['xianshi_info']['down_price'] = uk86_ncPriceFormat($goods_info['goods_price'] - $goods_info['xianshi_info']['xianshi_price']);
...
//         if (empty($pd_payment_info)) return;
    ...
    private function _parseMansongRuleDesc($rule_info) {
        ...
        " 送<a href='".uk86_urlShop('goods','index',array('goods_id'=>$rule_info['goods_id']))."' title='{$rule_info['mansong_goods_name']}' target='_blank'>[赠品]</a>" : '';
