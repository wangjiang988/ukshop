...
class buyLogic
{
    ...
    {
        $this->_logic_buy_1 = uk86_Logic('buy_1');
    }
    ...
    public function buyStep1($cart_id, $ifcart, $member_id, $store_id)
    {
        ...
        //得到页面所需要数据：收货地址、发票、卡券包、预存款、商品列表等信息
    ...
    public function getCartList($cart_id, $member_id)
    {
        ...
        if (empty($buy_items)) {
            return uk86_callback(false, '所购商品无效');
        }

        if (count($buy_items) > 50) {
            return uk86_callback(false, '一次最多只可购买50种商品');
        }
        ...
        return uk86_callback(true, '', array('goods_list' => $goods_list, 'store_cart_list' => $store_cart_list));
    ...
    public function getGoodsList($cart_id, $member_id, $store_id)
    {
        ...
        if (empty($goods_info)) {
            return uk86_callback(false, '您的购买数量已达到抢购上限，不能再购买');
        }

        //不能购买自己店铺的商品
        if ($goods_info['store_id'] == $store_id) {
            return uk86_callback(false, '不能购买自己店铺的商品');
        }
    ...
    public function getBuyStep1Data($member_id, $data)
    {
        ...
        return uk86_callback(true, '', $result);
    }
    ...
    public function buyStep2($post, $member_id, $member_name, $member_email)
    {
        ...
        try {
            ...
            return uk86_callback(true, '', $this->_order_data);
    ...
    public function delCart($ifcart, $member_id, $cart_ids)
    {
        ...
        if (preg_match('/^[\d,]+$/', $cart_id_str)) {
            Uk86QueueClient::push('delCart', array('buyer_id' => $member_id, 'cart_ids' => $cart_ids));
        }
    }
    ...
    private function _createOrderStep1()
    {
        ...
        if (($input_address_id > 0) && (intval($post['mention_radio']) != 1)) {
        ...
        if ((!in_array($input_if_offpay, array('allow_offpay', 'deny_offpay'))) && (intval($post['mention_radio']) != 1)) {
    ...
    private function _createOrderStep3()
    {
        ...
        //计算每个店铺运费
        if ($this->_post_data['mention_radio'] == 1) {
            //商品自提无运费
            $this->_order_data['goods_list'][0]['goods_freight'] = '0.00';
        } else {
        ...
        $this->_order_data['store_mentioning'] = intval($this->_post_data['mention_radio']);
    ...
    private function _createOrderStep4()
    {
        ...
        foreach ($store_cart_list as $store_id => $goods_list) {
            ...
            $order['buyer_name'] = $member_name;
            $order['buyer_email'] = $member_email;
            $order['is_anonymous'] = intval($_POST['is_anonymous']);
            $order['add_time'] = TIMESTAMP;
            ...
            $order['store_mentioning'] = $this->_order_data['store_mentioning'];
            //如果支持方式为空时，默认为货到付款 ukshop
            if ($order['payment_code'] == "") {
                $order['payment_code'] = "offline";
            }
...
//                      $order_goods[$i]['store_mentioning'] = $this->_order_data['store_mentioning'];
                        $order_goods[$i]['goods_num'] = $goods_info['goods_num'];
     ...
     * @return string
     */
    private function _get_code($order_code, $member_name)
    {
        $code_name = md5($order_code . time()) . '.png';
        $code_path = BASE_UPLOAD_PATH . DS . 'shop/mention_code/';
        $this->PhpQRCode->set('pngTempDir', $code_path);
        $this->PhpQRCode->set('date', '订单号：' . $order_code . Chr(10) . '用户名：' . $member_name);
        $this->PhpQRCode->set('pngTempName', $code_name);
        $this->PhpQRCode->init();
