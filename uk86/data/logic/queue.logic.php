...
class queueLogic {
    ...
    public function editGroupbuySaleCount($groupbuy_info) {
        ...
        if (!$update) {
            return uk86_callback(false,'更新抢购信息失败groupbuy_id:'.$groupbuy_info['groupbuy_id']);
        } else {
            return uk86_callback(true);
        }
    }
    ...
    public function editVoucherState($voucher_list) {
        ...
        foreach ($voucher_list as $store_id => $voucher_info) {
            ...
            if ($update) {
                ...
                return uk86_callback(false,'更新卡券包状态失败vcode:'.$voucher_info['voucher_code']);
            }
        }
        return uk86_callback(true);
    }
    ...
    public function createOrderUpdateStorage($goods_buy_quantity) {
        ...
        if (!$result) {
            return uk86_callback(false,'变更商品库存与销量失败');
        } else {
            return uk86_callback(true);
        }
    }
    ...
    public function updateGoodsFCode($fc_id) {
        ...
        if (!$update) {
            return uk86_callback(false,'更新F码使用状态失败fc_id:'.$fc_id);
        } else {
            return uk86_callback(true);
        }
    }
    ...
    public function delCart($cart) {
        if (!is_array($cart['cart_ids']) || empty($cart['buyer_id'])) return uk86_callback(true);
        $del = Model('cart')->delCart('db',array('buyer_id'=>$cart['buyer_id'],'cart_id'=>array('in',$cart['cart_ids'])));
        if (!$del) {
            return uk86_callback(false,'删除购物车数据失败');
        } else {
            return uk86_callback(true);
        }
    }
    ...
    public function updateGoodsPromotionPriceByGoodsId($goods_id) {
        ...
        if (!$update) {
            return uk86_callback(false,'根据商品ID更新促销价格失败');
        } else {
            return uk86_callback(true);
        }
    }
    ...
    public function updateGoodsPromotionPriceByGoodsCommonId($goods_commonid) {
        ...
        if (!$update) {
            return uk86_callback(false,'根据商品公共id更新促销价格失败');
        } else {
            return uk86_callback(true);
        }
    }
    ...
    public function sendStoreMsg($param) {
        $send = new Uk86sendStoreMsg();
        $send->uk86_set('code', $param['code']);
        $send->uk86_set('store_id', $param['store_id']);
        $send->uk86_send($param['param']);
        return uk86_callback(true);
    }
    ...
    public function createGoodsFCode($param) {
        ...
        for ($i = 0; $i < $param['fc_count']; $i++) {
            ...
            $array['fc_code'] = strtoupper($param['fc_prefix']).mt_rand(1000000000,9999999999);
        ...
        if (!empty($insert)) {
            ...
            if (!$insert) {
                return uk86_callback(false,'生成商品F码失败goods_commonid:'.$param['goods_commonid']);
            }
        }
        return uk86_callback(true);
    }
    ...
    public function createGoodsQRCode($param) {
        ...
        foreach ($param['goodsid_array'] as $goods_id) {
            // 生成商品二维码
            $PhpQRCode->set('date',uk86_urlShop('goods', 'index', array('goods_id'=>$goods_id)));
    ...
    public function delOrderCountCache($order_info){
        if (empty($order_info)) return uk86_callback(true);
    ...
    public function sendVrCode($param) {
        if (empty($param) && !is_array($param)) return uk86_callback(true);
        $condition = array();
        ...
        if (empty($code_list)) return uk86_callback(true);
        ...
        $sms = new Uk86Sms();
        $result = $sms->uk86_send($param["buyer_phone"],$message);
        if (!$result) {
            return uk86_callback(false,'兑换码发送失败order_id:'.$param['order_id']);
        } else {
            return uk86_callback(true);
        }
    }
    ...
    public function saveDeliveryOrder($param) {
        if (!is_array($param['order_sn_list'])) return uk86_callback(true);
        $data = array();
        ...
        foreach ($param['order_sn_list'] as $order_id => $v) {
            ...
            if (!$insert) {
                return uk86_callback(false,'保存自提站订单信息失败order_sn:'.$v['order_sn']);
            }
        }
        return uk86_callback(true);
    }
