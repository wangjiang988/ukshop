...
class paymentLogic {
    ...
    public function getRealOrderInfo($pay_sn, $member_id = null) {
        ...
        if(empty($order_pay_info)){
            return uk86_callback(false,'该支付单不存在');
        }
        ...
        return uk86_callback(true,'',$order_pay_info);
    }
    ...
    public function getVrOrderInfo($order_sn, $member_id = null) {
        ...
        if(empty($order_info)){
            return uk86_callback(false,'该订单不存在');
        }
        ...
        $pay_amount = uk86_ncPriceFormat(floatval($order_info['order_amount']) - floatval($order_info['pd_amount']) - floatval($order_info['rcb_amount']));

        $order_info['api_pay_amount'] = $pay_amount;

        return uk86_callback(true,'',$order_info);
    }
     ...
     * @return array
     */
    public function getPmOrderInfo($order_sn, $member_id = null) {

        //验证订单信息
        $model_order = Model('pm_order');
        $condition = array();
        $condition['order_sn'] = $order_sn;
        if (!empty($member_id)) {
            $condition['buyer_id'] = $member_id;
        ...
        $order_info['subject'] = '促销订单_'.$order_sn;
        $order_info['order_type'] = 'vr_order';
        $order_info['pay_sn'] = $order_sn;
     ...
     */
    public function getPromotionVrOrderInfo($order_sn, $member_id = null) {
        ...
        $order_info['subject'] = '促销虚拟订单_'.$order_sn;
        $order_info['order_type'] = 'promotion_vr_order';
        $order_info['pay_sn'] = $order_sn;
...
//      $order_info['api_pay_amount'] = $pay_amount;
    ...
    public function getPaymentInfo($payment_code) {
        if (in_array($payment_code,array('offline','predeposit')) || empty($payment_code)) {
            return uk86_callback(false,'系统不支持选定的支付方式');
        }
        ...
        return uk86_callback(true,'',$payment_info);
    }
    ...
    public function updateRealOrder($out_trade_no, $payment_code, $order_list, $trade_no) {
        ...
        return uk86_Logic('order')->changeOrderReceivePay($order_list, 'system', '系统', $post);
    }
    ...
    public function updateVrOrder($out_trade_no, $payment_code, $order_info, $trade_no) {
        $post['payment_code'] = $payment_code;
        $post['trade_no'] = $trade_no;
        return uk86_Logic('vr_order')->changeOrderStatePay($order_info, 'system', $post);
    }
