$(document).ready(function(){
    ...
    $("#btn_show_special_upload_img").click(function(){
        html_form('dialog_special_upload_img', '插入图片', $("#_dialog_special_upload_img").html(), 640);
    });
    ...
    //插入图片列表
    $("#btn_insert_img").live('click', function(){
    	var html = $("#special_content").val();
        html += "<div class='special-content-goods-list'>";
        var img_list = $("<div />").append($(this).parents('table').find("[nctype='special_image_list_up']").clone());
        img_list.find("dd.delete").remove();
        html += img_list.html();
        html += "</div>";
        $("#special_content").val(html);
        special_view_update();
        DialogManager.close("dialog_special_upload_img");
    });
