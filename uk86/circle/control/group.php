...
class groupControl extends BaseCircleThemeControl{
	...
	public function group_goodsOp(){
		...
		$cmid_list	= uk86_array_under_reset($cmid_list, 'member_id'); $cmid_array = array_keys($cmid_list);
		...
		if(!empty($goods_list)){
			...
			$goods_list	= uk86_array_under_reset($goods_list, 'share_id'); $shareid_array = array_keys($goods_list);
			Tpl::output('show_page', $model->showpage('2'));
