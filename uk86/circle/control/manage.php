...
class manageControl extends BaseCircleManageControl{
	...
	public function class_addOp(){
		...
		$allThclassList=Model('circle_thclass_list')->where("thclass_status=1")->select();  //获取所有的话题分类列表

		Tpl::output('allThclassList',$allThclassList);
	...
	public function search_circleOp(){
		...
		if (!empty($circle_list)){
			$circle_list = uk86_array_under_reset($circle_list, 'friendship_id'); $circle_array = array_keys($circle_list);
		}
