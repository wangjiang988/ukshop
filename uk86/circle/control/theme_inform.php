...
class theme_informControl extends BaseCircleControl{
	...
	public function __construct(){
		...
		Uk86Language::uk86_read('manage_inform');
	}
	/**
	 * Share the binding
	 */
	...
	public function enroll_ajaxOp(){
		//判断手机是否已报名
		$phone = $_POST['phone'];
		$t_id = intval($_POST['t_id']);
		$model = Model('circle_enroll');
		$result_phone = $model -> where(array('t_id' => $t_id, 'member_phone' => $phone))->field('theme_id')->find();
		if($result_phone['theme_id'] > 0){
			echo '10';
			return;
		}
		$insert = array();
		...
		$insert['member_truename'] = $_POST['truename'];
		$insert['member_phone'] = $phone;
		$insert['member_email'] = $_POST['email'];
		$insert['member_qq'] = $_POST['tx_qq'];
		$insert_id = $model->insert($insert);
