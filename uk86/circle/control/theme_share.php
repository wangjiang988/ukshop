...
class theme_shareControl extends BaseCircleControl{
	...
	public function indexOp(){
		...
		if(uk86_chksubmit()){
			...
			$validate_arr[] = array("input"=>$_POST["content"], "validator"=>'Length',"min"=>0,"max"=>140,"message"=>Uk86Language::uk86_get('sharebind_content_not_null'));
			...
			if ($result){
				...
				if (C('share_isuse') == 1){
					...
					if(!empty($affix_list[0])){
						$params['images'] = uk86_themeImageUrl($affix_list[0]['affix_filethumb']);
					}
					// Share Tencent Weibo
				...
				showDialog(Uk86Language::uk86_get('sharebind_share_succ'),'','succ','DialogManager.close("share");var count = $(\'em[nctype="share"]\').html(); $(\'em[nctype="share"]\').html(parseInt(count)+1);');
			}else {
				showDialog(Uk86Language::uk86_get('sharebind_share_fail'),'','error','DialogManager.close("share");');
			}
		}
