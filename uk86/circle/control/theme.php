...
class themeControl extends BaseCircleThemeControl{
	...
	public function ajax_themeinfoOp(){
		...
		if(!empty($affix_list)){
			foreach ($affix_list as $key=>$val){
				$affix_list[$key]['affix_filename'] = uk86_themeImageUrl($val['affix_filename']);
				$affix_list[$key]['affix_filethumb'] = uk86_themeImageUrl($val['affix_filethumb']);
			}
		}
		...
		$data['theme_content'] = uk86_ubb($data['theme_content']);
	...
	public function ajax_quickreplyOp(){
		...
		$data['member_avatar'] = uk86_getMemberAvatarForID($_SESSION['member_id']); // 头像
		// 回复
	...
	public function save_themeOp(){
		if(uk86_chksubmit()){
			...
			if(intval(C('circle_contentleast')) > 0) $validate_arr[] = array("input"=>$_POST["themecontent"],"validator"=>'Length',"min"=>intval(C('circle_contentleast')),"message"=>Uk86Language::uk86_get('circle_contentleast'));
...
// 	}
	...
	public function image_uploadOp(){
		...
		if($count < 10){
			...
			$result = $upload->uk86_upfile('test_file');		// 暂时的名字
			if ($result){
				$insert = array();
	...
	public function delimgOp(){
		...
		if($id > 0){
			...
			if(!empty($affix_info)){
				@unlink(uk86_themeImagePath($affix_info['affix_filename']));
				@unlink(uk86_themeImagePath($affix_info['affix_filethumb']));
	...
	public function theme_detailOp(){
		...
		if($this->m_readperm < $this->theme_info['theme_readperm']){
			uk86_showMessage(L('circle_Insufficient_permissions'), CIRCLE_SITE_URL, '', 'error');
		}
		...
		$member_list = uk86_array_under_reset($member_list, 'member_id');
		...
		if($this->theme_info['theme_special'] == 2){
			$enroll_model = Model('circle_enroll');
			$enroll_info = $enroll_model->where(array('t_id' => $this->t_id))->select();
			Tpl::output('enroll_info', $enroll_info);
			//已经报名的则不能继续报名
			$i = count($enroll_info);
			//获取报名人信息
			$member_info = Model()->table('member')->where(array('member_id' => intval($_SESSION['member_id'])))->field('member_truename, member_email, member_qq, member_mobile')->find();
			//报名人数
			$enroll_num = $i;
			Tpl::output('enroll_member', $member_info);
			Tpl::output('member_num', $enroll_num);
		}
	...
	public function new_themeOp(){
		...
		//不是圈子人员不能报名
		if($_GET['sp'] == 2  && !in_array($this->identity, array(1,2,3))){
			showDialog('没有加入圈子的用户不能报名');
		}
		// 不是圈子成员不能发帖
		if(!in_array($this->identity, array(1,2,3))){
			showDialog(L('circle_no_join_ban_release'));
