...
<style type="text/css">
.new-poll .new_biao_ming{background-position:-240px -130px !important;}
.enroll_info{margin:10px 20px 10px 20px; border:1px solid #e5ecee;}
.enroll_info li{ float:left; line-height:28px;}
.enroll_info li:nth-of-type(1){width:60px; line-height:56px; text-align:center;}
.enroll_info li:nth-of-type(2){font-size:14px;}
.enroll_info li:nth-of-type(3){width:90px; font-size:14px;}
.enroll_info li:nth-of-type(4){font-size:14px;}
.enroll_info li:nth-of-type(5){width:500px; font-size:14px;}
.enroll_info li:nth-of-type(7){width:140px;}
.enroll_info li:nth-of-type(9){margin-right:10px;}
.enroll_info li:nth-of-type(10){margin-left:10px;}
</style>
  ...
  <div class="base-layout mt20">
    <div class="mainbox">
      <div class="theme-detail-header">
        <div class="member-avatar-m"><img src="<?php echo uk86_getMemberAvatarForID($output['theme_info']['member_id']);?>"/></div>
      ...
      <?php if($output['theme_info']['theme_special'] == 2){require_once circle_template('theme.detail_enroll');} ?>
      <div class="theme-detail-content">
        <?php if($output['theme_info']['is_closed'] == '0'){?>
        <div class="main-text"><?php echo uk86_ubb($output['theme_info']['theme_content']);?></div>
        ...
        <div class="theme-content-file clearfix">
          ...
          <div class="file-hidden"> <i class="arrow"></i>
            <ul>
              ...
              <li><a href="<?php echo uk86_themeImageUrl($val['affix_filename']);?>" class="nyroModal" rel="gal" title="<?php echo $lang['circle_affix_image_title_one'].$output['theme_info']['theme_name'].$lang['circle_affix_image_title_two'];?>"><img src="<?php echo uk86_themeImageUrl($val['affix_filethumb']);?>"/></a> </li>
              <?php }?>
            </ul>
          </div>
        </div>
     ...
     <?php if($output['theme_info']['theme_special'] == 2){ ?>
       <?php if($_SESSION['member_id'] == $output['creator']['member_id'] || $_SESSION['member_id'] == $output['theme_info']['member_id'] || $ok){ ?>
        <div class="handle-bar"><div class="manage"><a class="manage-button" href="javascript:void(0);">报名人信息</a><a class="enroll_show" href="javascript:void(0);" style="text-decoration:none;">【收起】</a></div></div>
      </div>
        <?php if(!empty($output['enroll_info']) && is_array($output['enroll_info'])){ ?>
      	<?php foreach ($output['enroll_info'] as $k => $v){ ?>
      	  <ul class="enroll_info">
      	    <li><?php echo $k+1 ?></li>
      	  	<li>姓名：</li>
      	  	<li><?php echo $v['member_truename'] ?></li>
      	  	<li>手机：</li>
      	  	<li><?php echo $v['member_phone'] ?></li>
      	  	<li>报名时间：</li>
      	  	<li><?php echo date('Y-m-d H:i:s', $v['enroll_time']) ?></li>
      	  	<li>邮箱：</li>
      	  	<li><?php echo $v['member_email']?$v['member_email']:'未填写' ?></li>
      	  	<li>Q Q：</li>
      	  	<li><?php echo $v['member_qq']?$v['member_qq']:'未填写' ?></li>
      	  	<div class="clear"></div>
      	  </ul>
      	<?php } ?>
      	  <div class="pagination"><?php echo $output['show_page2'];?></div>
      	<?php }else{ ?>
      	  <div class="no_member" style="line-height:100px; font-size:18px; color:#2b9e56; text-align:center;">暂无人报名</div>
      	<?php } ?>
      <div class="theme-detail-content">
      <?php } ?>
     <?php } ?>
      ...
      <ul class="theme-reply-list">
        ...
        <li class="reply-info" id="f<?php echo $val['reply_id'];?>">
          <dl>
            ...
            <?php }?>
            <dd class="member-avatar-m"><img src="<?php echo uk86_getMemberAvatarForID($val['member_id']);?>" /></dd>
            <?php if($val['is_closed'] == '0'){?>
            <dd class="reply-content"><?php echo uk86_ubb($val['reply_content']);?></dd>
            ...
            <dd class="reply-file clearfix">
              ...
              <ul>
                ...
                <li> <a href="<?php echo uk86_themeImageUrl($val['affix_filename']);?>" class="nyroModal" rel="gal" title="<?php echo $lang['circle_reply_image_title_one'].$val['reply_id'].$lang['circle_reply_image_title_two'];?>"><img src="<?php echo uk86_themeImageUrl($val['affix_filethumb']);?>"/></a> </li>
                <?php }?>
              </ul>
            </dd>
            <?php }?>
      ...
      <div class="quick-reply">
        ...
        <div class="quick-reply-member"><a id="quickReply"></a>
          <div class="member-avatar-m"><img src="<?php echo uk86_getMemberAvatarForID($output['cm_info']['member_id']);?>"/></div>
        </div>
...
//横高局中比例缩放隐藏显示图片
	...
	$('.enroll_show').toggle(
		function(){
			$('.enroll_info,.no_member').hide();
			$(this).html('【展开】');
		},
		function(){
			$('.enroll_info,.no_member').show();
			$(this).html('【收起】');
		}
	);
});
