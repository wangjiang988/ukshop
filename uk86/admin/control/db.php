...
class dbControl extends SystemControl{
	...
	public function dbOp(){
		...
		if ($_POST['form_submit'] == 'ok'){
			...
			if (intval($_POST['file_size']) < 10){
				uk86_showMessage($lang['db_index_min_size']);
			}
			/**
			 * 验证文件夹是否重复
			 */
			if (is_dir(BASE_ROOT_PATH.DS.'sql_back'.DS.$_POST['backup_name'])) {
				uk86_showMessage($lang['db_index_name_exists']);
			}
			...
			if ($_POST['backup_type'] == 'custom'){
				...
				if (empty($_POST['tables'])){
					uk86_showMessage($lang['db_index_choose']);
				}
			...
			uk86_showMessage($lang['db_index_backup_to_wait'],'index.php?act=db&op=db&step=1');
		}
		/**
		 * 数据库备份步骤
		 */
		if (intval($_GET['step']) >= 1){
			...
			if (
				...
				uk86_showMessage($lang['db_index_backup_succ'],$url);
			}else {
				/**
				 * 跳转
				 */
				...
				uk86_showMessage($lang['db_index_backup_succ1'].intval($_GET['step']).$lang['db_index_backup_succ2'],$url);
			}
	...
	public function db_importOp(){
		...
		if ($_GET['dir_name'] != '' && file_exists(BASE_ROOT_PATH.DS.'sql_back'.DS.$_GET['dir_name'])){
			...
			if ($result == 'succ'){
				...
				uk86_showMessage($lang['db_import_succ'],$url);
			}
			/**
			 * 继续导入
			 */
			if ($result == 'continue'){
				...
				uk86_showMessage($lang['db_index_backup_succ1'].intval($_GET['step']).$lang['db_import_succ2'],$url);
			}
			/**
			 * 导入失败
			 */
			if ($result == false){
				uk86_showMessage($lang['db_import_fail']	);
			}
		}else {
			uk86_showMessage($lang['db_import_file_not_exists']);
		}
	}
	...
	public function db_delOp(){
		...
		if ($_GET['dir_name'] != '' && file_exists(BASE_ROOT_PATH.DS.'sql_back'.DS.$_GET['dir_name'])){
			...
			uk86_showMessage($lang['db_del_succ']);
		}else {
			uk86_showMessage($lang['db_restore_file_not_exists']);
		}
	}
