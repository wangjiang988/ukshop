		...
		if($_POST['form_submit']){
			...
			if ($error != ''){
				...
				if($return){
					...
					uk86_showMessage($lang['type_edit_type_attr_edit_succ'],'index.php?act=type&op=type');
				}else{
					$this->log(L('type_edit_type_attr_edit').'['.$_POST['attr_name'].']',0);
					uk86_showMessage($lang['type_edit_type_attr_edit_fail'],'','','error');
				}
			}
		}
	...
	public function type_delOp(){
		...
		if(is_array($type_list) && !empty($type_list)){
			...
			if(is_array($attr_list) && !empty($attr_list)){
				...
				if(!$return1 || !$return2){
					uk86_showMessage($lang['type_index_del_related_attr_fail']);
				}
			}
			...
			if(!$return){
				uk86_showMessage($lang['type_index_del_related_brand_fail']);
			}

			//删除对应规格
			$return = $model_type->delType('type_spec', array('in_type_id'=>$id));
			if(!$return){
				uk86_showMessage($lang['type_index_del_related_type_fail']);
			}

			//删除类型
			$return = $model_type->delType('type', array('in_type_id'=>$id));
			if(!$return){
				uk86_showMessage($lang['type_index_del_fail']);
			}

			$this->log(L('nc_delete,type_index_type_name').'[ID:'.$id.']',1);
			uk86_showMessage($lang['type_index_del_succ']);
		}else{
...
//			case 'name':
	...
	private function createExcel($data = array()){
		...
		$excel_obj->uk86_addWorksheet($excel_obj->uk86_charset(L('exp_type_name'),CHARSET));
		$excel_obj->uk86_generateXML($excel_obj->uk86_charset(L('exp_type_name'),CHARSET).$_GET['curpage'].'-'.date('Y-m-d-H',time()));
	}
}
