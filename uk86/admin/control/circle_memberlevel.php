...
class circle_memberlevelControl extends SystemControl{
	public function __construct(){
		parent::__construct();
		Uk86Language::uk86_read('circle_memberlevel');
	}
	/**
	...
	public function ref_editOp(){
		...
		if(uk86_chksubmit()){
			...
			if($rs){
				uk86_showMessage(L('nc_common_op_succ'), 'index.php?act=circle_memberlevel&op=ref');
			}else{
				uk86_showMessage(L('nc_common_op_fail'));
			}
		}
	...
	public function update_cacheOp() {
		uk86_dkcache('circle_level');
		uk86_showMessage(L('nc_common_op_succ'), 'index.php?act=circle_memberlevel');
