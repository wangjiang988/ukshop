...
class navigationControl extends SystemControl{
	public function __construct(){
		parent::__construct();
		Uk86Language::uk86_read('navigation');
	}
		...
		if (uk86_chksubmit()){
			...
			if ($error != ''){
				...
				if ($result){
					...
					uk86_showMessage($lang['navigation_add_succ'],$url);
				}else {
					uk86_showMessage($lang['navigation_add_fail']);
				}
			}
		}
		/**
	...
	public function navigation_delOp(){
		...
		if (intval($_GET['nav_id']) > 0){
			...
			uk86_showMessage($lang['navigation_index_del_succ'],'index.php?act=navigation&op=navigation');
		}else {
			uk86_showMessage($lang['navigation_index_choose_del'],'index.php?act=navigation&op=navigation');
		}
	}
