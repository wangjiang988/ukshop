...
class voucherControl extends SystemControl{
	...
	public function __construct(){
		parent::__construct();
		Uk86Language::uk86_read('voucher');
		if (C('voucher_allow') != 1 || C('points_isuse')!=1){
			uk86_showMessage(Uk86Language::uk86_get('admin_voucher_unavailable'),'index.php?act=operation&op=point','html','succ',1,4000);
		}
		$this->applystate_arr = array('new'=>array(1,Uk86Language::uk86_get('admin_voucher_applystate_new')),'verify'=>array(2,Uk86Language::uk86_get('admin_voucher_applystate_verify')),'cancel'=>array(3,Uk86Language::uk86_get('admin_voucher_applystate_cancel')));
		$this->quotastate_arr = array('activity'=>array(1,Uk86Language::uk86_get('admin_voucher_quotastate_activity')),'cancel'=>array(2,Uk86Language::uk86_get('admin_voucher_quotastate_cancel')),'expire'=>array(3,Uk86Language::uk86_get('admin_voucher_quotastate_expire')));
		//卡券包模板状态
		$this->templatestate_arr = array('usable'=>array(1,Uk86Language::uk86_get('admin_voucher_templatestate_usable')),'disabled'=>array(2,Uk86Language::uk86_get('admin_voucher_templatestate_disabled')));
    ...
    public function settingOp(){
    	...
    	if (uk86_chksubmit()){
			...
			$validate_arr[] = array('input'=>$_POST['promotion_voucher_price'],'require'=>'true','validator'=>'IntegerPositive','message'=>Uk86Language::uk86_get('admin_voucher_setting_price_error'));
			$validate_arr[] = array('input'=>$_POST['promotion_voucher_storetimes_limit'],'require'=>'true','validator'=>'IntegerPositive','message'=>Uk86Language::uk86_get('admin_voucher_setting_storetimes_error'));
			$validate_arr[] = array('input'=>$_POST['promotion_voucher_buyertimes_limit'],'require'=>'true','validator'=>'IntegerPositive','message'=>Uk86Language::uk86_get('admin_voucher_setting_buyertimes_error'));
	...
	public function priceaddOp(){
		if (uk86_chksubmit()){
			$obj_validate = new Uk86Validate();
			$validate_arr[] = array('input'=>$_POST['voucher_price'],'require'=>'true','validator'=>'IntegerPositive','message'=>Uk86Language::uk86_get('admin_voucher_price_error'));
			$validate_arr[] = array('input'=>$_POST['voucher_price_describe'],'require'=>'true','message'=>Uk86Language::uk86_get('admin_voucher_price_describe_error'));
			$validate_arr[] = array('input'=>$_POST['voucher_points'],'require'=>'true','validator'=>'IntegerPositive','message'=>Uk86Language::uk86_get('admin_voucher_price_points_error'));
			$obj_validate->validateparam = $validate_arr;
			$error = $obj_validate->uk86_validate();
			//验证面额是否存在
	        ...
	        if(!empty($voucherprice_info)) {
	            $error .= Uk86Language::uk86_get('admin_voucher_price_exist');
	        }
	        if ($error != ''){
	            ...
	            if ($rs){
	            	...
	            	uk86_showMessage(Uk86Language::uk86_get('nc_common_save_succ'),'index.php?act=voucher&op=pricelist');
	            }else {
	            	uk86_showMessage(Uk86Language::uk86_get('nc_common_save_fail'),'index.php?act=voucher&op=priceadd');
	            }
	        }
		}else {
    ...
    public function priceeditOp(){
    	...
    	if ($id <= 0){
    		uk86_showMessage(Uk86Language::uk86_get('param_error'),'index.php?act=voucher&op=pricelist');
    	}
    	$model = Model();
    	if (uk86_chksubmit()){
    		$obj_validate = new Uk86Validate();
			...
			if ($error != ''){
	    		...
	    		if ($rs){
	    			...
	    			uk86_showMessage(Uk86Language::uk86_get('nc_common_save_fail'),'index.php?act=voucher&op=pricelist');
	    		}
			}
    	}else {
    ...
    public function pricedropOp(){
        ...
        if ($rs){
        	...
        	uk86_showMessage(Uk86Language::uk86_get('nc_common_del_succ'),'index.php?act=voucher&op=pricelist');
        }else{
        	uk86_showMessage(Uk86Language::uk86_get('nc_common_del_fail'),'index.php?act=voucher&op=pricelist');
        }
    }
    ...
    public function templateeditOp(){
    	...
    	if ($t_id <= 0){
    		uk86_showMessage(Uk86Language::uk86_get('param_error'),'index.php?act=voucher&op=templatelist','','error');
    	}
        $model = Model('voucher');
        //查询模板信息
        $param = array();
        ...
        if (empty($t_info)){
        	uk86_showMessage(Uk86Language::uk86_get('param_error'),'index.php?act=voucher&op=templatelist','html','error');
        }
        if(uk86_chksubmit()){
        	$points = intval($_POST['points']);
        	if ($points < 0){
        		uk86_showMessage(Uk86Language::uk86_get('admin_voucher_template_points_error'),'','html','error');
        	}
	       	$update_arr = array();
	       	...
	       	if($rs){
	       		...
	       		uk86_showMessage(Uk86Language::uk86_get('nc_common_save_succ'),'index.php?act=voucher&op=templatelist','succ');
	       	}else{
	       		uk86_showMessage(Uk86Language::uk86_get('nc_common_save_fail'),'index.php?act=voucher&op=templatelist','error');
	       	}
        }else{
            //查询店铺分类
    ...
    private function show_menu($menu_type,$menu_key='') {
		...
		switch ($menu_type) {
			case 'voucher':
				$menu_array = array(
				3=>array('menu_key'=>'templatelist','menu_name'=>Uk86Language::uk86_get('admin_voucher_template_manage'), 'menu_url'=>'index.php?act=voucher&op=templatelist'),
				2=>array('menu_key'=>'quotalist','menu_name'=>Uk86Language::uk86_get('admin_voucher_quota_manage'), 'menu_url'=>'index.php?act=voucher&op=quotalist'),
				5=>array('menu_key'=>'pricelist','menu_name'=>Uk86Language::uk86_get('admin_voucher_pricemanage'), 'menu_url'=>'index.php?act=voucher&op=pricelist'),
				6=>array('menu_key'=>'priceadd','menu_name'=>Uk86Language::uk86_get('admin_voucher_priceadd'), 'menu_url'=>'index.php?act=voucher&op=priceadd'),
				4=>array('menu_key'=>'setting','menu_name'=>Uk86Language::uk86_get('admin_voucher_setting'),	'menu_url'=>'index.php?act=voucher&op=setting'),
				);
				break;
			case 'priceedit':
				$menu_array = array(
				1=>array('menu_key'=>'setting','menu_name'=>Uk86Language::uk86_get('admin_voucher_setting'),	'menu_url'=>'index.php?act=voucher&op=setting'),
				2=>array('menu_key'=>'pricelist','menu_name'=>Uk86Language::uk86_get('admin_voucher_pricemanage'), 'menu_url'=>'index.php?act=voucher&op=pricelist'),
				3=>array('menu_key'=>'priceedit','menu_name'=>Uk86Language::uk86_get('admin_voucher_priceedit'), 'menu_url'=>'')
				);
				break;
			case 'templateedit':
				$menu_array = array(
				1=>array('menu_key'=>'templatelist','menu_name'=>Uk86Language::uk86_get('admin_voucher_template_manage'), 'menu_url'=>'index.php?act=voucher&op=templatelist'),
				2=>array('menu_key'=>'templateedit','menu_name'=>Uk86Language::uk86_get('admin_voucher_template_edit'), 'menu_url'=>'')
				);
				break;
		}
