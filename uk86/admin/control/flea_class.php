...
class flea_classControl extends SystemControl{
	...
	public function brach_edit_saveOp(){
		$lang	= Uk86Language::uk86_getLangContent();
		if ($_POST['gc_show'] == '-1'){
			uk86_showMessage($lang['goods_class_batch_edit_succ'],'index.php?act=flea_class&op=flea_class');
		}
		if ($_POST['form_submit'] == 'ok'){
			$model_class = Model('flea_class');
			...
			if (is_array($array)){
				...
				uk86_showMessage($lang['goods_class_batch_edit_succ']);
			}else {
				uk86_showMessage($lang['goods_class_batch_edit_wrong_content']);
			}
		}else {
			uk86_showMessage($lang['goods_class_batch_edit_wrong_content']);
		}
		...
		if ($_POST['form_submit'] == 'ok'){
			...
			if ($error != ''){
				...
				if ($result){
					...
					uk86_showMessage($lang['goods_class_add_succ'],$url);
				}else {
					uk86_showMessage($lang['goods_class_add_fail']);
				}
			}
		}
		/**
		...
		if ($_POST['form_submit'] == 'ok'){
			...
			if ($error != ''){
				...
				if ($result){
					...
					uk86_showMessage($lang['goods_class_batch_edit_ok'],$url);
				}else {
	...
	public function goods_class_importOp(){
		...
		if ($_POST['form_submit'] == 'ok'){
			...
			if (!empty($_FILES['csv']) && !empty($_FILES['csv']['name']) && $file_type == 'csv'){
				 ...
				 * 重新生成缓存
				 */
				uk86_showMessage($lang['goods_class_import_succ'],'index.php?act=flea_class&op=flea_class');
			}else {
	...
	public function goods_class_delOp(){
		...
		if (intval($_GET['gc_id']) > 0){
			...
			uk86_showMessage($lang['goods_class_index_del_succ'],'index.php?act=flea_class&op=flea_class');
		}else {
			uk86_showMessage($lang['goods_class_index_choose_del'],'index.php?act=flea_class&op=flea_class');
		}
	}
	/**
	 * ajax操作
