...
class brandControl extends SystemControl{
	...
	public function brand_delOp(){
		if (intval($_GET['del_brand_id']) > 0){
			...
			uk86_showMessage(L('nc_common_del_succ'),'index.php?act=brand&op=brand');
		}else {
			$this->log(L('nc_delete,brand_index_brand').'[ID:'.intval($_GET['del_brand_id']).']',0);
			uk86_showMessage(L('nc_common_del_fail'),'index.php?act=brand&op=brand');
		}
	}
			...
			if (!empty($_POST['del_id'])){
				switch ($_POST['type']){
					case 'pass':
						...
						uk86_showMessage($lang['brand_apply_passed']);
						break;
					case 'refuse':
						//删除该品牌
						$brandid_array = array();
						...
						break;
						default:
							uk86_showMessage($lang['brand_apply_invalid_argument']);
				}
			}else {
	 ...
	 * 设置品牌购买价格
	 */
	public function brand_setPriceOp(){
		if (uk86_chksubmit()){
			
			$data['name'] = 'default_brand_price';
			$data['value'] = $_POST['default_brand_price'];
//			var_dump(C('ccc')!==NULL);die;
			if(C('default_brand_price')!==NULL){
				$tmp[$data['name']] = $data['value'];
				$r = Model('setting')->updateSetting($tmp);
			}else{
				$r = Model('setting')->insertSetting($data);
			}
			
			if($r){
				showDialog('价格修改成功','reload','succ');
			} else{
				showDialog('价格修改失败','reload');
			}					
		}
		$default_brand_price = C('default_brand_price');
		Tpl::output('default_brand_price',$default_brand_price);	
		Tpl::showpage('brand.setPrice');
	}
	 ...
	 * @author wangjiang
	 */
	public function brand_apply_editOp(){
		$lang	= Uk86Language::uk86_getLangContent();
		$model_brand = Model('brand');
		...
		if (uk86_chksubmit()){
			...
			$obj_validate->validateparam = array(
				array("input"=>$_POST["brand_name"], "require"=>"true", "message"=>$lang['brand_add_name_null']),
				array("input"=>$_POST["brand_initial"], "require"=>"true", "message"=>'请填写首字母'),
				array("input"=>$_POST["order_amount"], "require"=>"true", 'validator'=>'Number', "message"=>"实付价格，字段格式不正确"),
			);
			$error = $obj_validate->uk86_validate();
			if ($error != ''){
				uk86_showMessage($error);
			}else {
				$model_brand->beginTransaction();
			    $brand_info = $model_brand->getBrandInfo(array('brand_id' => intval($_POST['brand_id'])));
			    $where = array();
				$where['brand_id'] = intval($_POST['brand_id']);
				$update_array = array();
				...
				$update_array['brand_recommend'] = intval($_POST['brand_recommend']);
				$update_array['brand_sort'] = intval($_POST['brand_sort']);
				$update_array['brand_apply'] = 1; //设置审核通过
				$update_array['show_type'] = intval($_POST['show_type'])==1?1:0;
				$result = $model_brand->editBrand($where, $update_array);
				if ($result){
					//修改订单价格
					$condition['order_id']= $brand_info['order_id'];
					$data['goods_price'] = $_POST['order_amount'];
					$r = Model('pm_order')-> editOrder($data,$condition);
					if($r){
						$model_brand->commit();
						if (!empty($_POST['brand_pic']) && !empty($brand_info['brand_pic'])){
							@unlink(BASE_UPLOAD_PATH.DS.ATTACH_BRAND.DS.$brand_info['brand_pic']);
						...
						$url = array(
//							array(
//								'url'=>'index.php?act=brand&op=brand_apply_edit&brand_id='.intval($_POST['brand_id']),
//								'msg'=>$lang['brand_edit_again'],
//							),
							array(
								'url'=>'index.php?act=brand&op=brand',
						...
						$this->log(L('nc_edit,brand_index_brand').'['.$_POST['brand_name'].']',1);
						uk86_showMessage($lang['brand_apply_edit'],$url);
					}else{
						$model_brand->rollback();
						$this->log(L('nc_edit,brand_index_brand').'['.$_POST['brand_name'].']',0);
						uk86_showMessage($lang['brand_apply_fail']);
					}
				}else {
					$model_brand->rollback();
					$this->log(L('nc_edit,brand_index_brand').'['.$_POST['brand_name'].']',0);
		...
		$brand_info = $model_brand->getBrandInfo(array('brand_id' => intval($_GET['brand_id'])));
		$brand_info['goods_price'] = $this->_getGoodsPriceByPmOrderId($brand_info['order_id']);
		if (empty($brand_info)){
			uk86_showMessage($lang['param_error']);
		}
		Tpl::output('brand_array',$brand_info);
		...
		Tpl::showpage('brand.apply_edit');
	}
	
	private function _getGoodsPriceByPmOrderId($order_id){
		if(empty($order_id)) return C('default_brand_price');
		
		$pm_order = Model('pm_order')->getOrderInfo(array("order_id"=>$order_id),"goods_price");
		if($pm_order){
			return (int)$pm_order['goods_price'];
		}else{
			return C('default_brand_price');
		}
	}
			...
			switch ($_GET['state']){
				case 'pass':
					...
					if ($result === true){
						...
						uk86_showMessage(Uk86Language::uk86_get('brand_apply_pass'));
					}else {
						$this->log(L('brand_apply_pass').'[ID:'.intval($_GET['brand_id']).']',0);
						uk86_showMessage(Uk86Language::uk86_get('brand_apply_fail'));
					}
					break;
				case 'refuse':
					// 删除
					...
					break;
					default:
						uk86_showMessage(Uk86Language::uk86_get('brand_apply_paramerror'));
			}
		}else {
			$this->log(L('nc_delete,brand_index_brand').'[ID:'.intval($_GET['brand_id']).']',0);
			uk86_showMessage(Uk86Language::uk86_get('brand_apply_brandparamerror'));
		}
	}
	...
	private function createExcel($data = array()){
		...
		$excel_obj->uk86_addWorksheet($excel_obj->uk86_charset(L('exp_brand'),CHARSET));
		$excel_obj->uk86_generateXML($excel_obj->uk86_charset(L('exp_brand'),CHARSET).$_GET['curpage'].'-'.date('Y-m-d-H',time()));
	}
