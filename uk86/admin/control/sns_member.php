...
class sns_memberControl extends SystemControl{
	public function __construct(){
		parent::__construct();
		Uk86Language::uk86_read('sns_member');
	}
	...
	public function tag_addOp(){
		if(uk86_chksubmit()){
			...
			$obj_validate->validateparam = array(
					array("input"=>$_POST["membertag_name"], "require"=>"true", "message"=>Uk86Language::uk86_get('sns_member_tag_name_not_null')),
					array("input"=>$_POST["membertag_sort"], "require"=>"true", 'validator'=>'Number', "message"=>Uk86Language::uk86_get('sns_member_tag_sort_is_number')),
			);
			...
			if($error != ''){
				...
				$upload->uk86_set('default_dir',ATTACH_PATH.'/membertag');
				$input = '';
				if (!empty($_FILES['membertag_img']['name'])){
					$result = $upload->uk86_upfile('membertag_img');
					if (!$result){
				...
				if ($result){
					$url = array(
							array(
									...
									'msg'=>Uk86Language::uk86_get('sns_member_add_once_more'),
							),
							array(
									'url'=>'index.php?act=sns_member&op=member_tag',
									'msg'=>Uk86Language::uk86_get('sns_memner_return_list'),
							)
					);
					$this->log(L('nc_add,sns_member_tag').'['.$_POST['membertag_name'].']',1);
					uk86_showMessage(Uk86Language::uk86_get('nc_common_op_succ'),$url);
				}else {
					uk86_showMessage(Uk86Language::uk86_get('nc_common_op_fail'));
				}
			}
		...
		if(uk86_chksubmit()){
			...
			if($error != ''){
				...
				if ($_POST['old_membertag_name'] != ''){
					$upload->uk86_set('file_name', $_POST['old_membertag_name']);
				}
				...
				if ($result){
					...
					uk86_showMessage(Uk86Language::uk86_get('nc_common_op_succ'),'index.php?act=sns_member&op=member_tag');
				}else {
					uk86_showMessage(Uk86Language::uk86_get('nc_common_op_fail'));
				}
			}
