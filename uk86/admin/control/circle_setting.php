...
class circle_settingControl extends SystemControl{
	...
	public function indexOp(){
		...
		if(uk86_chksubmit()){
			...
			if (!empty($_FILES['c_logo']['name'])){
				...
				$upload->uk86_set('default_dir',ATTACH_CIRCLE);
				$result = $upload->uk86_upfile('c_logo');
				if ($result){
    ...
    public function superaddOp() {
        if (uk86_chksubmit()) {
            ...
            if ($result) {
                uk86_showMessage(L('nc_common_op_succ'), uk86_urlAdmin('circle_setting', 'super_list'));
            } else {
                uk86_showMessage(L('nc_common_op_fail'));
            }
        }
    ...
    public function super_listOp() {
        ...
        if (uk86_chksubmit()) {
            ...
            if (empty($id_array)) {
                uk86_showMessage(L('miss_argument'));
            }
    ...
    public function choose_superOp(){
        Tpl::output('search_url', uk86_urlAdmin('circle_setting', 'member_search'));
