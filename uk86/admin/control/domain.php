		...
		if (uk86_chksubmit()){
			...
			if (!empty($store_domain)){
				...
				if ($store_domain_count < $subdomain_length[0] || $store_domain_count > $subdomain_length[1]){
					uk86_showMessage(Uk86Language::uk86_get('store_domain_length_error').': '.$config_subdomain_length);
				}
				if (!preg_match('/^[\w-]+$/i',$store_domain)){//判断域名是否正确
					uk86_showMessage(Uk86Language::uk86_get('store_domain_invalid'));
				}
				...
				if (!empty($store_info) && ($store_id != $store_info['store_id'])){
					uk86_showMessage(Uk86Language::uk86_get('store_domain_exists'));
				}
				//判断二级域名是否为系统禁止
				...
				if(!empty($subdomain_reserved) && is_array($subdomain_reserved)){
						if (in_array($store_domain,$subdomain_reserved)){
							uk86_showMessage(Uk86Language::uk86_get('store_domain_sys'));
						}
				}
			...
			uk86_showMessage(Uk86Language::uk86_get('nc_common_save_succ'),'index.php?act=domain&op=store_domain_list');//保存成功
		}
