	...
	public function adminOp(){
		...
		if (uk86_chksubmit()){
			...
			if (@in_array(1,$_POST['del_id'])){
				uk86_showMessage(L('admin_index_not_allow_del'));
			}
	...
	public function admin_addOp(){
		if (uk86_chksubmit()){
			...
			if ($rs){
				...
				uk86_showMessage(L('nc_common_save_succ'),'index.php?act=admin&op=admin');
			}else {
				uk86_showMessage(L('nc_common_save_fail'));
			}
		}
	...
	public function admin_editOp(){
		if (uk86_chksubmit()){
			...
			if ($result){
				...
				uk86_showMessage(Uk86Language::uk86_get('admin_edit_success'),'index.php?act=admin&op=admin');
			}else{
				uk86_showMessage(Uk86Language::uk86_get('admin_edit_fail'),'index.php?act=admin&op=admin');
			}
		}else{
			//查询用户信息
			$admin_model = Model('admin');
			...
			if (!is_array($admininfo) || count($admininfo)<=0){
				uk86_showMessage(Uk86Language::uk86_get('admin_edit_admin_error'),'index.php?act=admin&op=admin');
			}
