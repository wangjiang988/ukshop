...
class snstraceControl extends SystemControl{
	public function __construct(){
		parent::__construct();
		Uk86Language::uk86_read('snstrace');
	}
		...
		if (!empty($tracelist)){
			foreach ($tracelist as $k=>$v){
				...
				if(!empty($v['trace_content'])){
					...
					//将收藏商品和店铺连接剔除
					$v['trace_content'] = str_replace(Uk86Language::uk86_get('admin_snstrace_collectgoods'), "", $v['trace_content']);
					$v['trace_content'] = str_replace(Uk86Language::uk86_get('admin_snstrace_collectstore'), "", $v['trace_content']);
				}
				$tracelist[$k] = $v;
			}
		}
	...
	public function tracedelOp(){
		$tid = $_POST['t_id'];
		if(empty($tid)){
			uk86_showMessage(Uk86Language::uk86_get('admin_snstrace_pleasechoose_del'),'index.php?act=snstrace&op=tracelist','','error');
		}
		$tid_str = implode("','",$tid);
		//删除动态
		$tracelog_model = Model('sns_tracelog');
		...
		if($result){
			...
			uk86_showMessage(Uk86Language::uk86_get('nc_common_del_succ'),'index.php?act=snstrace&op=tracelist','','succ');
		}else{
			uk86_showMessage(Uk86Language::uk86_get('nc_common_del_fail'),'index.php?act=snstrace&op=tracelist','','error');
		}
	}
	...
	public function traceeditOp(){
		$tid = $_POST['t_id'];
		if(empty($tid)){
			uk86_showMessage(Uk86Language::uk86_get('admin_snstrace_pleasechoose_edit'),'index.php?act=snstrace&op=tracelist','','error');
		}
		$tid_str = implode("','",$tid);
		$type = $_GET['type'];
		//删除动态
		...
		if($result){
			...
			uk86_showMessage(Uk86Language::uk86_get('nc_common_op_succ'),'index.php?act=snstrace&op=tracelist','','succ');
		}else{
			uk86_showMessage(Uk86Language::uk86_get('nc_common_op_fail'),'index.php?act=snstrace&op=tracelist','','error');
		}
	}
	...
	public function commentdelOp(){
		$cid = $_POST['c_id'];
		if(empty($cid)){
			uk86_showMessage(Uk86Language::uk86_get('admin_snstrace_pleasechoose_del'),'index.php?act=snstrace&op=commentlist','','error');
		}
		$cid_str = implode("','",$cid);
		//删除评论
		$comment_model = Model('sns_comment');
		...
		if($result){
			...
			uk86_showMessage(Uk86Language::uk86_get('nc_common_del_succ'),'index.php?act=snstrace&op=commentlist','','succ');
		}else{
			uk86_showMessage(Uk86Language::uk86_get('nc_common_del_fail'),'index.php?act=snstrace&op=commentlist','','error');
		}
	}
	...
	public function commenteditOp(){
		$cid = $_POST['c_id'];
		if(empty($cid)){
			uk86_showMessage(Uk86Language::uk86_get('admin_snstrace_pleasechoose_edit'),'index.php?act=snstrace&op=commentlist','','error');
		}
		$cid_str = implode("','",$cid);
		$type = $_GET['type'];
		//删除动态
		...
		if($result){
			...
			uk86_showMessage(Uk86Language::uk86_get('nc_common_op_succ'),'index.php?act=snstrace&op=commentlist','','succ');
		}else{
			uk86_showMessage(Uk86Language::uk86_get('nc_common_op_fail'),'index.php?act=snstrace&op=commentlist','','error');
		}
	}
}
?>
