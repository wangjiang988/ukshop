...
class advControl extends SystemControl{
		...
		if(!uk86_chksubmit()){
			...
			if ($error != ''){
			    ...
			    if ($result&&$result2){
					...
					uk86_showMessage($lang['adv_add_succ'],'index.php?act=adv&op=adv&ap_id='.$_POST['ap_id']);
				}else {
					uk86_showMessage($lang['adv_add_fail']);
				}
		}
	  }
	}
	...
	public function ap_manageOp(){
		...
		if (uk86_chksubmit()){
			...
			uk86_showMessage($lang['ap_del_succ'],$url);
		}
		/**
		 * 显示广告位管理界面
		 */
	 ...
	 * 旗舰店-管理广告位
	 */
	public function brand_ap_manageOp(){
		$lang = Uk86Language::uk86_getLangContent();
		$adv  = Model('adv');
		/**
		 * 多选删除广告位
		...
		$ap_list  = $adv->getBrandApList($condition,$page,$orderby);
		$adv_list = $adv->getList();
		Tpl::output('ap_list',$ap_list);
		Tpl::output('adv_list',$adv_list);
		Tpl::output('page',$page->uk86_show());
		Tpl::showpage('brand_ap_manage');
	}
		...
		if(!uk86_chksubmit()){
			...
			if ($error != ''){
			    ...
			    if($_FILES['default_pic']['name'] != ''){
			    	$upload->uk86_set('default_dir',ATTACH_ADV);
					$result = $upload->uk86_upfile('default_pic');
			    	if (!$result){
						uk86_showMessage($upload->error,'','','error');
					}
			    ...
			    if ($result){
					...
					uk86_showMessage($lang['ap_change_succ'],$_POST['ref_url']);
				}else {
					uk86_showMessage($lang['ap_change_fail'], $url);
				}
			}
		}
	}
	...
	public function ap_addOp(){
		if($_POST['form_submit'] != 'ok'){
			...
			if ($error != ''){
				...
				if(isset($_POST['adv_type'])){
					$insert_array['adv_type'] = $_POST['adv_type'];
				}
			    $result  = $adv->ap_add($insert_array);

				if ($result){
					$this->log(L('ap_add_succ').'['.$_POST["ap_name"].']',null);
					uk86_showMessage($lang['ap_add_succ'],'index.php?act=adv&op=ap_manage','html','succ',1,4000);
				}else {
					uk86_showMessage($lang['ap_add_fail']);
				}
			}
		}
	}
       ...
       /**
		 ...
		 */
		    ...
		    if (!$result){
					...
					uk86_showMessage($lang['adv_del_fail'],$url);die;
				}else{
					$this->log(L('adv_del_succ').'['.intval($_GET['adv_id']).']',null);
					uk86_showMessage($lang['adv_del_succ'],$url);die;
				}
		 /**
		  * 多选删除多个广告
		  */
