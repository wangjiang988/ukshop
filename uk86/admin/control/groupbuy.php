...
class groupbuyControl extends SystemControl{
    ...
    public function __construct(){
        parent::__construct();
        Uk86Language::uk86_read('groupbuy');
        ...
        if (C('groupbuy_allow') != 1){
 			$url = array(
 				array(
					...
					'msg'=>Uk86Language::uk86_get('close'),
				),
				array(
					'url'=>'index.php?act=groupbuy&op=groupbuy_template_list&groupbuy_open=1',
					'msg'=>Uk86Language::uk86_get('open'),
				),
			);
			uk86_showMessage(Uk86Language::uk86_get('admin_groupbuy_unavailable'),$url,'html','succ',1,6000);
        }
    }
    ...
    public function groupbuy_review_passOp(){
       ...
       $result = $model_groupbuy->reviewPassGroupbuy($groupbuy_id);
        if($result) {
        	...
        	//修改店铺动态状态
        	$store_info = Model()->table('groupbuy')->where(array('groupbuy_id' => $groupbuy_id))->field('store_id')->find();
        	$store_snstracelog = Model()->table('store_sns_tracelog')->where(array('strace_storeid' => $store_info['store_id'], 'strace_type' => 8))->field('strace_id, strace_goodsdata')->select();
        	foreach($store_snstracelog as $k => $v){
        		$groupbuy_info = json_decode($v['strace_goodsdata'], true);
        		if($groupbuy_info['group_id'] == $groupbuy_id && !empty($groupbuy_info['group_id'])){
        			if($groupbuy_info['state'] == 20){
        				break;
        			}
        			$groupbuy_info['state'] = 20;
        			$update['strace_goodsdata'] = json_encode($groupbuy_info);
        			Model()->table('store_sns_tracelog')->where(array('strace_id' => $v['strace_id']))->update($update);
        			break;
        		}
        	}
    ...
    public function groupbuy_review_failOp(){
        ...
        $model_groupbuy = Model('groupbuy');
        //  可能需要考虑注入问题
        $result = $model_groupbuy->reviewFailGroupbuy($groupbuy_id, $_POST['reject_reason']);
        if($result) {
    ...
    public function groupbuy_cancelOp() {
        ...
        if($result) {
        	...
        	foreach($store_snstracelog as $k => $v){
        		$groupbuy_info = json_decode($v['strace_goodsdata'], true);
        		if($groupbuy_info['group_id'] == $groupbuy_id && !empty($groupbuy_info['group_id'])){
        			if($groupbuy_info['state'] == 10){
        				break;
        			}
        			$groupbuy_info['state'] = 10;
        			$update['strace_goodsdata'] = json_encode($groupbuy_info);
        			Model()->table('store_sns_tracelog')->where(array('strace_id' => $v['strace_id']))->update($update);
        			break;
        		}
    ...
    public function groupbuy_delOp(){
        ...
        if($result) {
        	...
        	foreach($store_snstracelog as $k => $v){
        		$groupbuy_info = json_decode($v['strace_goodsdata'], true);
        		if($groupbuy_info['group_id'] == $groupbuy_id && !empty($groupbuy_info['group_id'])){
        			Model()->table('store_sns_tracelog')->where(array('strace_id' => $v['strace_id']))->delete();
        			break;
        		}
        	}
    ...
    public function class_saveOp() {
        ...
        if(empty($param['class_name'])) {
            uk86_showMessage(Uk86Language::uk86_get('class_name_error'),'');
        }
        ...
        if(empty($class_id)) {
            ...
            if($model_groupbuy_class->save($param)) {
                ...
                uk86_showMessage(Uk86Language::uk86_get('groupbuy_class_add_success'),'index.php?act=groupbuy&op=class_list');
            }
            else {
                uk86_showMessage(Uk86Language::uk86_get('groupbuy_class_add_fail'),'index.php?act=groupbuy&op=class_list');
            }
        }
        else {
            //编辑
            if($model_groupbuy_class->update($param,array('class_id'=>$class_id))) {
                ...
                uk86_showMessage(Uk86Language::uk86_get('groupbuy_class_edit_success'),'index.php?act=groupbuy&op=class_list');
            }
            else {
                uk86_showMessage(Uk86Language::uk86_get('groupbuy_class_edit_fail'),'index.php?act=groupbuy&op=class_list');
            }
        }
    ...
    public function class_dropOp() {
        ...
        if($model_groupbuy_class->drop($param)) {
            ...
            uk86_showMessage(Uk86Language::uk86_get('groupbuy_class_drop_success'),'');
        }
        else {
            uk86_showMessage(Uk86Language::uk86_get('groupbuy_class_drop_fail'),'');
        }
    ...
    public function price_saveOp() {
        ...
        if(empty($param['range_name'])) {
            uk86_showMessage(Uk86Language::uk86_get('range_name_error'),'');
        }
        ...
        if(empty($range_id)) {
            //新增
            if($model->save($param)) {
            	uk86_dkcache('groupbuy_price');
            	$this->log(L('groupbuy_price_range_add_success').'['.$_POST['range_name'].']',null);
                uk86_showMessage(Uk86Language::uk86_get('groupbuy_price_range_add_success'),'index.php?act=groupbuy&op=price_list');
            }
            else {
                uk86_showMessage(Uk86Language::uk86_get('groupbuy_price_range_add_fail'),'index.php?act=groupbuy&op=price_list');
            }
        }
        else {
            //编辑
            if($model->update($param,array('range_id'=>$range_id))) {
                ...
                uk86_showMessage(Uk86Language::uk86_get('groupbuy_price_range_edit_success'),'index.php?act=groupbuy&op=price_list');
            }
            else {
                uk86_showMessage(Uk86Language::uk86_get('groupbuy_price_range_edit_fail'),'index.php?act=groupbuy&op=price_list');
            }
        }
    ...
    public function price_dropOp() {
        ...
        if($model->drop($param)) {
            ...
            uk86_showMessage(Uk86Language::uk86_get('groupbuy_price_range_drop_success'),'');
        }
        else {
            uk86_showMessage(Uk86Language::uk86_get('groupbuy_price_range_drop_fail'),'');
        }
    }
    ...
    private function show_menu($menu_key) {
        $menu_array = array(
            ...
            'class_list'=>array('menu_type'=>'link','menu_name'=>Uk86Language::uk86_get('groupbuy_class_list'),'menu_url'=>'index.php?act=groupbuy&op=class_list'),
            'class_add'=>array('menu_type'=>'link','menu_name'=>Uk86Language::uk86_get('groupbuy_class_add'),'menu_url'=>'index.php?act=groupbuy&op=class_add'),
            'price_list'=>array('menu_type'=>'link','menu_name'=>Uk86Language::uk86_get('groupbuy_price_list'),'menu_url'=>'index.php?act=groupbuy&op=price_list'),
            'price_add'=>array('menu_type'=>'link','menu_name'=>Uk86Language::uk86_get('groupbuy_price_add'),'menu_url'=>'index.php?act=groupbuy&op=price_add'),
            'price_edit'=>array('menu_type'=>'link','menu_name'=>Uk86Language::uk86_get('groupbuy_price_edit'),'menu_url'=>'index.php?act=groupbuy&op=price_edit'),
            'groupbuy_setting'=>array('menu_type'=>'link','menu_name'=>'设置','menu_url'=>uk86_urlAdmin('groupbuy', 'groupbuy_setting')),
            'slider'=>array('menu_type'=>'link','menu_name'=>'幻灯片管理','menu_url'=>uk86_urlAdmin('groupbuy', 'slider')),
        );
        switch ($menu_key) {
            case 'class_add':
