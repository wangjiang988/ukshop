...
class mb_adControl extends SystemControl{
	...
	public function mb_ad_delOp(){
		...
		if ($link_id > 0){
			...
			uk86_showMessage(L('link_index_del_succ'),'index.php?act=mb_ad&op=mb_ad_list');
		}else {
			uk86_showMessage(L('link_index_choose_del'),'index.php?act=mb_ad&op=mb_ad_list');
		}
	}
	...
	public function mb_ad_addOp(){
		...
		if ($count > 5){
			uk86_showMessage(L('link_add_count_limit'));
		}
		if ($_POST['form_submit'] == 'ok'){
			...
			if ($error != ''){
				...
				if ($result){
					...
					uk86_showMessage(L('link_add_succ'),$url);
				}else {
					uk86_showMessage(L('link_add_fail'));
				}
			}
		}
