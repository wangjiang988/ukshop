...
class vr_billControl extends SystemControl{
	...
	public function bill_checkOp(){
		...
		if ($update){
			...
			uk86_showMessage('审核成功，账单进入付款环节');
		}else{
		    $this->log('审核账单，账单号：'.$_GET['ob_no'],0);
			uk86_showMessage('审核失败','','html','error');
		}
	}
	...
	public function bill_payOp(){
		...
		if (uk86_chksubmit()){
			...
			if ($update){
				...
				uk86_showMessage('保存成功','index.php?act=vr_bill&op=show_statis&os_month='.$bill_info['os_month']);
			}else{
			    $this->log('账单付款,账单号：'.$_GET['ob_no'],1);
	...
	public function export_billOp(){
		...
		foreach ($data as $k => $v) {
		    ...
		    $export_data[$k+1][] = uk86_billState($v['ob_state']);
		...
		$csv->filename = $csv->uk86_charset('账单-',CHARSET).$_GET['os_month'];
	...
	public function export_orderOp(){
		...
		$csv->filename = $csv->uk86_charset($file_name.'-',CHARSET).$_GET['ob_no'];
		$csv->uk86_export($export_data);
	}
