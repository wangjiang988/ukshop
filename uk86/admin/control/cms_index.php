...
class cms_indexControl extends SystemControl{
	...
	public function cms_indexOp() {
        ...
        $module_list = $model_module->getList(TRUE, 4);
    ...
    public function update_page_module_titleOp() {
        ...
        $obj_validate->validateparam = array(
            array('input'=>$new_title,'require'=>'true',"validator"=>"Length","min"=>"1","max"=>"20",'message'=>Uk86Language::uk86_get('class_name_error')),
        );
    ...
    public function cms_index_buildOp() {
        ...
        if(!is_dir($html_path)){
            if (!@mkdir($html_path, 0755)){
                $data = array();
                $data['result'] = FALSE;
                $data['message'] = Uk86Language::uk86_get('cms_index_build_fail');
        ...
        if($result) {
            $this->log(Uk86Language::uk86_get('cms_log_index_build'), 1);
            $data['message'] = Uk86Language::uk86_get('cms_index_build_success');
        } else {
            $this->log(Uk86Language::uk86_get('cms_log_index_build'), 0);
            $data['message'] = Uk86Language::uk86_get('cms_index_build_fail');
        }
        echo json_encode($data);die;
    }
    ...
    public function save_page_moduleOp() {
        ...
        if($result) {
            $this->log(Uk86Language::uk86_get('cms_log_index_edit').$_POST['module_id'], 1);
            uk86_showMessage(Uk86Language::uk86_get('nc_common_save_succ'), 'index.php?act=cms_index&op=cms_index');
        } else {
            $this->log(Uk86Language::uk86_get('cms_log_index_edit').$_POST['module_id'], 0);
            uk86_showMessage(Uk86Language::uk86_get('nc_common_save_fail'), '', '','error');
        }
    }
    ...
    private function image_drop($image_name) {
        $file = uk86_getCMSIndexImagePath($image_name);
        if(is_file($file)) {
            unlink($file);
        }
    }
    ...
    private function show_menu($menu_key) {
        $menu_array = array(
            'cms_index'=>array('menu_type'=>'link','menu_name'=>Uk86Language::uk86_get('nc_manage'),'menu_url'=>'index.php?act=cms_index&op=cms_index'),
        );
        if($menu_key == 'module_edit') {
            $menu_array['module_edit'] = array('menu_type'=>'link','menu_name'=>Uk86Language::uk86_get('nc_edit'),'menu_url'=>'###');
        }
