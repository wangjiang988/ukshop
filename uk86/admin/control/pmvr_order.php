...
class pmvr_orderControl extends SystemControl{
    /**
     * 每次导出订单数量
     * @var int
     */
	...
	public function indexOp(){
        ...
        $order_list	= $model_pm_order->getOrderList($condition,30);

        foreach ($order_list as $k => $order_info) {
            //显示取消订单
            $order_list[$k]['if_cancel'] = $model_pm_order->getOrderOperateState('system_cancel',$order_info);
            //显示收到货款
            $order_list[$k]['if_system_receive_pay'] = $model_pm_order->getOrderOperateState('system_receive_pay',$order_info);
        }
        ...
        Tpl::output('show_page',$model_pm_order->showpage());
        Tpl::showpage('pmvr_order.index');
	}
	...
	public function change_stateOp() {
	    ...
	    $order_info	= $model_pm_order->getOrderInfo($condition);
	    ...
	    if(!$result['state']) {
	        uk86_showMessage($result['msg'],$_POST['ref_url'],'html','error');
	    } else {
	        uk86_showMessage($result['msg'],$_POST['ref_url']);
	    }
	}
	...
	private function _order_cancel($order_info) {
	    ...
	    $if_allow = $model_pm_order->getOrderOperateState('system_cancel',$order_info);
	    if (!$if_allow) {
	        return uk86_callback(false,'无权操作');
	    }
	    $this->log('关闭了虚拟订单,'.L('order_number').':'.$order_info['order_sn'],1);
	    return $logic_pm_order->changeOrderStateCancel($order_info,'store', '管理员关闭虚拟订单');
	}
	...
	public function show_orderOp(){
	    ...
	    if($order_id <= 0 ){
	        uk86_showMessage(L('miss_order_number'));
	    }
        $model_pm_order	= Model('pm_order');
        $order_info	= $model_pm_order->getOrderInfo(array('order_id'=>$order_id));
        if (empty($order_info)) {
            uk86_showMessage('订单不存在','','html','error');
        }

//      //取兑换码列表
//      $vr_code_list = $model_pm_order->getOrderCodeList(array('order_id' => $order_info['order_id']));
//      $order_info['extend_vr_order_code'] = $vr_code_list;

        //显示取消订单
        $order_info['if_cancel'] = $model_pm_order->getOrderOperateState('buyer_cancel',$order_info);

        //显示订单进行步骤
        $order_info['step_list'] = $model_pm_order->getOrderStep($order_info);
	...
	public function export_step1Op(){
		...
		if (!is_numeric($_GET['curpage'])){
			$count = $model_pm_order->getOrderCount($condition);
			$array = array();
			if ($count > self::EXPORT_SIZE ){	//显示下载链接
				...
				$data = $model_pm_order->getOrderList($condition,'','*','order_id desc',self::EXPORT_SIZE);
				$this->createExcel($data);
			}
		}else{	//下载
			$limit1 = ($_GET['curpage']-1) * self::EXPORT_SIZE;
			$limit2 = self::EXPORT_SIZE;
			$data = $model_pm_order->getOrderList($condition,'','*','order_id desc',"{$limit1},{$limit2}");
			$this->createExcel($data);
		}
	}
