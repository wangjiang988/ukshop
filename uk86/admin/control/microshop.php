...
//	}
	...
	public function manage_saveOp() {
        ...
        if(!empty($_FILES['microshop_logo']['name'])) {
            ...
            $upload->uk86_set('default_dir',ATTACH_MICROSHOP);
            $result = $upload->uk86_upfile('microshop_logo');
            if(!$result) {
        ...
        if(!empty($_FILES['microshop_header_pic']['name'])) {
            $upload	= new Uk86UploadFile();
            $upload->uk86_set('default_dir',ATTACH_MICROSHOP);
            $result = $upload->uk86_upfile('microshop_header_pic');
            if(!$result) {
                uk86_showMessage($upload->error);
            }
    ...
    private function class_edit($type) {
        ...
        if(empty($class_id)) {
            uk86_showMessage(Uk86Language::uk86_get('param_error'),'','','error');
        }
        $model_class = Model("micro_{$type}_class");
        $condition = array();
    ...
    private function class_save($type) {
        ...
        $validate_array = array(
            array('input'=>$_POST['class_name'],'require'=>'true',"validator"=>"Length","min"=>"1","max"=>"10",'message'=>Uk86Language::uk86_get('class_name_error')),
            array('input'=>$_POST['class_sort'],'require'=>'true','validator'=>'Range','min'=>0,'max'=>255,'message'=>Uk86Language::uk86_get('class_sort_error')),
        );
        if($type == 'goods') {
            $validate_array[] = array('input'=>$_POST['class_parent_id'],'require'=>'true','validator'=>'Number','message'=>Uk86Language::uk86_get('parent_id_error'));
        }
        $obj_validate->validateparam = $validate_array;
        $error = $obj_validate->uk86_validate();
        if ($error != ''){
            uk86_showMessage(Uk86Language::uk86_get('error').$error,'','','error');
        }
        ...
        if(!empty($_FILES['class_image']['name'])) {
            $upload	= new Uk86UploadFile();
            $upload->uk86_set('default_dir',ATTACH_MICROSHOP);
            $result = $upload->uk86_upfile('class_image');
            if(!$result) {
                uk86_showMessage($upload->error);
            }
        ...
        if($result) {
            uk86_showMessage(Uk86Language::uk86_get('class_add_success'),"index.php?act=microshop&op={$type}class_list");
        } else {
            uk86_showMessage(Uk86Language::uk86_get('class_add_fail'),"index.php?act=microshop&op={$type}class_list",'','error');
        }
    ...
    private function update_class_sort($type) {
        if(intval($_GET['id']) <= 0) {
            echo json_encode(array('result'=>FALSE,'message'=>Uk86Language::uk86_get('param_error')));
            die;
        }
        $new_sort = intval($_GET['value']);
        if ($new_sort > 255){
            echo json_encode(array('result'=>FALSE,'message'=>Uk86Language::uk86_get('class_sort_error')));
            die;
        } else {
            $model_class = Model("micro_{$type}_class");
            ...
            if($result) {
                ...
                echo json_encode(array('result'=>FALSE,'message'=>Uk86Language::uk86_get('class_add_fail')));
                die;
            }
        }
    }
    ...
    private function update_class_name($type) {
        ...
        $obj_validate->validateparam = array(
            array('input'=>$new_name,'require'=>'true',"validator"=>"Length","min"=>"1","max"=>"10",'message'=>Uk86Language::uk86_get('class_name_error')),
        );
        $error = $obj_validate->uk86_validate();
        if ($error != ''){
            echo json_encode(array('result'=>FALSE,'message'=>Uk86Language::uk86_get('class_name_error')));
            die;
		} else {
            $model_class = Model("micro_{$type}_class");
     ...
     **/
     public function goodsclass_dropOp() {
        ...
        if($result) {
            uk86_showMessage(Uk86Language::uk86_get('class_drop_success'),'');
        } else {
            uk86_showMessage(Uk86Language::uk86_get('class_drop_fail'),'','','error');
        }
    ...
    public function goodsclass_binding_saveOp() {
        ...
        if($result) {
            uk86_showMessage(Uk86Language::uk86_get('goods_relation_save_success'),self::MICROSHOP_CLASS_LIST);
        } else {
            uk86_showMessage(Uk86Language::uk86_get('goods_relation_save_fail'),self::MICROSHOP_CLASS_LIST,'','error');
        }
    }
    ...
    public function goodsclass_defaultOp() {
        ...
        if($result) {
            ...
            uk86_showMessage(Uk86Language::uk86_get('nc_common_op_fail'),'','','error');
        }
    }
    ...
    private function update_microshop_sort($model_name,$key_name) {
        ...
        $new_sort = intval($_GET['value']);
        if ($new_sort > 255){
            echo json_encode(array('result'=>FALSE,'message'=>Uk86Language::uk86_get('microshop_sort_error')));
            die;
        } else {
            ...
            if($result) {
                ...
                echo json_encode(array('result'=>FALSE,'message'=>Uk86Language::uk86_get('nc_common_op_fail')));
                die;
            }
        }
    }
    ...
    public function adv_saveOp() {
        $obj_validate = new Uk86Validate();
        $validate_array = array(
            array('input'=>$_POST['adv_sort'],'require'=>'true','validator'=>'Range','min'=>0,'max'=>255,'message'=>Uk86Language::uk86_get('class_sort_error')),
        );
        $obj_validate->validateparam = $validate_array;
        $error = $obj_validate->uk86_validate();
        if ($error != ''){
        ...
        if(!empty($_FILES['adv_image']['name'])) {
            $upload	= new Uk86UploadFile();
            $upload->uk86_set('default_dir',ATTACH_MICROSHOP.DS.'adv');
            $result = $upload->uk86_upfile('adv_image');
            if(!$result) {
                uk86_showMessage($upload->error);
            }
            ...
            if(empty($_POST['adv_id'])) {
                uk86_showMessage(Uk86Language::uk86_get('microshop_adv_image_error'),'','','error');
            }
        }
        ...
        if($result) {
            uk86_showMessage(Uk86Language::uk86_get('nc_common_save_succ'),"index.php?act=microshop&op=adv_manage");
        } else {
            uk86_showMessage(Uk86Language::uk86_get('nc_common_save_fail'),"index.php?act=microshop&op=adv_manage",'','error');
        }
    }
    ...
    private function get_adv_type_list() {
        $adv_type_list = array();
        $adv_type_list['index'] = Uk86Language::uk86_get('microshop_adv_type_index');
        $adv_type_list['store_list'] = Uk86Language::uk86_get('microshop_adv_type_store_list');
    ...
    private function get_channel_array() {
        $channel_array = array();
        $channel_array[self::GOODS_FLAG] = array('name'=>Uk86Language::uk86_get('nc_microshop_goods'),'key'=>'goods');
        $channel_array[self::PERSONAL_FLAG] = array('name'=>Uk86Language::uk86_get('nc_microshop_personal'),'key'=>'personal');
        $channel_array[self::STORE_FLAG] = array('name'=>Uk86Language::uk86_get('nc_microshop_store'),'key'=>'store');
    ...
    private function show_menu($menu_key) {
        $menu_array = array(
            "{$menu_key}"=>array('menu_type'=>'link','menu_name'=>Uk86Language::uk86_get('nc_manage'),'menu_url'=>'index.php?act=microshop&op='.$menu_key),
        );
    ...
    private function show_menu_goods_class($menu_key) {
        $menu_array = array(
            'goods_class_list'=>array('menu_type'=>'link','menu_name'=>Uk86Language::uk86_get('nc_manage'),'menu_url'=>'index.php?act=microshop&op=goodsclass_list'),
            'goods_class_add'=>array('menu_type'=>'link','menu_name'=>Uk86Language::uk86_get('nc_new'),'menu_url'=>'index.php?act=microshop&op=goodsclass_add'),
        );
        if($menu_key == 'goods_class_edit') {
            $menu_array['goods_class_edit'] = array('menu_type'=>'link','menu_name'=>Uk86Language::uk86_get('nc_edit'),'menu_url'=>'###');
        }
        if($menu_key == 'goods_class_binding') {
            $menu_array['goods_class_binding'] = array('menu_type'=>'link','menu_name'=>Uk86Language::uk86_get('microshop_goods_class_binding'),'menu_url'=>'###');
        }
        $menu_array[$menu_key]['menu_type'] = 'text';
        Tpl::output('menu',$menu_array);
    }
    ...
    private function show_menu_personal_class($menu_key) {
        $menu_array = array(
            'personal_class_list'=>array('menu_type'=>'link','menu_name'=>Uk86Language::uk86_get('nc_manage'),'menu_url'=>'index.php?act=microshop&op=personalclass_list'),
            'personal_class_add'=>array('menu_type'=>'link','menu_name'=>Uk86Language::uk86_get('nc_new'),'menu_url'=>'index.php?act=microshop&op=personalclass_add'),
        );
        if($menu_key == 'personal_class_edit') {
            $menu_array['personal_class_edit'] = array('menu_type'=>'link','menu_name'=>Uk86Language::uk86_get('nc_edit'),'menu_url'=>'###');
        }
        $menu_array[$menu_key]['menu_type'] = 'text';
        Tpl::output('menu',$menu_array);
    }
    ...
    private function show_menu_store($menu_key) {
        $menu_array = array(
            'store_manage'=>array('menu_type'=>'link','menu_name'=>Uk86Language::uk86_get('nc_manage'),'menu_url'=>'index.php?act=microshop&op=store_manage'),
            'store_add'=>array('menu_type'=>'link','menu_name'=>Uk86Language::uk86_get('nc_new'),'menu_url'=>'index.php?act=microshop&op=store_add'),
        );
        $menu_array[$menu_key]['menu_type'] = 'text';
        Tpl::output('menu',$menu_array);
    }
    ...
    private function show_menu_adv($menu_key) {
        $menu_array = array(
            'adv_manage'=>array('menu_type'=>'link','menu_name'=>Uk86Language::uk86_get('nc_manage'),'menu_url'=>'index.php?act=microshop&op=adv_manage'),
            'adv_add'=>array('menu_type'=>'link','menu_name'=>Uk86Language::uk86_get('nc_new'),'menu_url'=>'index.php?act=microshop&op=adv_add'),
        );

        if($menu_key == 'adv_edit') {
            $menu_array['adv_edit'] = array('menu_type'=>'link','menu_name'=>Uk86Language::uk86_get('nc_edit'),'menu_url'=>'###');
