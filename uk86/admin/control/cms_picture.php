...
class cms_pictureControl extends SystemControl{
    ...
    public function update_picture_sortOp() {
        ...
        if ($new_sort > 255){
            echo json_encode(array('result'=>FALSE,'message'=>Uk86Language::uk86_get('picture_sort_error')));
            die;
        } else {
    ...
    public function update_picture_clickOp() {
        if(intval($_GET['id']) <= 0 || intval($_GET['value']) < 0) {
            echo json_encode(array('result'=>FALSE,'message'=>Uk86Language::uk86_get('param_error')));die;
        }
        $model_class = Model("cms_picture");
     ...
     **/
     public function cms_picture_dropOp() {
        ...
        if($result) {
            $this->log(Uk86Language::uk86_get('cms_log_picture_drop').$picture_id, 1);
            uk86_showMessage(Uk86Language::uk86_get('nc_common_del_succ'),'');
        } else {
            $this->log(Uk86Language::uk86_get('cms_log_picture_drop').$picture_id, 0);
    ...
    private function get_picture_state_list() {
        $array = array();
        $array[self::ARTICLE_STATE_DRAFT] = array('text'=>Uk86Language::uk86_get('cms_text_draft'));
        $array[self::ARTICLE_STATE_VERIFY] = array('text'=>Uk86Language::uk86_get('cms_text_verify'));
        $array[self::ARTICLE_STATE_PUBLISHED] = array('text'=>Uk86Language::uk86_get('cms_text_published'));
        $array[self::ARTICLE_STATE_RECYCLE] = array('text'=>Uk86Language::uk86_get('cms_text_recycle'));
        return $array;
    }

    private function show_menu($menu_key) {
        $menu_array = array(
            'list'=>array('menu_type'=>'link','menu_name'=>Uk86Language::uk86_get('nc_list'),'menu_url'=>'index.php?act=cms_picture&op=cms_picture_list'),
            'list_verify'=>array('menu_type'=>'link','menu_name'=>Uk86Language::uk86_get('cms_article_list_verify'),'menu_url'=>'index.php?act=cms_picture&op=cms_picture_list_verify'),
            'list_published'=>array('menu_type'=>'link','menu_name'=>Uk86Language::uk86_get('cms_article_list_published'),'menu_url'=>'index.php?act=cms_picture&op=cms_picture_list_published'),
        );
