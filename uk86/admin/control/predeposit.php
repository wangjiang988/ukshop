...
class predepositControl extends SystemControl{
	...
	public function __construct(){
		parent::__construct();
		Uk86Language::uk86_read('predeposit');
	}
	...
	public function recharge_editOp(){
		$id = intval($_GET['id']);
		if ($id <= 0){
			uk86_showMessage(Uk86Language::uk86_get('admin_predeposit_parameter_error'),'index.php?act=predeposit&op=predeposit','','error');
		}
		//查询充值信息
		$model_pd = Model('predeposit');
		$condition = array();
		...
		if (empty($info)){
			uk86_showMessage(Uk86Language::uk86_get('admin_predeposit_record_error'),'index.php?act=predeposit&op=predeposit','','error');
		}
		if (!uk86_chksubmit()) {
		    //显示支付接口列表
		...
		if(!$payment_info || $payment_info['payment_code'] == 'offline' || $payment_info['payment_code'] == 'offline') {
		    uk86_showMessage(L('payment_index_sys_not_support'),'','html','error');
		}
		...
		try {
		    ...
		    if (!$state) {
		        throw Exception(Uk86Language::uk86_get('predeposit_payment_pay_fail'));
		    }
		    //变更会员预存款
		    $data = array();
		    ...
		    $model_pd->commit();
		    $this->log($log_msg,1);
		    uk86_showMessage(Uk86Language::uk86_get('admin_predeposit_recharge_edit_success'),'index.php?act=predeposit&op=predeposit');
		} catch (Exception $e) {
		    $model_pd->rollback();
		    $this->log($log_msg,0);
		    uk86_showMessage($e->getMessage(),'index.php?act=predeposit&op=predeposit','html','error');
		}
	}
	...
	public function recharge_delOp(){
		...
		if ($result){
			uk86_showMessage(Uk86Language::uk86_get('admin_predeposit_recharge_del_success'),'index.php?act=predeposit&op=predeposit');
		}else {
			uk86_showMessage(Uk86Language::uk86_get('admin_predeposit_recharge_del_fail'),'index.php?act=predeposit&op=predeposit','','error');
		}
	}
	...
	public function pd_cash_delOp(){
		...
		if ($pdc_id <= 0){
			uk86_showMessage(Uk86Language::uk86_get('admin_predeposit_parameter_error'),'index.php?act=predeposit&op=pd_cash_list','','error');
		}
		$model_pd = Model('predeposit');
		$condition = array();
		...
		try {
		    ...
		    if (!$result) {
		        throw new Exception(Uk86Language::uk86_get('admin_predeposit_cash_del_fail'));
		    }
		    //退还冻结的预存款
		    ...
		    $model_pd->commit();
			uk86_showMessage(Uk86Language::uk86_get('admin_predeposit_cash_del_success'),'index.php?act=predeposit&op=pd_cash_list');

		} catch (Exception $e) {
		    $model_pd->commit();
		    uk86_showMessage($e->getMessage(),'index.php?act=predeposit&op=pd_cash_list','html','error');
		}
	}
	...
	public function pd_cash_payOp(){
	    ...
	    if (!is_array($info) || count($info)<0){
	        uk86_showMessage(Uk86Language::uk86_get('admin_predeposit_record_error'),'index.php?act=predeposit&op=pd_cash_list','','error');
	    }
        ...
        try {
            ...
            if (!$result) {
                throw new Exception(Uk86Language::uk86_get('admin_predeposit_cash_edit_fail'));
            }
            //扣除冻结的预存款
            $data = array();
            $data['member_id'] = $member_info['member_id'];
            ...
            $model_pd->commit();
            $this->log($log_msg,1);
            uk86_showMessage(Uk86Language::uk86_get('admin_predeposit_cash_edit_success'),'index.php?act=predeposit&op=pd_cash_list');
        } catch (Exception $e) {
            $model_pd->rollback();
            $this->log($log_msg,0);
            uk86_showMessage($e->getMessage(),'index.php?act=predeposit&op=pd_cash_list','html','error');
	...
	private function createExcel($data = array()){
		...
		foreach ((array)$data as $k=>$v){
			...
			$tmp[] = array('format'=>'Number','data'=>uk86_ncPriceFormat($v['pdr_amount']));
		...
		$excel_obj->uk86_addWorksheet($excel_obj->uk86_charset(L('exp_yc_yckcz'),CHARSET));
		$excel_obj->uk86_generateXML($excel_obj->uk86_charset(L('exp_yc_yckcz'),CHARSET).$_GET['curpage'].'-'.date('Y-m-d-H',time()));
	}
	...
	private function createCashExcel($data = array()){
		...
		foreach ((array)$data as $k=>$v){
			...
			$tmp[] = array('format'=>'Number','data'=>uk86_ncPriceFormat($v['pdc_amount']));
		...
		$excel_data = $excel_obj->uk86_charset($excel_data,CHARSET);
		$excel_obj->uk86_addArray($excel_data);
		$excel_obj->uk86_addWorksheet($excel_obj->uk86_charset(L('exp_tx_title'),CHARSET));
		$excel_obj->uk86_generateXML($excel_obj->uk86_charset(L('exp_tx_title'),CHARSET).$_GET['curpage'].'-'.date('Y-m-d-H',time()));
	}
