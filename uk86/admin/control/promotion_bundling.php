...
class promotion_bundlingControl extends SystemControl{
    ...
    public function __construct(){
        ...
        //读取语言包
        Uk86Language::uk86_read('promotion_bundling');
    ...
    public function bundling_settingOp() {
		...
		if (uk86_chksubmit()){
			...
			$obj_validate->validateparam = array(
				array("input"=>$_POST["promotion_bundling_price"], "require"=>"true", 'validator'=>'Number', "message"=>Uk86Language::uk86_get('bundling_price_error')),
				array("input"=>$_POST["promotion_bundling_sum"], "require"=>"true", 'validator'=>'Number', "message"=>Uk86Language::uk86_get('bundling_sum_error')),
				array("input"=>$_POST["promotion_bundling_goods_sum"], "require"=>"true", 'validator'=>'Number', "message"=>Uk86Language::uk86_get('bundling_goods_sum_error')),
			);
    ...
    public function del_bundlingOp() {
        ...
        if ($bl_id <= 0) {
            uk86_showMessage(L('param_error'), '', 'html', 'error');
        }
        $rs = Model('p_bundling')->delBundlingForAdmin(array('bl_id' => $bl_id));
        if ($rs) {
            uk86_showMessage(L('nc_common_op_succ'));
        } else {
            uk86_showMessage(L('nc_common_op_fail'), '', 'html', 'error');
        }
    }
}
