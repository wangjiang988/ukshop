...
class storeControl extends SystemControl{
	...
	public function __construct(){
		parent::__construct();
		Uk86Language::uk86_read('store,store_grade');
	}
	...
	public function store_editOp(){
		...
		if (uk86_chksubmit()){
			...
			if (empty($grade_array)){
				uk86_showMessage($lang['please_input_store_level']);
			}
			//结束时间
			$time	= '';
...
//			$update_array['management_model']=intval($_POST['management_model']);
		...
		if (empty($store_array)){
			uk86_showMessage($lang['store_no_exist']);
		}
		//整理店铺内容
    ...
    public function edit_save_joininOp() {
        if (uk86_chksubmit()) {
            ...
            if ($result) {
                uk86_showMessage(L('nc_common_op_succ'), 'index.php?act=store&op=store');
            } else {
    ...
    public function store_bind_classOp() {
        ...
        if(empty($store_info)) {
            uk86_showMessage(L('param_error'),'','','error');
        }
    ...
    public function store_bind_class_addOp() {
        ...
        if(!empty($store_bind_class_info)) {
            uk86_showMessage('该类目已经存在','','','error');
        }
	...
	public function store_joininOp(){
        ...
        if(!empty($_GET['join_type']) && intval($_GET['join_type']) > 0) {
			$condition['join_type'] = $_GET['join_type'];
		}
        ...
        Tpl::output('joinin_type_array', $this->get_store_joinin_type());
	...
	public function store_bind_class_applay_checkOp() {
	    ...
	    if ($update) {
	        ...
	        uk86_showMessage('审核成功',uk86_getReferer());
	    } else {
	        uk86_showMessage('审核失败',uk86_getReferer(),'html','error');
	    }
	}
	...
	public function store_bind_class_applay_delOp() {
	    ...
	    if ($del) {
	        ...
	        uk86_showMessage('删除成功',uk86_getReferer());
	    } else {
	        uk86_showMessage('删除失败',uk86_getReferer(),'html','error');
	    }
	}
    ...
    private function get_store_joinin_type() {
        $join_type_array = array(
            STORE_JOIN_TYPE_PERSON => '个人申请',
            STORE_JOIN_TYPE_COMPANY => '企业申请',
        );
        return $join_type_array;
    }
    ...
    public function reopen_checkOp() {
        ...
        if ($update) {
            ...
            $this->log($msg,1);
            uk86_showMessage('续签成功，店铺有效成功延续到了'.date('Y-m-d',$data['re_end_time']).'日',uk86_getReferer());
        } else {
            uk86_showMessage('审核失败',uk86_getReferer(),'html','error');
        }
    }
    ...
    private function store_joinin_verify_pass($joinin_detail) {
        ...
        if ($param['paying_amount'] > 0) {
            uk86_showMessage('店铺入驻申请审核完成','index.php?act=store&op=store_joinin');
        } else {
    ...
    private function store_joinin_verify_open($joinin_detail) {
        ...
        if($_POST['verify_type'] === 'pass') {
			...
			if($state) {
				...
				$album_arr = array();
				$album_arr['aclass_name'] = Uk86Language::uk86_get('store_save_defaultalbumclass_name');
				...
				$msg = Uk86Language::uk86_get('store_save_create_success');
                ...
                uk86_showMessage('店铺开店成功','index.php?act=store&op=store_joinin');
            } else {
                uk86_showMessage('店铺开店失败','index.php?act=store&op=store_joinin');
            }
        } else {
            uk86_showMessage('店铺开店拒绝','index.php?act=store&op=store_joinin');
        }
    }
    ...
    public function remind_renewalOp() {
        ...
        if (!empty($store_info) && $store_info['store_end_time'] < (TIMESTAMP + 864000) && uk86_cookie('remindRenewal'.$store_id) == null) {
            ...
            uk86_setNcCookie('remindRenewal'.$store_id, 1, 86400 * 10);  // 十天
	    ...
	    public function delOp()
    {
        ...
        if (empty($storeArray)) {
            uk86_showMessage('外驻店铺不存在', '', 'html', 'error');
        }

        if ($storeArray['is_own_shop']) {
            uk86_showMessage('不能在此删除自营店铺', '', 'html', 'error');
        }
        ...
        if ((int) model('goods')->getGoodsCount($condition) > 0)
            uk86_showMessage('已经发布商品的外驻店铺不能被删除', '', 'html', 'error');
    ...
    public function newshop_addOp()
    {
        if (uk86_chksubmit())
        {
            ...
            if (strlen($memberName) < 3 || strlen($memberName) > 15
                ...
                uk86_showMessage('账号名称必须是3~15位', '', 'html', 'error');

            if (strlen($memberPasswd) < 6)
                uk86_showMessage('登录密码不能短于6位', '', 'html', 'error');

            if (!$this->checkMemberName($memberName))
                uk86_showMessage('店主账号已被占用', '', 'html', 'error');

            if (!$this->checkSellerName($_POST['seller_name']))
                uk86_showMessage('店主卖家账号名称已被其它店铺占用', '', 'html', 'error');
            ...
            {
                uk86_showMessage('店主账号新增失败', '', 'html', 'error');
            }
            ...
            uk86_showMessage('操作成功', uk86_urlAdmin('store', 'store'));
            return;
        }
    ...
    public function store_import_csvOp(){
    	if (isset($_POST['import'])) {
			...
			//上传文件存在判断
			if(empty($file['name'])){
    			uk86_showMessage('请选择要上传csv的文件!','','html','error');
			}
    		
    		// 检查文件格式
    		if ($file_type != 'csv') {
    			uk86_showMessage('文件格式不对,请重新上传!','','html','error');
    			exit;
    		}
		    ...
		    $rows = count($result); 
		    if($rows == 0){ 
    			uk86_showMessage('没有任何数据!','','html','error');
		        exit; 
		    } 
            ...
            uk86_showMessage('操作成功,成功导入 '.strval($scounter).' 条数据' , uk86_urlAdmin('store', 'store'));
            return;
     ...
     * 店铺经营方式
     */
   public function management($model_mm){
    $model_mm = Model('store');
    $management_list = $model_mm->getClassList($condition);
    $tmp_class_name = array();
    if (is_array($class_list)){
    	foreach ($class_list as $k => $v){
    		$tmp_class_name[$v['management_model']] = $v['management_model'];
    	}
    }
}
