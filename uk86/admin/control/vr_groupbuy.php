...
class vr_groupbuyControl extends SystemControl
{
    public function __construct()
    {
        ...
        if (C('groupbuy_allow') != 1) {
            uk86_showMessage('虚拟抢购功能尚未开启', 'index.php?act=dashboard&op=welcome', 'html', 'error');
        }
    }
    ...
    {
        if (uk86_chksubmit()) { //添加虚拟抢购分类
            // 数据验证
            $obj_validate = new Uk86Validate();
            $validate_array = array(
                array('input'=>$_POST['class_name'],'require'=>'true',"validator"=>"Length","min"=>"1","max"=>"10",'message'=>Uk86Language::uk86_get('groupbuy_class_name_is_not_null')),
                array('input'=>$_POST['class_name'],'require'=>'true','validator'=>'Range','min'=>0,'max'=>255,'message'=>Uk86Language::uk86_get('groupbuy_class_sort_is_not_null')),
            );
            ...
            if ($res) {
                ...
                uk86_showMessage('添加成功', $url);
            } else {
                uk86_showMessage('添加失败', 'index.php?act=vr_groupbuy&op=class_list', '', 'error');
            }
        }
    ...
    public function class_editOp()
    {
        if (uk86_chksubmit()) {
            ...
            $validate_array = array(
                array('input'=>$_POST['class_name'],'require'=>'true',"validator"=>"Length","min"=>"1","max"=>"10",'message'=>Uk86Language::uk86_get('groupbuy_class_name_is_not_null')),
                array('input'=>$_POST['class_sort'],'require'=>'true','validator'=>'Range','min'=>0,'max'=>255,'message'=>Uk86Language::uk86_get('groupbuy_class_sort_is_not_null')),
            );
            $obj_validate->validateparam = $validate_array;
            $error = $obj_validate->uk86_validate();
            if ($error != '') {
            ...
            if ($res) {
                ...
                uk86_showMessage('编辑成功', 'index.php?act=vr_groupbuy&op=class_list', '', 'succ');
            } else {
                uk86_showMessage('编辑失败', 'index.php?act=vr_groupbuy&op=class_list', '', 'error');
            }
        }
    ...
    public function class_delOp()
    {
        ...
        uk86_showMessage('删除成功', 'index.php?act=vr_groupbuy&op=class_list', '', 'succ');
    }
    ...
    public function area_addOp()
    {
        if (isset($_POST) && !empty($_POST)) {
            ...
            if ($res) {
                ...
                uk86_showMessage('添加成功','index.php?act=vr_groupbuy&op=area_list','','succ');
            } else {
                uk86_showMessage('添加失败','index.php?act=vr_groupbuy&op=area_list','','error');
            }
        }
        ...
        if (isset($_GET['area_id'])) {
            ...
            Tpl::output('area_name', Uk86Language::uk86_get('area_first_area'));
    ...
    public function area_editOp()
    {
        if (isset($_POST) && !empty($_POST)) {
            ...
            if ($res) {
                ...
                uk86_showMessage('编辑成功', 'index.php?act=vr_groupbuy&op=area_list', '', 'succ');
            } else {
                uk86_showMessage('编辑失败', 'index.php?act=vr_groupbuy&op=area_list', '', 'error');
            }
        }
        ...
        if(!empty($parent_area)){
            ...
            Tpl::output('parent_area_name',Uk86Language::uk86_get('area_first_area'));
        }
    ...
    public function area_dropOp()
    {
        ...
        if ($res) {
            ...
            uk86_showMessage('删除成功','index.php?act=vr_groupbuy&op=area_list','','succ');
        } else {
            uk86_showMessage('删除失败','index.php?act=vr_groupbuy&op=area_list','','error');
        }
    }
