	    ...
	    if (uk86_chksubmit()) {
    	    ...
    	    for ($i;$i <= $size;$i++) {
        		...
        		if (!empty($_FILES[$file]['name'])) {//上传图片
        			...
        			$upload->uk86_set('file_name',$file_name);
    	    ...
    	    uk86_showMessage(Uk86Language::uk86_get('nc_common_save_succ'),'index.php?act=store_joinin&op=edit_info');
	    }
	...
	public function edit_helpOp() {
		...
		if (uk86_chksubmit()) {
			...
			if ($state) {
				...
				uk86_showMessage(Uk86Language::uk86_get('nc_common_save_succ'),'index.php?act=store_joinin&op=help_list');
			} else {
