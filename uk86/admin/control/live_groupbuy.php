...
class live_groupbuyControl extends SystemControl{
	...
	public function del_groupbuyOp(){
		if(isset($_POST)&&!empty($_POST)){
			...
			if($res){
				uk86_showMessage('删除抢购成功','index.php?act=live_groupbuy','','succ');
			}else{
				uk86_showMessage('删除抢购失败','index.php?act=live_groupbuy','','error');
			}
		}
	}
	...
	public function auditOp(){
		...
		if($res){
			...
			uk86_showMessage('审核成功','index.php?act=live_groupbuy','','succ');
		}else{
			uk86_showMessage('审核失败','index.php?act=live_groupbuy','','error');
		}
	}
	...
	public function cancelOp(){
		...
		if($res){
			...
			uk86_showMessage('操作成功','index.php?act=live_groupbuy','','succ');
		}else{
			uk86_showMessage('操作失败','index.php?act=live_groupbuy','','error');
		}
	...
	public function groupbuyvoucherOp(){
		...
		if(empty($groupbuy)){
			uk86_showMessage('抢购不存在','index.php?act=live_groupbuy','','error');
		}
