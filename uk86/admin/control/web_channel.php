...
class web_channelControl extends SystemControl{
    ...
    public function add_floorOp() {
        ...
        if (uk86_chksubmit()) {
            ...
            if ($state) {
                ...
                uk86_showMessage(Uk86Language::uk86_get('nc_common_save_succ'),'index.php?act=web_channel&op=floor_list');
            } else {
                uk86_showMessage(Uk86Language::uk86_get('nc_common_save_fail'));
            }
        }
    ...
    public function set_channelOp() {
        ...
        $service_cities=Model("area")->getServiceGoodsCity();    //获取开通本地化城市的列表
        $parent_class = array();
        ...
        Tpl::output('service_cities',$service_cities);        //传递给前台页面
        ...
        if (uk86_chksubmit()) {
            ...
            if ($gc_id > 0) {
                ...
                foreach($service_cities as $key=>$area){
                    if($area['area_id']==$area_id){
                        $area_name=$area['area_name'];
                    }
                }
                $model_channel->editChannelCityArea($area_id);//消除分类已绑定的频道
            }

            $channel_array['gc_id'] = $gc_id;
            $channel_array['gc_name'] = $gc_name;
            $channel_array['top_id'] = $_POST['top_id'];
            $channel_array['area_id']=$area_id;
            $channel_array['area_name']=$area_name;
            ...
            $state = $model_channel->editChannel($condition, $channel_array);    //更新数据库
            if ($state) {
                $this->log('编辑商城频道，编号'.$condition['channel_id']);
                uk86_dkcache('channel');
                uk86_showMessage(Uk86Language::uk86_get('nc_common_save_succ'),'index.php?act=web_channel&op=web_channel');
            } else {
                uk86_showMessage(Uk86Language::uk86_get('nc_common_save_fail'));
            }
        ...
        Tpl::output('area_id',$channel['area_id']);
     ...
     * 检查城市是否已经绑定到频道
     */
    public function  check_city_areasOp(){
        $model_channel = Model('web_channel');
        ...
        $list = $model_channel->getChannelList($condition);
        if (!empty($list[0])) {
            echo $list[0]['channel_name'];exit;    //已经绑定
        }
    }
    /**
     * 保存设置
