...
class store_gradeControl extends SystemControl{
	public function __construct(){
		parent::__construct();
		Uk86Language::uk86_read('store_grade,store');
	}
	/**
	 * 店铺等级
	 */
			...
			if (!empty($_POST['check_sg_id'])){
				if (is_array($_POST['check_sg_id'])){
					...
					foreach ($_POST['check_sg_id'] as $k => $v){
						...
						//判断是否默认等级，默认等级不能删除
						if ($v == 1){
							//uk86_showMessage('默认等级不能删除 ','index.php?act=store_grade&op=store_grade');
							uk86_showMessage($lang['default_store_grade_no_del'],'index.php?act=store_grade&op=store_grade');
						}
		...
		if (uk86_chksubmit()){
			...
			if ($error != ''){
				...
				if (!$this->checkGradeName(array('sg_name'=>trim($_POST['sg_name'])))){
					uk86_showMessage($lang['now_store_grade_name_is_there']);
				}
				//验证级别是否存在
				if (!$this->checkGradeSort(array('sg_sort'=>trim($_POST['sg_sort'])))){
					uk86_showMessage($lang['add_gradesortexist']);
				}
				$insert_array = array();
				...
				if ($result){
					...
					uk86_showMessage($lang['nc_common_save_succ'],'index.php?act=store_grade&op=store_grade');
				}else {
	...
	public function store_grade_editOp(){
		...
		if (uk86_chksubmit()){
			if (!$_POST['sg_id']){
				uk86_showMessage($lang['grade_parameter_error'],'index.php?act=store_grade&op=store_grade');
			}
			/**
			 * 验证
			 */
			...
			if ($error != ''){
				...
				if (!$this->checkGradeName(array('sg_name'=>trim($_POST['sg_name']),'sg_id'=>intval($_POST['sg_id'])))){
					uk86_showMessage($lang['now_store_grade_name_is_there'],'index.php?act=store_grade&op=store_grade_edit&sg_id='.intval($_POST['sg_id']));
				}
				//验证级别是否存在
				if (!$this->checkGradeSort(array('sg_sort'=>trim($_POST['sg_sort']),'sg_id'=>intval($_POST['sg_id'])))){
					uk86_showMessage($lang['add_gradesortexist'],'index.php?act=store_grade&op=store_grade_edit&sg_id='.intval($_POST['sg_id']));
				}
				$update_array = array();
	...
	public function store_grade_delOp(){
		...
		if (intval($_GET['sg_id']) > 0){
			...
			if (!$this->isable_delGrade(intval($_GET['sg_id']))){
				uk86_showMessage($lang['del_gradehavestore'],'index.php?act=store_grade&op=store_grade');
			}
			/**
			 * 删除分类
			 */
			...
			uk86_showMessage($lang['nc_common_del_succ'],'index.php?act=store_grade&op=store_grade');
		}else {
			uk86_showMessage($lang['nc_common_del_fail'],'index.php?act=store_grade&op=store_grade');
		}
	}
