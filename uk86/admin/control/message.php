...
class messageControl extends SystemControl
{
    private $links = array(
        ...
        array('url' => 'act=message&op=mobile', 'lang' => 'sms_set'),
    ...
    {
        parent::__construct();
        Uk86Language::uk86_read('setting,message');
    }
        ...
        if (uk86_chksubmit()) {
            ...
            if ($error != '') {
                ...
                if ($result === true) {
                    ...
                    uk86_showMessage(L('mailtemplates_edit_succ'), 'index.php?act=message&op=email_tpl');
                } else {
                    $this->log(L('nc_edit,email_tpl'), 0);
                    uk86_showMessage(L('mailtemplates_edit_fail'));
                }
            }
        }
        if (empty($_GET['code'])) {
            uk86_showMessage(L('mailtemplates_edit_code_null'));
        }
...
// 		}
    ...
    public function mobileOp()
    {
        ...
        if (uk86_chksubmit()) {
            ...
            $update_array['sms_url'] = $_POST['sms_url'];
            $update_array['sms_serial_number'] = $_POST['sms_serial_number'];
            $update_array['sms_password'] = $_POST['sms_password'];
            $update_array['sms_sessionKey'] = $_POST['sms_sessionKey'];
            $update_array['sms_plugin'] = $_POST['sms_plugin'];
            $update_array['params_url'] = $_POST['params_url'];
            $update_array['params_uid'] = $_POST['params_uid'];
            $update_array['params_auth'] = $_POST['params_auth'];
    ...
    private function seller_tpl_update_message()
    {
        ...
        $this->seller_tpl_update_uk86_showMessage($result);
    }
    ...
    private function seller_tpl_update_uk86_showMessage($result)
    {
        if ($result) {
            uk86_showMessage(L('nc_common_op_succ'), uk86_urlAdmin('message', 'seller_tpl'));
        } else {
    ...
    private function member_tpl_update_message()
    {
        ...
        $this->member_tpl_update_uk86_showMessage($result);
    }
    ...
    private function member_tpl_update_uk86_showMessage($result)
    {
        if ($result) {
            uk86_showMessage(L('nc_common_op_succ'), uk86_urlAdmin('message', 'member_tpl'));
        } else {
            uk86_showMessage(L('nc_common_op_fail'));
        }
    }
