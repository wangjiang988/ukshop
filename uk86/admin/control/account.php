		...
		if (uk86_chksubmit()){
			...
			if (trim($_POST['qq_isuse']) == '1'){
				$obj_validate->validateparam = array(
					array("input"=>$_POST["qq_appid"], "require"=>"true","message"=>Uk86Language::uk86_get('qq_appid_error')),
					array("input"=>$_POST["qq_appkey"], "require"=>"true","message"=>Uk86Language::uk86_get('qq_appkey_error'))
				);
			}
	...
	public function sinaOp(){
		...
		if (uk86_chksubmit()){
			...
			if (trim($_POST['sina_isuse']) == '1'){
				$obj_validate->validateparam = array(
					array("input"=>$_POST["sina_wb_akey"], "require"=>"true","message"=>Uk86Language::uk86_get('sina_wb_akey_error')),
					array("input"=>$_POST["sina_wb_skey"], "require"=>"true","message"=>Uk86Language::uk86_get('sina_wb_skey_error'))
				);
			}
			$error = $obj_validate->uk86_validate();
			if ($error != ''){
