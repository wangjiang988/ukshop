...
class goods_classControl extends SystemControl{
	...
	public function __construct(){
		parent::__construct();
		Uk86Language::uk86_read('goods_class');
	}
				...
				if ($result){
    				if ($insert_array['gc_parent_id'] == 0) {
            			if (!empty($_FILES['pic']['name'])) {//上传图片
                			...
                			$upload->uk86_set('file_name','category-pic-'.$result.'.jpg');
	...
	public function goods_class_importOp(){
		...
		if (uk86_chksubmit()){
			...
			if (!empty($_FILES['csv']) && !empty($_FILES['csv']['name']) && $file_type == 'csv'){
				...
				uk86_showMessage($lang['nc_common_op_succ'],'index.php?act=goods_class&op=goods_class');
			}else {
				$this->log(L('goods_class_index_import,goods_class_index_class'),0);
				uk86_showMessage($lang['goods_class_import_csv_null']);
			}
		}
	...
	public function goods_class_delOp(){
		...
		if (intval($_GET['gc_id']) > 0){
			...
			uk86_showMessage($lang['nc_common_del_succ'],'index.php?act=goods_class&op=goods_class');
		}else {
			$this->log(L('nc_delete,goods_class_index_class') . '[ID:' . intval($_GET['gc_id']) . ']',0);
			uk86_showMessage($lang['nc_common_del_fail'],'index.php?act=goods_class&op=goods_class');
		}
	}
	...
	public function tag_resetOp(){
		...
		if(!$return){
			uk86_showMessage($lang['goods_class_reset_tag_fail'], 'index.php?act=goods_class&op=tag');
		}
		...
		if(is_array($goods_class) and !empty($goods_class)) {
			...
			if($return){
				...
				uk86_showMessage($lang['nc_common_op_succ'], 'index.php?act=goods_class&op=tag');
			}else{
				$this->log(L('nc_reset').'tag',0);
				uk86_showMessage($lang['nc_common_op_fail'], 'index.php?act=goods_class&op=tag');
			}
		}else{
			$this->log(L('nc_reset').'tag',0);
			uk86_showMessage($lang['goods_class_reset_tag_fail_no_class'], 'index.php?act=goods_class&op=tag');
		}
	}
	...
	public function tag_updateOp(){
		...
		if(is_array($tag_list) && !empty($tag_list)){
			...
			$this->log(L('nc_update').'tag',0);
			uk86_showMessage($lang['goods_class_update_tag_fail_no_class'], 'index.php?act=goods_class&op=tag');
		}
