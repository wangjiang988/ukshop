...
class web_configControl extends SystemControl{
	public function __construct(){
		parent::__construct();
		Uk86Language::uk86_read('web_config,recommend');
	}
	...
	public function web_editOp(){
		...
		$web=$model_web_config->getWebById($web_id);
		if (uk86_chksubmit()){
			$web_array = array();
			...
			$model_web_config->updateWebHtml($web_id,$web_array['style_name'],$web['web_style']);//更新前台显示的html内容
			$this->log(l('web_config_code_edit').'['.$_POST["web_name"].']',1);
			uk86_showMessage(Uk86Language::uk86_get('nc_common_save_succ'),'index.php?act=web_config&op=web_config');
		}
	...
	public function code_editOp(){
		$model_web_config = Model('web_config');
		$web_id = intval($_GET["web_id"]);
		$code_list = $model_web_config->getCodeList(array('web_id'=>"$web_id"));   //获取板块列表
		if(is_array($code_list) && !empty($code_list)) {
			...
			Tpl::output('parent_goods_class',$parent_goods_class);   //商品分类
			...
			$web_list = $model_web_config->getWebList(array('web_id'=>$web_id));
			Tpl::output('web_array',$web_list[0]);
			Tpl::showpage('web_code_1.edit');
		} else {
			uk86_showMessage(Uk86Language::uk86_get('nc_no_record'));
		}
	}
	...
	public function web_htmlOp(){
		$model_web_config = Model('web_config');
		$web_id = intval($_GET["web_id"]);   //前台模板id
		$web_style=intval($_GET['web_style']);  //前台模板样式，
		$web_list = $model_web_config->getWebList(array('web_id'=>$web_id));     //获取该模板
		$web_array = $web_list[0];
		if(!empty($web_array) && is_array($web_array)) {
			$model_web_config->updateWebHtml($web_id,$web_array['style_name'],$web_style);
			uk86_showMessage(Uk86Language::uk86_get('nc_common_op_succ'),'index.php?act=web_config&op=web_config');
		} else {
