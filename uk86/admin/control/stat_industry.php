...
class stat_industryControl extends SystemControl{
	...
	public function scale_listOp(){
	    ...
	    if($gc_childarr){
    		...
    		$statjson = uk86_getStatData_Basicbar($stat_arr);
	...
	public function rankOp(){
		...
		$store_statjson = uk86_getStatData_Column2D($store_stat_arr);
    ...
    public function priceOp(){
		...
		if ($pricerange_arr){
    		...
    		$goodsnum_stat_json = uk86_getStatData_LineLabels($goodsnum_stat_arr);
    		$orderamount_stat_json = uk86_getStatData_LineLabels($orderamount_stat_arr);
		} else {
		    $goodsnum_stat_json = '';
	...
	public function general_listOp(){
	    ...
	    if ($gc_childarr){
    		...
    		foreach ($gc_childarr as $k=>$v){
    		    ...
    		    $tmp['priceavg'] = uk86_ncPriceFormat(($t = $goods_list[$v['gc_id']]['priceavg'])?$t:0);
            ...
            if ($this->search_arr['exporttype'] == 'excel'){
    		    ...
    		    $excel_obj->uk86_addWorksheet($excel_obj->uk86_charset('行业概况总览',CHARSET));
    		    $excel_obj->uk86_generateXML($excel_obj->uk86_charset('行业概况总览',CHARSET).date('Y-m-d-H',time()));
    			exit();
            }
	    }
         //列表header
