...
class flea_regionControl extends SystemControl{
	...
	public function flea_regionOp(){
		...
		if ($_POST['form_submit'] == 'ok'){
			...
			uk86_showMessage($lang['region_index_modify_succ']);
		}
		/**
		 * 导航地区内容
		 */
	...
	public function flea_region_importOp(){
		...
		if ($_POST['form_submit'] == 'ok'){
			if (!empty($_FILES['csv'])){
				...
				uk86_showMessage($lang['region_import_succ'],'index.php?act=flea_region&op=flea_region');
			}else {
				uk86_showMessage($lang['region_import_csv_null']);
			}
		}
		        ...
		        if (trim($buffer) != ''){
			        ...
			        if (substr(rtrim($buffer),-1) == ';'){
			        	...
			        	if (!empty($tmp_sql)){
			        		...
			        		$tmp_sql = str_replace("`ukshop_flea_area`","`".DBPRE."flea_area`",$tmp_sql);
			        		Db::query($tmp_sql);
			        		unset($tmp_sql);
			        	}
			        }
