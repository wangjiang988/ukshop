...
class rechargecardControl extends SystemControl
{
    ...
    public function add_cardOp()
    {
        ...
        if ($denomination < 0.01) {
            uk86_showMessage('面额不能小于0.01', '', 'html', 'error');
            return;
        }
        if ($denomination > 1000) {
            uk86_showMessage('面额不能大于1000', '', 'html', 'error');
            return;
        }
        ...
        case '0':
            ...
            if ($total < 1 || $total > 9999) {
                uk86_showMessage('总数只能是1~9999之间的整数', '', 'html', 'error');
                exit;
            }
            $prefix = (string) $_POST['prefix'];
            if (!preg_match('/^[0-9a-zA-Z]{0,16}$/', $prefix)) {
                uk86_showMessage('前缀只能是16字之内字母数字的组合', '', 'html', 'error');
                exit;
            }
        ...
        case '1':
            $f = $_FILES['_textfile'];
            if (!$f || $f['error'] != 0) {
                uk86_showMessage('文件上传失败', '', 'html', 'error');
                exit;
            }
            if (!is_uploaded_file($f['tmp_name'])) {
                uk86_showMessage('未找到已上传的文件', '', 'html', 'error');
                exit;
            }
        ...
        if ($totalKeys < 1 || $totalKeys > 9999) {
            uk86_showMessage('只能在一次操作中增加1~9999个充值卡号', '', 'html', 'error');
            exit;
        }

        if (empty($snKeys)) {
            uk86_showMessage('请输入至少一个合法的卡号', '', 'html', 'error');
            exit;
        }
        ...
        if (empty($snKeys)) {
            uk86_showMessage('操作失败，所有新增的卡号都与已有的卡号冲突', '', 'html', 'error');
            exit;
        }
        ...
        if (!$model->insertAll($snToInsert)) {
            uk86_showMessage('操作失败', '', 'html', 'error');
            exit;
        }
        ...
        uk86_showMessage($msg, uk86_urlAdmin('rechargecard', 'index'));
    }
