...
class mb_categoryControl extends SystemControl{
	...
	public function mb_category_delOp(){
		...
		if (intval($_GET['gc_id']) > 0){
			...
			uk86_showMessage($lang['link_index_del_succ'],'index.php?act=mb_category&op=mb_category_list');
		}else {
			uk86_showMessage($lang['link_index_choose_del'],'index.php?act=mb_category&op=mb_category_list');
		}
	}
	...
	public function mb_category_addOp(){
		...
		if ($_POST['form_submit'] == 'ok'){
			...
			if (!empty($category)){
				uk86_showMessage($lang['link_add_category_exist']);
			}
