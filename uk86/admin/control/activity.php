...
class activityControl extends SystemControl{
	public function __construct(){
		parent::__construct();
		Uk86Language::uk86_read('activity');
	}
	/**
	 * 活动列表
	 */
	...
	public function newOp(){
		...
		$validate_arr[] = array("input"=>$_POST["activity_title"],"require"=>"true","message"=>Uk86Language::uk86_get('activity_new_title_null'));
		$validate_arr[] = array("input"=>$_POST["activity_start_date"],"require"=>"true","message"=>Uk86Language::uk86_get('activity_new_startdate_null'));
		$validate_arr[] = array("input"=>$_POST["activity_end_date"],"require"=>"true",'validator'=>'Compare','operator'=>'>','to'=>"{$_POST['activity_start_date']}","message"=>Uk86Language::uk86_get('activity_new_enddate_null'));
		$validate_arr[] = array("input"=>$_POST["activity_style"],"require"=>"true","message"=>Uk86Language::uk86_get('activity_new_style_null'));
		$validate_arr[] = array('input'=>$_POST['activity_type'],'require'=>'true','message'=>Uk86Language::uk86_get('activity_new_type_null'));
		$validate_arr[] = array('input'=>$_FILES['activity_banner']['name'],'require'=>'true','message'=>Uk86Language::uk86_get('activity_new_banner_null'));
		$validate_arr[] = array('input'=>$_POST['activity_sort'],'require'=>'true','validator'=>'Range','min'=>0,'max'=>255,'message'=>Uk86Language::uk86_get('activity_new_sort_error'));
		...
		$upload->uk86_set('default_dir',ATTACH_ACTIVITY);
		$result = $upload->uk86_upfile('activity_banner');
		if(!$result){
		...
		if($result){
			...
			uk86_showMessage(Uk86Language::uk86_get('nc_common_op_succ'),'index.php?act=activity&op=activity');
		}else{
	...
	public function delOp(){
		...
		if(empty($_REQUEST['activity_id'])){
			uk86_showMessage(Uk86Language::uk86_get('activity_del_choose_activity'));
		}
		...
		uk86_showMessage(Uk86Language::uk86_get('activity_del_fail'));
	}
	...
	public function editOp(){
		...
		$validate_arr[] = array("input"=>$_POST["activity_style"],"require"=>"true","message"=>Uk86Language::uk86_get('activity_new_style_null'));
		$validate_arr[] = array('input'=>$_POST['activity_type'],'require'=>'true','message'=>Uk86Language::uk86_get('activity_new_type_null'));
		$validate_arr[] = array('input'=>$_POST['activity_desc'],'require'=>'true','message'=>Uk86Language::uk86_get('activity_new_desc_null'));
		$validate_arr[] = array('input'=>$_POST['activity_sort'],'require'=>'true','validator'=>'Range','min'=>0,'max'=>255,'message'=>Uk86Language::uk86_get('activity_new_sort_error'));
		$obj_validate->validateparam = $validate_arr;
		$error = $obj_validate->uk86_validate();
		if ($error != ''){
		...
		if($result){
			...
			uk86_showMessage(Uk86Language::uk86_get('nc_common_save_succ'),'index.php?act=activity&op=activity');
		}else{
			if($_FILES['activity_banner']['name']!=''){
				@unlink(BASE_UPLOAD_PATH.DS.ATTACH_ACTIVITY.DS.$upload->file_name);
			}
	...
	public function dealOp(){
		if(empty($_REQUEST['activity_detail_id'])){
			uk86_showMessage(Uk86Language::uk86_get('activity_detail_del_choose_detail'));
		}
		//获取id
		$id	= '';
