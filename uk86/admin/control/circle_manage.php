...
class circle_manageControl extends SystemControl{
		...
		if(uk86_chksubmit()){
			...
			if($_POST['submit_type'] == 'batchdel'){
				...
				if(empty($id_array) && !is_array($id_array)){
					uk86_showMessage(L('circle_choose_del_circle'));
				}
				$model = Model();
		...
		if(uk86_chksubmit()){
			...
			if($error != ''){
				...
				if($rs){
					...
					uk86_showMessage(L('nc_common_op_succ'), 'index.php?act=circle_manage&op=circle_list');
				}else{
					uk86_showMessage(L('nc_common_op_fail'));
				}
			}
	...
	public function choose_masterOp(){
	    Tpl::output('search_url', (intval($_GET['c_id']) > 0) ? uk86_urlAdmin('circle_manage', 'search_member', array('c_id' => intval($_GET['c_id']))) : uk86_urlAdmin('circle_manage', 'search_member'));
	...
	public function search_memberOp(){
		...
		$member_list = uk86_array_under_reset($member_list, 'member_id', 1);
		...
		if(strtoupper(CHARSET) == 'GBK'){
			$member_list = Uk86Language::uk86_getUTF8($member_list);
		}
