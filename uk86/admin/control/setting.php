...
class settingControl extends SystemControl{
	...
	public function baseOp(){
		...
		if (uk86_chksubmit()){
			...
			if (!empty($_FILES['site_logo']['name'])){
				...
				$result = $upload->uk86_upfile('site_logo');
				if ($result){
			...
			if (!empty($_FILES['site_logowx']['name'])){
				$upload = new Uk86UploadFile();
				$upload->uk86_set('default_dir',ATTACH_COMMON);
				$result = $upload->uk86_upfile('site_logowx');
				if ($result){
			...
			if (!empty($_FILES['member_logo']['name'])){
				$upload = new Uk86UploadFile();
				$upload->uk86_set('default_dir',ATTACH_COMMON);
				$result = $upload->uk86_upfile('member_logo');
				if ($result){
			...
			if (!empty($_FILES['seller_center_logo']['name'])){
				$upload = new Uk86UploadFile();
				$upload->uk86_set('default_dir',ATTACH_COMMON);
				$result = $upload->uk86_upfile('seller_center_logo');
				if ($result){
			...
			$update_array['aboutus'] = $_POST['aboutus'];
	...
	public function ajax_categoryOp(){
		...
		if (strtoupper(CHARSET) == 'GBK'){
			$list = Uk86Language::uk86_getUTF8($list);//网站GBK使用编码时,转换为UTF-8,防止json输出汉字问题
		}
