...
class rec_positionControl extends SystemControl{
	public function __construct(){
		parent::__construct();
		Uk86Language::uk86_read('rec_position');
	}
	...
	public function rec_listOp(){
		...
		if (uk86_chksubmit()){
			...
			if (!$list)	uk86_showMessage(Uk86Language::uk86_get('param_error'));
	...
	public function rec_editOp(){
		...
		if (!$info)	uk86_showMessage(Uk86Language::uk86_get('no_record'));
		$info['content'] = unserialize($info['content']);
	...
	public function rec_saveOp(){
			...
			if (is_array($_FILES['pic']['tmp_name'])){
				foreach($_FILES['pic']['tmp_name'] as $k=>$v){
					...
					if (in_array($ext,array('jpg','jpeg','gif','png'))){
						...
						if ($_FILES['pic']['error'][$k] != 0) uk86_showMessage(Uk86Language::uk86_get('nc_common_op_fail'));
						$c = count($array['body']);
					...
					if (empty($array)) uk86_showMessage(Uk86Language::uk86_get('param_error'));
				}
			}
		...
		uk86_showMessage(Uk86Language::uk86_get('nc_common_save_succ'),'index.php?act=rec_position&op=rec_list');
	}
	...
	public function rec_edit_saveOp(){
		if (!is_numeric($_POST['rec_id'])) uk86_showMessage(Uk86Language::uk86_get('param_error'));
		$array = array();
		$data = array();
		$pattern = "/^http:\/\/[A-Za-z0-9]+[A-Za-z0-9.]+\.[A-Za-z0-9]+/i";
		//文字类型
			...
			if (is_array($_FILES['pic']['tmp_name'])){
				foreach($_FILES['pic']['tmp_name'] as $k=>$v){
					...
					if (in_array($ext,array('jpg','jpeg','gif','png','bmp'))){
						...
						if ($_FILES['pic']['error'][$k] != 0) uk86_showMessage(Uk86Language::uk86_get('nc_common_save_fail'));
		...
		if ($result){
			...
			uk86_dkcache("rec_position/{$_POST['rec_id']}");
			uk86_showMessage(Uk86Language::uk86_get('nc_common_save_succ'),'index.php?act=rec_position&op=rec_list');
		}else{
			uk86_showMessage(Uk86Language::uk86_get('nc_common_save_fail'),'index.php?act=rec_position&op=rec_list');
		}
	}
	...
	public function rec_viewOp(){
		...
		echo uk86_rec(intval($_GET['rec_id']));
	}
}
