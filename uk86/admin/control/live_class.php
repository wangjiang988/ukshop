...
class live_classControl extends SystemControl{
	...
	public function add_classOp(){
		if(uk86_chksubmit()){//添加线下抢购分类
			//数据验证
			$obj_validate = new Uk86Validate();
			$validate_array = array(
				array('input'=>$_POST['live_class_name'],'require'=>'true',"validator"=>"Length","min"=>"1","max"=>"10",'message'=>Uk86Language::uk86_get('live_groupbuy_class_name_is_not_null')),
				array('input'=>$_POST['live_class_name'],'require'=>'true','validator'=>'Range','min'=>0,'max'=>255,'message'=>Uk86Language::uk86_get('live_groupbuy_class_sort_is_not_null')),
			);
			...
			if($res){
				...
				uk86_showMessage('添加成功','index.php?act=live_class','','succ');
			}else{
				uk86_showMessage('添加失败','index.php?act=live_class','','error');
			}
		}
	...
	public function edit_classOp(){
		if(uk86_chksubmit()){
			...
			$validate_array = array(
					array('input'=>$_POST['live_class_name'],'require'=>'true',"validator"=>"Length","min"=>"1","max"=>"10",'message'=>Uk86Language::uk86_get('live_groupbuy_class_name_is_not_null')),
					array('input'=>$_POST['live_class_sort'],'require'=>'true','validator'=>'Range','min'=>0,'max'=>255,'message'=>Uk86Language::uk86_get('live_groupbuy_class_sort_is_not_null')),
			);
			$obj_validate->validateparam = $validate_array;
			$error = $obj_validate->uk86_validate();
			if ($error != ''){
			...
			if($res){
				...
				uk86_showMessage('编辑成功','index.php?act=live_class','','succ');
			}else{
				uk86_showMessage('编辑失败','index.php?act=live_class','','error');
			}
		}
	...
	public function del_classOp(){
		...
		uk86_showMessage('删除成功','index.php?act=live_class','','succ');
	}
