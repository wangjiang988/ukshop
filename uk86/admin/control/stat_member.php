...
class stat_memberControl extends SystemControl{
	...
	public function analyzeinfoOp(){
        ...
        if ($_GET['exporttype'] == 'excel'){
			...
			$excel_data = $excel_obj->uk86_charset($excel_data,CHARSET);
			$excel_obj->uk86_addArray($excel_data);
		    $excel_obj->uk86_addWorksheet($excel_obj->uk86_charset('会员'.$caption.'统计',CHARSET));
		    $excel_obj->uk86_generateXML($excel_obj->uk86_charset('会员'.$caption.'统计',CHARSET).date('Y-m-d-H',time()));
			exit();
		} else {
	...
	public function showmemberOp(){
        ...
        if ($this->search_arr['exporttype'] == 'excel'){
			...
			$excel_data = $excel_obj->uk86_charset($excel_data,CHARSET);
			$excel_obj->uk86_addArray($excel_data);
		    $excel_obj->uk86_addWorksheet($excel_obj->uk86_charset('新增会员',CHARSET));
		    $excel_obj->uk86_generateXML($excel_obj->uk86_charset('新增会员',CHARSET).date('Y-m-d-H',time()));
			exit();
        }
	...
	public function scaleOp(){
        ...
        if ($this->search_arr['exporttype'] == 'excel'){
		    ...
		    $excel_data[0][] = array('styleid'=>'s_title','data'=>'增U币');
		    $excel_data[0][] = array('styleid'=>'s_title','data'=>'减U币');
			//data
			...
			$excel_data = $excel_obj->uk86_charset($excel_data,CHARSET);
			$excel_obj->uk86_addArray($excel_data);
		    $excel_obj->uk86_addWorksheet($excel_obj->uk86_charset('会员规模分析',CHARSET));
		    $excel_obj->uk86_generateXML($excel_obj->uk86_charset('会员规模分析',CHARSET).date('Y-m-d-H',time()));
			exit();
        }
		Tpl::output('statlist',$statlist);
		Tpl::output('show_page',$model->showpage(2));
