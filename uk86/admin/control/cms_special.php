...
class cms_specialControl extends SystemControl{
    ...
    public function cms_special_saveOp() {
        ...
        $param['special_btn_background'] = $_POST['special_btn_background'];
        $param['special_btn_color'] = $_POST['special_btn_color'];
        $param['special_btn_top'] = $_POST['special_btn_top'];
        $param['special_btn_left'] = $_POST['special_btn_left']?$_POST['special_btn_left']:'auto';
        $param['special_btn_url'] = $_POST['special_btn_url'];
        $param['special_enroll_status'] = intval($_POST['special_enroll_status']);
        ...
        if($result) {
            ...
            $this->log(Uk86Language::uk86_get('cms_log_special_save').$result, 1);
            uk86_showMessage(Uk86Language::uk86_get('nc_common_save_succ'), self::LINK_CMS_SPECIAL);
        } else {
            $this->log(Uk86Language::uk86_get('cms_log_special_save').$result, 0);
            uk86_showMessage(Uk86Language::uk86_get('nc_common_save_fail'), self::LINK_CMS_SPECIAL);
        }
    }
    ...
    private function generate_html($special_id) {
        ...
        if(!is_dir($html_path)){
            if (!@mkdir($html_path, 0755)){
                uk86_showMessage(Uk86Language::uk86_get('cms_special_build_fail'),'','','error');
            }
        }
    ...
    public function cms_special_dropOp() {
        ...
        if($result) {
            $this->log(Uk86Language::uk86_get('cms_log_special_drop').$_POST['special_id'], 1);
            uk86_showMessage(Uk86Language::uk86_get('nc_common_del_succ'),'');
        } else {
            $this->log(Uk86Language::uk86_get('cms_log_special_drop').$_POST['special_id'], 0);
    ...
    private function cms_special_image_upload($image) {
        if(!empty($_FILES[$image]['name'])) {
            ...
            $upload->uk86_set('default_dir',ATTACH_CMS.DS.'special');
    ...
    private function cms_special_image_drop($image) {
        $file = uk86_getCMSSpecialImagePath($image);
        if(is_file($file)) {
            unlink($file);
        }
    }
    ...
    public function special_image_uploadOp() {
        ...
        if(!empty($_FILES['special_image_upload']['name'])) {
            ...
            $result = $upload->uk86_upfile('special_image_upload');
            if(!$result) {
                $data['status'] = 'fail';
            ...
            $data['file_url'] = uk86_getCMSSpecialImageUrl($upload->file_name);
        }
    ...
    public function goods_info_by_urlOp() {
        ...
        if(empty($url)) {
            self::return_json(Uk86Language::uk86_get('param_error'),'false');
        }
    ...
    private function get_special_state_list() {
        $array = array();
        $array[self::SPECIAL_STATE_DRAFT] = Uk86Language::uk86_get('cms_text_draft');
        $array[self::SPECIAL_STATE_PUBLISHED] = Uk86Language::uk86_get('cms_text_published');
        return $array;
    }
    ...
    private function show_menu($menu_key) {
        $menu_array = array(
            'special_list'=>array('menu_type'=>'link','menu_name'=>Uk86Language::uk86_get('nc_manage'),'menu_url'=>'index.php?act=cms_special&op=cms_special_list'),
            'special_add'=>array('menu_type'=>'link','menu_name'=>Uk86Language::uk86_get('nc_new'),'menu_url'=>'index.php?act=cms_special&op=cms_special_add'),
            'special_edit'=>array('menu_type'=>'link','menu_name'=>Uk86Language::uk86_get('nc_edit'),'menu_url'=>'###'),
        );
