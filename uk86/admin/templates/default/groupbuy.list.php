  ...
  <form id="list_form" method="post">
    ...
    <table class="table tb-type2">
      ...
      <tbody id="treet1">
        ...
        <tr class="hover">
          <td class="w60 picture"><div class="size-56x56"><span class="thumb size-56x56"><i></i><a target="_blank" href="<?php echo SHOP_SITE_URL."/index.php?act=show_groupbuy&op=groupbuy_detail&group_id=".$val['groupbuy_id'];?>"><img src="<?php echo uk86_gthumb($val['groupbuy_image'], 'small');?>" style=" max-width: 56px; max-height: 56px;"/></a></span></div></td>
          <td class="group"><p><a target="_blank" href="<?php echo SHOP_SITE_URL."/index.php?act=show_groupbuy&op=groupbuy_detail&group_id=".$val['groupbuy_id'];?>"><?php echo $val['groupbuy_name'];?></a></p>
            ...
            <p class="store"><?php echo $lang['groupbuy_index_store_name'];?>:<a href="<?php echo uk86_urlShop('show_store','index', array('store_id'=>$val['store_id']));?>" title="<?php echo $val['store_name'];?>"><?php echo $val['store_name'];?></a>
...
<?php } ?>
        ...
        <td class="align-center">
            ...
            <a nctype="btn_review_pass" data-groupbuy-id="<?php echo $val['groupbuy_id'];?>" href="javascript:void(0);">通过</a>
            <a nctype="btn_review_fail" data-groupbuy-id="<?php echo $val['groupbuy_id'];?>" href="javascript:void(0);">拒绝</a>
            <?php } ?>
            <?php if($val['cancelable']) { ?>
            <a nctype="btn_cancel" data-groupbuy-id="<?php echo $val['groupbuy_id'];?>" href="javascript:void(0);">取消</a>
            <?php } ?>
            <a nctype="btn_del" data-groupbuy-id="<?php echo $val['groupbuy_id'];?>" href="javascript:void(0);">删除</a>
        </td>
        </tr>
        <?php } ?>
        <?php }else { ?>
...
<form id="op_form" action="" method="POST">
    ...
    <input type="hidden" id="reject_reason" name="reject_reason"  />
</form>
    ...
    $(document).ready(function() {
        $('[nctype="btn_review_pass"]').on('click', function() {
            if(confirm('确认通过该抢购申请？')) {
                var action = '<?php echo uk86_urlAdmin('groupbuy', 'groupbuy_review_pass');?>';
        ...
        $('[nctype="btn_review_fail"]').on('click', function() {
            var reason = prompt("请输入审核不通过的理由","");
            if (reason != null && reason != "")
            {
                console.log(reason);
                var action = '<?php echo uk86_urlAdmin('groupbuy', 'groupbuy_review_fail');?>';
                var groupbuy_id = $(this).attr('data-groupbuy-id');
                $('#op_form').attr('action', action);
                $('#groupbuy_id').val(groupbuy_id);
        ...
        $('[nctype="btn_cancel"]').on('click', function() {
            if(confirm('确认取消该抢购活动？')) {
                var action = '<?php echo uk86_urlAdmin('groupbuy', 'groupbuy_cancel');?>';
                var groupbuy_id = $(this).attr('data-groupbuy-id');
                $('#op_form').attr('action', action);
                $('#groupbuy_id').val(groupbuy_id);
                $('#op_form').submit();
        ...
        $('[nctype="btn_del"]').on('click', function() {
            if(confirm('确认删除该抢购活动？')) {
                var action = '<?php echo uk86_urlAdmin('groupbuy', 'groupbuy_del');?>';
                var groupbuy_id = $(this).attr('data-groupbuy-id');
                $('#op_form').attr('action', action);
                $('#groupbuy_id').val(groupbuy_id);
                $('#op_form').submit();
