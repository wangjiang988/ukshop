...
<div class="page">
  ...
  <table class="table tb-type2" >
    ...
    <tbody>
      ...
      <tr class="noborder">
        <td class="vatop rowform"><form id="add_form" action="<?php echo uk86_urlAdmin('store', 'store_bind_class_add');?>" method="post">
...
$(document).ready(function(){
    ...
    $('[nctype="btn_del_store_bind_class"]').on('click', function() {
        if(confirm('确认删除？删除后店铺对应分类商品将全部下架')) {
            ...
            $this = $(this);
            $.post('<?php echo uk86_urlAdmin('store', 'store_bind_class_del');?>', {bid: bid}, function(data) {
                 if(data.result) {
