  ...
  <form id="web_form" method="post" name="form1">
    ...
    <div class="ncap-form-default">
      ...
      <dl class="row">
        ...
        <dd class="opt">
          <select name="area_id" id="area_id" class=" w200" onchange="check_city_areas();">
            <option value="0"><?php echo $lang['nc_please_choose'];?></option>
            <?php if(!empty($output['service_cities']) && is_array($output['parent_goods_class'])){ ?>
              <?php foreach($output['service_cities'] as $k => $v){ ?>
                <option <?php if($v['area_id'] == $output['area_id']){?>selected<?php }?> value="<?php echo $v['area_id'];?>"><?php echo $v['area_name'];?></option>
              <?php } ?>
            <?php } ?>
          </select>
          <span class="err"></span>
          <p class="notic">
            一个城市只能绑定一个频道。如果想添加除列表外的别的城市，请前往本地化城市列表里进行开通。<br/>
            新生成的城市频道请前往配置频道首页模块<br/></p>
        </dd>
      </dl>
      <dl class="row">
        <dt class="tit">
...
function check_goods_class() {
    ...
    if (gc_id>0 && gc_id!="<?php echo $output['gc_id'];?>") {
    	...
    	if(get_text!='') {
          ...
          $('#area_id').val(0);    //绑定分类的时候不能绑定城市
        }
	}
}
function check_city_areas() {
  var area_id = $('#area_id').val();
  if (area_id>0 && area_id!="<?php echo $output['area_id'];?>") {
    var get_text = $.ajax({
      url: 'index.php?act=web_channel&op=check_city_areas&area_id='+area_id,
      async: false
    }).responseText;
    if(get_text!='') {
      alert('该城市已经绑定到“'+get_text+'”频道，请选择其它城市。');
      $('#area_id').val(0);
    }else{
      $('#gc_id').val(0);              //绑定城市的时候不能绑定分类
    }
  }
}
